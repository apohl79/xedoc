...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a Xedoc owner digest."
    )
    parser.add_argument(
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(xedoc(?: desktop| app|\.app| cli)?|desktop|windows xedoc app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
