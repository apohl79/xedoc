...
description: Upgrade the local apohl79 Xedoc fork to a newer stable OpenAI release. Use when discovering newer upstream releases, staging a fork upgrade on an upgrade branch, replaying upstream commits through alpha and stable checkpoints, or preserving and auditing the README.fork.md inventory.
---
...
```bash
base_version=$(sed -n 's/^version = "\([0-9][^"]*\)"/\1/p' xedoc-rs/Cargo.toml | head -1)
...
CLI. `xedoc debug models` returns the provider catalog as JSON; `--bundled` is
...
while read -r upstream_commit; do
  ...
  # Run just fmt from xedoc-rs whenever code changed, then repeat the
...
the development profile; preserve `xedoc-rs/target/release` and the
...
verified Xedoc-built V8 artifact pair through `scripts/xedoc_package/v8.py`
...
used by app-server integration tests: `xedoc`, `xedoc-code-mode-host`, and
