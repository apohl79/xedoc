...
Use `TestXedocBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$XEDOC_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `xedoc_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap xedoc_remote_env_cleanup EXIT

  cd xedoc-rs
  just test -p xedoc-core --test all
'
```
...
bash -c '
  ...
  cd xedoc-rs
  just test -p xedoc-app-server --test all
'
```
...
```sh
bazel test //xedoc-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `xedoc` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of xedoc in `~/code/xedoc`. Reset files if needed. Multiple checkouts take longer to build and take up more space.
