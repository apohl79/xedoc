...
This crate implements the business logic for Xedoc. It is designed to be used by the various Xedoc UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //xedoc-rs/core:core-all-wine-exec-test`.
...
Note that `xedoc-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.xedoc` read-only.
...
Expects the binary containing `xedoc-core` to run the equivalent of `xedoc sandbox` when `arg0` is `xedoc-linux-sandbox`. See the `xedoc-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `xedoc-resources/bwrap`
binary shipped with Xedoc and Xedoc surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Xedoc also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Xedoc rejects sandboxed shell commands that would enter the
...
Expects the binary containing `xedoc-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--xedoc-run-as-apply-patch`. See the
`xedoc-arg0` crate for details.
