...
use tracing::warn;
use xedoc_config::ConfigEditsBuilder;
use xedoc_config::McpServerConfig;
use xedoc_config::load_global_mcp_servers;
use xedoc_core_mcp_skill_dependency::canonical_mcp_server_key;
use xedoc_core_mcp_skill_dependency::collect_missing_mcp_dependencies;
use xedoc_core_mcp_skill_dependency::format_missing_mcp_dependencies;
use xedoc_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(xedoc_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&xedoc_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&xedoc_home)
