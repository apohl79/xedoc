...
use xedoc_exec_server::SelectedCapabilityRootsStatus;
...
use xedoc_thread_store::ThreadStoreResult;
...
/// Explains why `XedocThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
impl ThreadConfigSnapshot {
    ...
    pub fn sandbox_policy(&self) -> SandboxPolicy {
        xedoc_sandboxing::compatibility_sandbox_policy_for_permission_profile(
...
/// (formerly called a conversation) in Xedoc.
impl XedocThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> XedocResult<String> {
    ...
    pub async fn shutdown_and_wait(&self) -> XedocResult<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(xedoc_extension_api::ThreadResumeInput {
    ...
    pub async fn emit_event(&self, msg: xedoc_protocol::protocol::EventMsg) {
        self.session
            .send_event_raw(xedoc_protocol::protocol::Event {
                id: String::new(),
                msg,
            })
            .await;
    ...
    pub fn session_source(&self) -> &xedoc_protocol::protocol::SessionSource {
        &self.session_source
    }
    ...
    /// callers that only hold a `XedocThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: XedocThreadSettingsOverrides,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> XedocResult<()> {
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> XedocResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> XedocResult<()> {
        if items.is_empty() {
    ...
    pub async fn runtime_mcp_config(&self, config: &crate::config::Config) -> xedoc_mcp::McpConfig {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> XedocResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            XedocErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> XedocResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(XedocErr::InvalidRequest(
