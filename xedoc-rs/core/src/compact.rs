...
#[cfg(test)]
use xedoc_core_context_manager::content_items_to_text;
use xedoc_protocol::error::Result as XedocResult;
use xedoc_protocol::error::XedocErr;
use xedoc_protocol::items::ContextCompactionItem;
...
pub use xedoc_prompts::SUMMARIZATION_PROMPT;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = XedocErr::TurnAborted;
    ...
    let (summary_suffix, history_items) = if use_hierarchical_compaction {
        ...
        loop {
            ...
            match attempt_result {
                ...
                Err(err @ (XedocErr::Interrupted | XedocErr::TurnAborted)) => {
                    return Err(err);
                }
                Err(e @ XedocErr::SessionBudgetExceeded) => {
                    let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                    sess.send_event(&turn_context, event).await;
                    return Err(e);
                }
                Err(e @ XedocErr::ContextWindowExceeded) => {
                    if turn_input_len > 1 {
