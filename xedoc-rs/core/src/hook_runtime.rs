...
use xedoc_core_client::responses_metadata::CompactionTrigger;
use xedoc_hooks::PermissionRequestDecision;
use xedoc_hooks::PermissionRequestOutcome;
use xedoc_hooks::PermissionRequestRequest;
use xedoc_hooks::PostToolUseOutcome;
use xedoc_hooks::PostToolUseRequest;
use xedoc_hooks::PreToolUseOutcome;
use xedoc_hooks::PreToolUseRequest;
use xedoc_hooks::SessionStartOutcome;
use xedoc_hooks::StartHookTarget;
use xedoc_hooks::StopHookTarget;
use xedoc_hooks::StopOutcome;
use xedoc_hooks::SubagentHookContext;
use xedoc_hooks::UserPromptSubmitOutcome;
use xedoc_hooks::UserPromptSubmitRequest;
use xedoc_otel::HOOK_RUN_DURATION_METRIC;
use xedoc_otel::HOOK_RUN_METRIC;
...
use xedoc_protocol::protocol::HookCompletedEvent;
use xedoc_protocol::protocol::HookEventName;
use xedoc_protocol::protocol::HookRunStatus;
use xedoc_protocol::protocol::HookRunSummary;
use xedoc_protocol::protocol::HookSource;
use xedoc_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    xedoc_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = xedoc_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = xedoc_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = xedoc_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = xedoc_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(xedoc_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: xedoc_hooks::HookEvent::AfterAgent {
                event: xedoc_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            xedoc_hooks::HookResult::Success => continue,
            xedoc_hooks::HookResult::FailedContinue(error) => (error, false),
            xedoc_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(xedoc_protocol::protocol::ErrorEvent {
        message,
...
mod tests {
    ...
    use xedoc_protocol::protocol::HookEventName;
    use xedoc_protocol::protocol::HookExecutionMode;
    use xedoc_protocol::protocol::HookHandlerType;
    use xedoc_protocol::protocol::HookRunStatus;
    use xedoc_protocol::protocol::HookScope;
    use xedoc_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    xedoc_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
