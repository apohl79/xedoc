...
use xedoc_network_proxy::BlockedRequest;
...
use xedoc_network_proxy::NetworkPolicyRequest;
use xedoc_network_proxy::NetworkProtocol;
                ...
                if let Some(owner_call) = owner_call.as_ref() {
                    self.record_call_outcome(
                        ...
                        NetworkApprovalOutcome::DeniedByPolicy(
                            "approval review timed out".to_string(),
                        ),
                    )
                    .await;
                }
...
pub(crate) async fn begin_network_approval(
    ...
    let execution_proxy = network
        ...
        .map_err(|err| {
            ToolError::Xedoc(xedoc_protocol::error::XedocErr::Io(io::Error::other(
