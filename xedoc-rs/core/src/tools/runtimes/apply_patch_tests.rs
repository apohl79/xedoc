...
fn test_turn_environment(environment_id: &str) -> crate::session::turn_context::TurnEnvironment {
    crate::session::turn_context::TurnEnvironment::new(
        ...
        std::sync::Arc::new(xedoc_exec_server::Environment::default_for_tests()),
...
async fn permission_request_payload_uses_apply_patch_hook_name_and_aliases() {
    ...
    let req = ApplyPatchRequest {
        turn_environment: test_turn_environment(xedoc_exec_server::LOCAL_ENVIRONMENT_ID),
        action,
...
async fn file_system_sandbox_context_uses_active_attempt() {
    ...
    assert_eq!(
        sandbox.cwd,
        Some(xedoc_utils_path_uri::PathUri::from_abs_path(&path))
    );
