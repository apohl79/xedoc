...
use xedoc_execpolicy::Evaluation;
use xedoc_execpolicy::MatchOptions;
use xedoc_execpolicy::Policy;
use xedoc_execpolicy::RuleMatch;
...
use xedoc_protocol::protocol::NetworkPolicyRuleAction;
use xedoc_protocol::protocol::ReviewDecision;
use xedoc_sandboxing::SandboxCommand;
use xedoc_sandboxing::SandboxManager;
use xedoc_sandboxing::SandboxTransformRequest;
use xedoc_sandboxing::SandboxType;
use xedoc_sandboxing::SandboxablePreference;
use xedoc_shell_command::bash::parse_shell_lc_plain_commands;
use xedoc_shell_command::bash::parse_shell_lc_single_command_prefix;
use xedoc_shell_escalation::EscalateServer;
use xedoc_shell_escalation::EscalationDecision;
use xedoc_shell_escalation::EscalationExecution;
use xedoc_shell_escalation::EscalationPermissions;
use xedoc_shell_escalation::EscalationPolicy;
use xedoc_shell_escalation::EscalationPolicyFuture;
use xedoc_shell_escalation::EscalationSession;
use xedoc_shell_escalation::ExecParams;
use xedoc_shell_escalation::ExecResult;
use xedoc_shell_escalation::PreparedExec;
use xedoc_shell_escalation::ResolvedPermissionProfile;
use xedoc_shell_escalation::ShellCommandExecutor;
use xedoc_shell_escalation::ShellCommandExecutorFuture;
use xedoc_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        arg0,
        sandbox_policy_cwd,
        xedoc_linux_sandbox_exe: ctx.turn.config.xedoc_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    xedoc_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<xedoc_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["XEDOC_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            xedoc_linux_sandbox_exe: self.xedoc_linux_sandbox_exe.as_deref(),
        ...
        if let Some(network) = exec_request.network.as_ref() {
            network
                ...
                .map_err(|err| {
                    ...
                    XedocErr::Io(io::Error::other(format!(
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Xedoc(XedocErr::Sandbox(SandboxErr::Timeout {
            output: Box::new(output),
        })));
    }

    if is_likely_sandbox_denied(sandbox, &output) {
        return Err(ToolError::Xedoc(XedocErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),
