...
use xedoc_models_manager::manager::MultiProviderModelsManager;
...
use xedoc_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.xedoc_home)
        .await
        .expect("xedoc home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    xedoc_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: xedoc_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn spawn_agent_role_service_tier_falls_back_to_supported_parent_tier() {
    ...
    tokio::fs::create_dir_all(&turn.config.xedoc_home)
        .await
        .expect("xedoc home should be created");
    let role_config_path = turn.config.xedoc_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.xedoc_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);
