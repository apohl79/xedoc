...
use xedoc_tools::ToolCall as ExtensionToolCall;
use xedoc_tools::ToolEnvironment;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn xedoc_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn xedoc_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let xedoc_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    impl xedoc_extension_api::ToolExecutor<xedoc_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> xedoc_tools::ToolName {
            xedoc_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> xedoc_tools::ToolSpec {
            xedoc_tools::ToolSpec::Function(xedoc_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: xedoc_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: xedoc_tools::ToolCall) -> xedoc_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(xedoc_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn xedoc_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<xedoc_tools::ToolCall>>>,
    }

    impl xedoc_extension_api::ToolExecutor<xedoc_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> xedoc_tools::ToolName {
            xedoc_tools::ToolName::plain("extension_echo")
        }
        ...
        fn handle(&self, call: xedoc_tools::ToolCall) -> xedoc_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: xedoc_tools::ToolCall,
        ) -> Result<Box<dyn xedoc_tools::ToolOutput>, xedoc_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: xedoc_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = xedoc_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        xedoc_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        xedoc_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
