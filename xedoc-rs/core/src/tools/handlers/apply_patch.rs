...
use xedoc_apply_patch::ApplyPatchAction;
use xedoc_apply_patch::ApplyPatchFileChange;
use xedoc_apply_patch::Hunk;
use xedoc_apply_patch::StreamingPatchParser;
...
use xedoc_protocol::protocol::PatchApplyUpdatedEvent;
use xedoc_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use xedoc_sandboxing::policy_transforms::merge_permission_profiles;
use xedoc_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[xedoc_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &xedoc_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    xedoc_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        xedoc_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match xedoc_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match xedoc_apply_patch::verify_apply_patch_args(
            args,
        ...
        .await
        {
            xedoc_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            xedoc_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            xedoc_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            xedoc_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match xedoc_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        xedoc_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
        ...
        xedoc_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
