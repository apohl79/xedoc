...
pub(crate) use xedoc_core_tool_specs::multi_agents_common::MAX_SPAWN_AGENT_MODEL_OVERRIDES;
pub(crate) use xedoc_core_tool_specs::multi_agents_common::model_supports_multi_agent_backend;
...
pub(crate) fn collab_spawn_error(err: XedocErr) -> FunctionCallError {
    match err {
        XedocErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        XedocErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: XedocErr) -> FunctionCallError {
    match err {
        XedocErr::ThreadNotFound(id) => {
