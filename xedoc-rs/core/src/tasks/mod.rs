...
use xedoc_otel::TURN_E2E_DURATION_METRIC;
use xedoc_otel::TURN_NETWORK_PROXY_METRIC;
...
const TASK_COMPACT_METRIC: &str = "xedoc.task.compact";

pub(crate) type SessionTaskResult = XedocResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Xedoc workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`XedocErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            xedoc.turn.reasoning_effort = %reasoning_effort,
            xedoc.turn.token_usage.input_tokens = field::Empty,
            xedoc.turn.token_usage.cached_input_tokens = field::Empty,
            xedoc.turn.token_usage.cache_write_input_tokens = field::Empty,
            xedoc.turn.token_usage.non_cached_input_tokens = field::Empty,
            xedoc.turn.token_usage.output_tokens = field::Empty,
            xedoc.turn.token_usage.reasoning_output_tokens = field::Empty,
            xedoc.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Xedoc will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub async fn abort_all_tasks(self: &Arc<Self>, reason: TurnAbortReason) {
        self.cancel_model_router_script_invocation().await;
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(XedocErr::TurnAborted) => (None, Some(TurnAbortReason::Interrupted)),
            Err(err) => {
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "xedoc.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "xedoc.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "xedoc.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "xedoc.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "xedoc.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "xedoc.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "xedoc.turn.token_usage.total_tokens",
