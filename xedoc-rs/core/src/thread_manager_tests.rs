...
impl xedoc_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: xedoc_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> xedoc_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<xedoc_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> xedoc_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<xedoc_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> xedoc_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.xedoc_home = temp_dir.path().join("xedoc-home").abs();
    config.cwd = config.xedoc_home.abs();
    std::fs::create_dir_all(&config.xedoc_home).expect("create xedoc home");
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl xedoc_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: xedoc_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl xedoc_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        ) -> xedoc_extension_api::ExtensionFuture<'a, Vec<xedoc_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let server = xedoc_config::types::McpServerConfig {
                    auth: Default::default(),
                    transport: xedoc_config::McpServerTransportConfig::StreamableHttp {
                ...
                vec![xedoc_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let mut extensions = xedoc_extension_api::ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(recorder.clone());
    extensions.mcp_server_contributor(recorder);
    let auth_manager =
        AuthManager::from_auth_for_testing(XedocAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = xedoc_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread_with_options(StartThreadOptions {
            ...
            thread_source: None,
            dynamic_tools: Vec::new(),
            metrics_service_name: Some("xedoc_work_desktop".to_string()),
            parent_trace: None,
            environments: Vec::new(),
    ...
    let selected_servers = |config: &xedoc_mcp::McpConfig| {
        xedoc_mcp::configured_mcp_servers(config)
            .into_iter()
...
#[tokio::test]
async fn explicit_installation_id_skips_xedoc_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.xedoc_home = temp_dir.path().join("xedoc-home").abs();
    config.cwd = config.xedoc_home.abs();
    ...
    assert!(!config.xedoc_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn provider_profile_catalog_is_used_for_non_active_provider() -> std::io::Result<()> {
    ...
    let catalog_path = xedoc_home.path().join("anthropic-models.json");
...
"#,
    ...
    std::fs::write(
        xedoc_home.path().join("claude.config.toml"),
...
async fn active_provider_ignores_legacy_shared_models_cache() {
    ...
    let legacy_cache = serde_json::json!({
        ...
        "etag": null,
        "client_version": xedoc_models_manager::client_version_to_whole(),
        "models": [],
    });
    std::fs::write(
        config.xedoc_home.join("models_cache.json"),
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.xedoc_home.join("models_cache.json").exists());
}
