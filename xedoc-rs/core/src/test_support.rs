...
use xedoc_models_manager::collaboration_mode_presets;
...
pub fn auth_manager_from_auth(auth: XedocAuth) -> Arc<AuthManager> {
    AuthManager::from_auth_for_testing(auth)
}

pub fn auth_manager_from_auth_with_home(auth: XedocAuth, xedoc_home: PathBuf) -> Arc<AuthManager> {
    AuthManager::from_auth_for_testing_with_home(auth, xedoc_home)
}
...
) -> xedoc_protocol::error::Result<crate::NewThread> {
    thread_manager
...
pub fn models_manager_with_provider(
    ...
    provider.models_manager(xedoc_home, /*config_model_catalog*/ None)
}
...
#[derive(Clone, Copy)]
pub enum TestXedocResponsesRequestKind {
    Turn,
    Prewarm,
    WebsocketConnection,
}
...
pub fn responses_metadata(
    ...
    let request_kind = match request_kind {
        ...
        TestXedocResponsesRequestKind::Prewarm => Some(XedocResponsesRequestKind::Prewarm),
        TestXedocResponsesRequestKind::WebsocketConnection => None,
    };
    XedocResponsesMetadata {
