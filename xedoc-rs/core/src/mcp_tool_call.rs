...
use xedoc_core_approval_policy::MCP_TOOL_APPROVAL_QUESTION_ID_PREFIX;
use xedoc_core_approval_policy::McpToolApprovalDecision;
use xedoc_core_approval_policy::McpToolApprovalElicitationRequest;
use xedoc_core_approval_policy::McpToolApprovalKey;
use xedoc_core_approval_policy::McpToolApprovalMetadata;
use xedoc_core_approval_policy::build_mcp_tool_approval_display_params;
use xedoc_core_approval_policy::build_mcp_tool_approval_elicitation_request;
use xedoc_core_approval_policy::build_mcp_tool_approval_question;
use xedoc_core_approval_policy::mcp_tool_approval_prompt_options;
use xedoc_core_approval_policy::normalize_approval_decision_for_mode;
use xedoc_core_approval_policy::parse_mcp_tool_approval_elicitation_response;
use xedoc_core_approval_policy::parse_mcp_tool_approval_response;
use xedoc_core_approval_policy::persistent_mcp_tool_approval_key;
use xedoc_core_approval_policy::requires_mcp_tool_approval_for_mode;
use xedoc_core_approval_policy::session_mcp_tool_approval_key;
use xedoc_core_tool_output::sanitize_mcp_tool_result_for_model;
use xedoc_core_tool_output::truncate_mcp_tool_result_for_event;
use xedoc_features::Feature;
use xedoc_hooks::PermissionRequestDecision;
use xedoc_mcp::McpConnectionManager;
use xedoc_mcp::McpPermissionPromptAutoApproveContext;
use xedoc_mcp::SandboxState;
use xedoc_mcp::mcp_permission_prompt_is_auto_approved;
use xedoc_mcp::tool_is_model_visible;
use xedoc_protocol::items::McpToolCallError;
use xedoc_protocol::items::McpToolCallItem;
use xedoc_protocol::items::McpToolCallStatus;
...
use xedoc_core_mcp_runtime::tool_call_telemetry::McpCallMetricOutcome;
use xedoc_core_mcp_runtime::tool_call_telemetry::McpToolCallSpanFields;
use xedoc_core_mcp_runtime::tool_call_telemetry::emit_mcp_call_metrics;
use xedoc_core_mcp_runtime::tool_call_telemetry::mcp_call_metric_outcome;
use xedoc_core_mcp_runtime::tool_call_telemetry::mcp_tool_call_span;
use xedoc_core_mcp_runtime::tool_call_telemetry::record_mcp_result_span_telemetry;
use xedoc_core_mcp_runtime::tool_item_metadata::McpToolCallItemMetadata;
use xedoc_core_mcp_runtime::tool_metadata::get_mcp_app_resource_uri;
use xedoc_core_mcp_runtime::tool_request_metadata::build_mcp_tool_call_request_meta;
use xedoc_core_mcp_runtime::tool_request_metadata::with_mcp_tool_call_thread_id_meta;
const MCP_TOOL_CALL_EVENT_RESULT_MAX_BYTES: usize = xedoc_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let metadata = lookup_mcp_tool_metadata(manager, &server, &tool_name).await;
    ...
    let approval_mode = if let Some(approval_mode) = {
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let request_meta = build_mcp_tool_call_request_meta(
            crate::X_XEDOC_TURN_METADATA_HEADER,
            turn_context
                .turn_metadata_state
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let server_environment_id = manager
        ...
        .unwrap_or(xedoc_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID);
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        xedoc_linux_sandbox_exe: step_context.mcp.config().xedoc_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                xedoc_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == xedoc_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, xedoc_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result =
        persist_non_app_mcp_tool_approval(sess, &turn_context.config, &key.server, &tool_name)
            .await;
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, xedoc_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
