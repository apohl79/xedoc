...
use xedoc_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(xedoc_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<XedocThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<XedocThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &XedocThread) -> String {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<XedocThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<XedocThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(XedocErr::ThreadNotFound(id)) => assert_eq!(id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, XedocErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn v2_communication_reloads_registered_unloaded_agent_and_preserves_status() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(XedocErr::ThreadNotFound(id)) => assert_eq!(id, spawned_agent.thread_id),
        Err(err) => panic!("expected ThreadNotFound, got {err:?}"),
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        ...
        harness.config.xedoc_home.to_path_buf(),
        std::sync::Arc::new(xedoc_exec_server::EnvironmentManager::default_for_tests()),
...
#[tokio::test]
async fn plaintext_inter_agent_envelope_records_payload_as_last_task_message() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, _) = harness.start_thread().await;
    let agent_path = AgentPath::try_from("/root/worker").expect("agent path");
    let spawned_agent = harness
    ...
    assert_eq!(
        harness
            ...
            .agent_metadata_for_thread(spawned_agent.thread_id)
            .and_then(|metadata| metadata.last_task_message),
        Some("plain task".to_string())
    );
}

#[tokio::test]
async fn final_answer_completion_envelope_records_payload_as_last_task_message() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, _) = harness.start_thread().await;
    let agent_path = AgentPath::try_from("/root/worker").expect("agent path");
    let spawned_agent = harness
    ...
    assert_eq!(
        harness
            ...
            .agent_metadata_for_thread(spawned_agent.thread_id)
            .and_then(|metadata| metadata.last_task_message),
        Some("completion result".to_string())
    );
}
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = xedoc_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let XedocErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected XedocErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let XedocErr::AgentLimitReached { max_threads } = err else {
        panic!("expected XedocErr::AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "xedoc_work_desktop");
...
async fn resume_thread_subagent_restores_stored_metadata() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(XedocAuth::from_api_key("dummy"));
