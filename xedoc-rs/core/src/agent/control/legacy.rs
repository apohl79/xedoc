...
impl AgentControl {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> XedocResult<String> {
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> XedocResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
                ...
                if !thread.config_snapshot().await.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            xedoc_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(XedocErr::ThreadNotFound(_)) if known_agent => {
    ...
    pub(crate) async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> XedocResult<String> {
        let descendant_ids = self.live_thread_spawn_descendants(agent_id).await?;
        let result = self.shutdown_live_agent(agent_id).await;
        for descendant_id in descendant_ids {
            match self.shutdown_live_agent(descendant_id).await {
                Ok(_) | Err(XedocErr::ThreadNotFound(_)) | Err(XedocErr::InternalAgentDied) => {}
