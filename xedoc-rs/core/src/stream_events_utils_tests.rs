...
impl TurnItemContributor for TestTurnItemContributor {
    ...
    ) -> xedoc_extension_api::ExtensionFuture<'a, Result<(), String>> {
        Box::pin(async move {
...
async fn handle_non_tool_response_item_runs_turn_item_contributors_only_when_requested() {
    ...
    let item = assistant_output_text("hello<oai-mem-citation>ignored</oai-mem-citation> world");
    ...
    assert!(matches!(provisional_turn_item, TurnItem::AgentMessage(_)));
