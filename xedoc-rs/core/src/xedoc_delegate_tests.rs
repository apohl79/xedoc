...
async fn forward_ops_preserves_submission_trace_context() {
    ...
    let submission = Submission {
        ...
        trace: Some(xedoc_protocol::protocol::W3cTraceContext {
            traceparent: Some(
...
#[tokio::test]
async fn run_xedoc_thread_interactive_respects_pre_cancelled_spawn() {
    ...
    assert!(matches!(result, Err(XedocErr::TurnAborted)));
}
