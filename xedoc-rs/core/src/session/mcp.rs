...
use xedoc_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use xedoc_exec_server::ResolvedSelectedCapabilityRoot;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        xedoc_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> xedoc_mcp::ElicitationLifecycle {
        let elicitations = self.services.elicitations.clone();
        xedoc_mcp::ElicitationLifecycle::new(move || elicitations.register())
    }
    ...
    pub async fn request_mcp_server_elicitation(
            ...
            .elicitations_auto_deny()
        {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: xedoc_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                xedoc_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                xedoc_protocol::mcp::RequestId::Integer(value)
            }
        };
