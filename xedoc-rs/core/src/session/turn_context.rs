...
pub(crate) use xedoc_core_environment::TurnEnvironment;
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let unified_exec_shell_mode = UnifiedExecShellMode::for_session(
            xedoc_tools::unified_exec_feature_mode_for_features(per_turn_config.features.get()),
        ...
        let extension_data = Arc::new(xedoc_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    ) -> XedocResult<Arc<TurnContext>> {
        let notify_config_contributors = !self.services.extensions.config_contributors().is_empty();
        let update_result: XedocResult<_> = {
            ...
            match state.session_configuration.clone().apply(&updates) {
                ...
                Err(err) => Err(XedocErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (session_configuration, permission_profile_changed, previous_config, new_config) =
            match update_result {
                ...
                Err(err) => {
                    ...
                    .await;
                    return Err(XedocErr::InvalidRequest(message));
                }
            };
