...
use xedoc_core_skills::injection::HostSkillsCatalogInWorldState;
use xedoc_exec_server::EnvironmentManager;
use xedoc_execpolicy::prefix_rule_migration;
...
use xedoc_mcp::McpResourceClient;
...
use xedoc_network_proxy::NetworkProxyAuditMetadata;
use xedoc_network_proxy::normalize_host;
use xedoc_otel::current_span_trace_id;
...
use xedoc_protocol::approvals::ElicitationRequest;
use xedoc_protocol::approvals::ElicitationRequestEvent;
use xedoc_protocol::approvals::ExecPolicyAmendment;
use xedoc_protocol::approvals::NetworkPolicyAmendment;
...
use xedoc_protocol::dynamic_tools::DynamicToolSpec;
use xedoc_protocol::items::EnteredReviewModeItem;
...
use xedoc_protocol::models::AdditionalPermissionProfile;
...
use xedoc_protocol::models::format_allow_prefixes;
...
use xedoc_protocol::protocol::AdditionalContextEntry;
use xedoc_protocol::protocol::FileChange;
use xedoc_protocol::protocol::HasLegacyEvent;
...
use xedoc_protocol::protocol::RawResponseItemEvent;
...
use xedoc_protocol::protocol::TurnContextItem;
...
use xedoc_protocol::protocol::WorldStateItem;
...
use xedoc_thread_store::LiveThreadInitGuard;
use xedoc_thread_store::LocalThreadStore;
use xedoc_thread_store::ReadThreadParams;
use xedoc_thread_store::ResumeThreadParams;
use xedoc_thread_store::ThreadPersistenceMetadata;
        ...
        match self {
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                ...
                ErrorEvent {
                    ...
                    xedoc_error_info: Some(XedocErrorInfo::ActiveTurnNotSteerable {
                        turn_kind: *turn_kind,
                    }),
                }
            }
...
use xedoc_otel::THREAD_STARTED_METRIC;
...
use xedoc_protocol::models::LocalImagePreparation;
...
use xedoc_protocol::protocol::DeprecationNoticeEvent;
...
use xedoc_protocol::protocol::ModelRerouteEvent;
use xedoc_protocol::protocol::ModelRerouteReason;
use xedoc_protocol::protocol::ModelVerification;
use xedoc_protocol::protocol::ModelVerificationEvent;
use xedoc_protocol::protocol::NetworkApprovalContext;
...
use xedoc_protocol::protocol::SessionNetworkProxyRuntime;
use xedoc_protocol::protocol::StreamErrorEvent;
...
use xedoc_protocol::protocol::TurnModerationMetadataEvent;
...
use xedoc_tools::UnifiedExecShellMode;
use xedoc_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use xedoc_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) extensions: Arc<xedoc_extension_api::ExtensionRegistry<crate::config::Config>>,
...
impl Session {
    ...
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> XedocResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if xedoc_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> XedocResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if let Some(exec_policy) = &inherited_exec_policy {
                ...
                .ignore_user_and_project_exec_policy_rules()
            {
                ...
                let policy_path = default_policy_path(xedoc_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| XedocErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            xedoc_models_manager::manager::RefreshStrategy::Offline
        } else {
            xedoc_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.xedoc_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> XedocResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> XedocResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| XedocErr::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> XedocResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(XedocErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> XedocResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> XedocResult<PermissionProfileState> {
    ...
    match fragment.slot() {
        ...
        PromptSlot::ContextualUser => {
            ...
            match fragment.position {
                Some(xedoc_extension_api::PluginContextPosition::Preamble) => {
                    preamble_sections.push(text);
                }
                Some(xedoc_extension_api::PluginContextPosition::Supplement) => {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &xedoc_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn xedoc_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn xedoc_network_proxy::BlockedRequestObserver>>,
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Xedoc performance."
                            ),
                        }),
                    )
                    .await;
        ...
        let (
            ...
            if model_provider_changed {
                state.queue_pending_session_start_source(
                    xedoc_hooks::SessionStartSource::ModelChange,
                );
            }
    ...
    pub(crate) async fn refresh_runtime_config(&self, next_config: Config) {
        ...
        let (previous_config, new_config, config) = {
            ...
            config.token_usage_optimizer = next_config.token_usage_optimizer.clone();
            let config = Arc::new(config);
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .xedoc_error_info
                .as_ref()
                .is_some_and(XedocErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
    ...
    async fn forward_child_completion_to_parent(
        ...
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| xedoc_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => xedoc_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&xedoc_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Xedoc host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(xedoc_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        xedoc_error: XedocErr,
    ) {
        let additional_details = xedoc_error.to_string();
        let xedoc_error_info = XedocErrorInfo::ResponseStreamDisconnected {
            http_status_code: xedoc_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            xedoc_error_info: Some(xedoc_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<xedoc_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::XedocHooks),
