...
use xedoc_core_client::responses_metadata::CompactionPhase;
use xedoc_core_client::responses_metadata::CompactionReason;
use xedoc_core_skills::injection::InjectedHostSkillPrompts;
use xedoc_extension_api::TurnInputContext;
use xedoc_extension_api::TurnInputEnvironment;
use xedoc_features::Feature;
use xedoc_git_utils::get_git_repo_root_with_fs;
...
use xedoc_protocol::items::PlanItem;
...
use xedoc_protocol::protocol::AgentMessageContentDeltaEvent;
use xedoc_protocol::protocol::AgentReasoningSectionBreakEvent;
use xedoc_protocol::protocol::ErrorEvent;
use xedoc_protocol::protocol::EventMsg;
use xedoc_protocol::protocol::MultiAgentVersion;
use xedoc_protocol::protocol::PlanDeltaEvent;
use xedoc_protocol::protocol::RawResponseCompletedEvent;
use xedoc_protocol::protocol::ReasoningContentDeltaEvent;
use xedoc_protocol::protocol::ReasoningRawContentDeltaEvent;
use xedoc_protocol::protocol::SafetyBufferingEvent;
...
use xedoc_utils_stream_parser::AssistantTextChunk;
use xedoc_utils_stream_parser::AssistantTextStreamParser;
use xedoc_utils_stream_parser::ProposedPlanSegment;
use xedoc_utils_stream_parser::extract_proposed_plan_text;
use xedoc_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<xedoc_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        ...
        let error = err.to_xedoc_protocol_error();
    ...
    // Although from the perspective of xedoc.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: XedocResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                XedocResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if continued_due_to_output_limit && !made_progress {
                    ...
                    {
                        let error = XedocErr::RepeatedOutputLimit;
                        sess.emit_turn_error_lifecycle(
                            turn_context.as_ref(),
                            error.to_xedoc_protocol_error(),
                        )
                        .await;
                        sess.send_event(
                            &turn_context,
            ...
            Err(err @ XedocErr::TurnAborted) => {
                return Err(err);
            }
            Err(XedocErr::InvalidImageRequest()) => {
                let error = XedocErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    xedoc_error_info: Some(error),
                });
                sess.send_event(&turn_context, event).await;
                break;
            }
            Err(XedocErr::ContextWindowExceeded) if !retried_after_context_window_error => {
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_xedoc_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
...
async fn build_skills_and_plugins(
    ...
    let mcp_tools = if mentioned_plugins.is_empty() {
        ...
        // Plugin mentions need the raw MCP inventory so we can describe the
        // plugin's currently usable capabilities for this turn.
        step_context
            .mcp
            .manager_arc()
            .list_all_tools()
...
async fn run_pre_sampling_compact(
    ...
    maybe_run_model_switch_inline_compact(sess, turn_context, client_session).await?;
    let token_status =
...
async fn maybe_run_model_switch_request_baseline_compact(
    ...
    client_session: &mut ModelClientSession,
    cancellation_token: &CancellationToken,
) -> XedocResult<(Option<Arc<ToolRouter>>, bool)> {
...
/// Runs pre-sampling compaction against the target model when its compaction compatibility hash
/// changed or when switching to a smaller context-window model.
///
/// Returns `Err(_)` only when compaction was attempted and failed.
async fn maybe_run_model_switch_inline_compact(
    sess: &Arc<Session>,
    turn_context: &Arc<TurnContext>,
    client_session: &mut ModelClientSession,
) -> XedocResult<()> {
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<xedoc_extension_api::ExtensionData>,
    ...
    cancellation_token: CancellationToken,
) -> XedocResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(XedocErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(XedocErr::ContextWindowExceeded);
            }
            Err(XedocErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(XedocErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(super) fn agent_message_text(item: &xedoc_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: xedoc_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(xedoc_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &xedoc_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, XedocResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> XedocResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> XedocResult<SamplingRequestResult> {
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, XedocResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: XedocResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            xedoc.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            xedoc.usage.reasoning_output_tokens = field::Empty,
            xedoc.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(xedoc_async_utils::CancelErr::Cancelled) => break Err(XedocErr::TurnAborted),
        };
                    ...
                    && let Some(parent_thread_id) = turn_context.parent_thread_id
                {
                    sess.services.agent_control.record_sub_agent_activity(
                        ...
                        turn_context
                            .session_source
                            .get_agent_path()
                            .unwrap_or_else(xedoc_protocol::AgentPath::root),
                        &item,
                    );
                }
                        ...
                        if stream_item_to_client {
                            ...
                            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                                agent_message.content =
                                    vec![xedoc_protocol::items::AgentMessageContent::Text {
                                        text: if plan_mode {
                                            String::new()
                                        } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(XedocErr::TurnAborted);
    }
