...
use xedoc_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use xedoc_config::LoaderOverrides;
use xedoc_config::NetworkConstraints;
use xedoc_config::NetworkDomainPermissionToml;
use xedoc_config::NetworkDomainPermissionsToml;
use xedoc_config::RequirementSource;
...
use xedoc_models_manager::model_info;
use xedoc_models_manager::test_support::construct_model_info_offline_for_tests;
use xedoc_models_manager::test_support::get_model_offline_for_tests;
...
use xedoc_protocol::exec_output::ExecToolCallOutput;
...
use xedoc_protocol::models::ImageDetail;
use xedoc_protocol::models::PermissionProfile;
use xedoc_protocol::models::SandboxEnforcement;
...
use xedoc_protocol::protocol::NonSteerableTurnKind;
...
use core_test_support::test_xedoc::local;
...
use xedoc_config::config_toml::ProjectConfig;
use xedoc_config::permissions_toml::FilesystemPermissionToml;
use xedoc_config::permissions_toml::FilesystemPermissionsToml;
use xedoc_config::permissions_toml::NetworkToml;
use xedoc_config::permissions_toml::PermissionProfileToml;
use xedoc_config::permissions_toml::PermissionsToml;
use xedoc_execpolicy::Decision;
use xedoc_execpolicy::NetworkRuleProtocol;
use xedoc_execpolicy::Policy;
use xedoc_network_proxy::NetworkProxyConfig;
use xedoc_otel::MetricsClient;
use xedoc_otel::MetricsConfig;
use xedoc_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use xedoc_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use xedoc_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use xedoc_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use xedoc_protocol::models::InternalChatMessageMetadataPassthrough;
...
use xedoc_protocol::protocol::CreditsSnapshot;
use xedoc_protocol::protocol::GranularApprovalConfig;
...
use xedoc_protocol::protocol::NetworkApprovalProtocol;
...
use xedoc_protocol::protocol::SessionMeta;
use xedoc_protocol::protocol::SessionMetaLine;
use xedoc_protocol::protocol::SkillScope;
...
use xedoc_protocol::protocol::UserMessageEvent;
...
use xedoc_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "xedoc-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("xedoc-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        xedoc_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_xedoc: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_xedoc)?;
    std::fs::write(
        dot_xedoc.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        xedoc_home.join(xedoc_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<xedoc_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&xedoc_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: xedoc_hooks::StartHookTarget::SessionStart {
                source: xedoc_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn xedoc_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { xedoc_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> xedoc_sandboxing::SandboxablePreference {
            xedoc_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(xedoc_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: xedoc_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    ...
    let config_toml_path = xedoc_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = xedoc_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let xedoc_home = session.xedoc_home().await;
    std::fs::create_dir_all(&xedoc_home).expect("create xedoc home");
    let base_config_path = xedoc_home.join(CONFIG_TOML_FILE);
    let profile_config_path = xedoc_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(xedoc_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable Xedoc hooks");
    })
    .await?;
    let xedoc_home = session.xedoc_home().await;
    std::fs::create_dir_all(&xedoc_home)?;
    let config_toml_path = xedoc_home.join(CONFIG_TOML_FILE);
    let user_config: xedoc_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = xedoc_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = xedoc_hooks::list_hooks(xedoc_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        xedoc_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: xedoc_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: xedoc_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: xedoc_config::MatcherGroup {
                matcher: None,
                hooks: vec![xedoc_config::HookHandlerConfig::Command {
        ...
        let identity = xedoc_config::TomlValue::try_from(identity)?;
        xedoc_config::version_for_toml(&identity)
    };
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        XedocAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {
...
#[tokio::test]
async fn record_initial_history_does_not_restore_parent_cost_for_fork() {
    let (session, _turn_context) = make_session_and_context().await;
    session
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl xedoc_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a xedoc_extension_api::ExtensionData,
            thread_store: &'a xedoc_extension_api::ExtensionData,
            turn_store: &'a xedoc_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder = xedoc_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl xedoc_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: xedoc_extension_api::TurnStartInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl xedoc_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: xedoc_extension_api::TurnErrorInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: XedocErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, XedocErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl xedoc_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &xedoc_extension_api::ExtensionData,
            thread_store: &xedoc_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: xedoc_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_xedoc().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_xedoc().with_config(|config| {
        config.permissions.approval_policy =
            xedoc_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.xedoc.ensure_rollout_materialized().await;
    initial
        .xedoc
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        xedoc_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "xedoc_core__xedoc_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: xedoc_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            xedoc_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            xedoc_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            xedoc_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.xedoc_error_info,
        Some(XedocErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let config = build_test_config(xedoc_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(xedoc_protocol::account::PlanType::Plus),
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(xedoc_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.xedoc_error_info == Some(XedocErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(xedoc_home: &Path) -> Config {
...
async fn session_provider_update_replaces_provider_metadata() {
    ...
    assert!(matches!(
        ...
        Some(xedoc_hooks::SessionStartSource::ModelChange)
    ));
}
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = xedoc_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        xedoc_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    ...
    std::fs::write(
        xedoc_home.path().join(xedoc_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let xedoc_home = parent_config.xedoc_home.clone();
    let skill_dir = xedoc_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&xedoc_exec_server::LOCAL_FS));
    ...
    let role_path = xedoc_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let xedoc_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(xedoc_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(XedocAuth::from_api_key("Test API Key"));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(xedoc_extension_api::ExtensionRegistryBuilder::new().build()),
        xedoc_extension_api::ExtensionDataInit::default(),
        /*supports_openai_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        Arc::new(xedoc_thread_store::LocalThreadStore::new(
            xedoc_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(xedoc_mcp::McpRuntime::new(
    ...
    let services = SessionServices {
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(xedoc_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: xedoc_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: xedoc_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: xedoc_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(xedoc_thread_store::LocalThreadStore::new(
            xedoc_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            xedoc_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(xedoc_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(xedoc_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            xedoc_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(xedoc_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = xedoc_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(xedoc_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    xedoc_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(xedoc_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: xedoc_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(xedoc_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = xedoc_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        xedoc_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = xedoc_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(xedoc_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn xedoc_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        xedoc_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl xedoc_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: xedoc_extension_api::ThreadStopInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl xedoc_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: xedoc_extension_api::ThreadStopInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl xedoc_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: xedoc_extension_api::TurnAbortInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(xedoc_home).await;
...
async fn refresh_mcp_servers_keeps_the_previous_runtime_alive() {
    ...
    assert!(Arc::ptr_eq(&step_context.mcp, &old_runtime));
    assert_eq!(
        xedoc_mcp::configured_mcp_servers(new_runtime.config()),
...
impl xedoc_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: xedoc_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<xedoc_exec_server::NoiseRendezvousConnectBundle, xedoc_exec_server::ExecServerError>,
    > {
...
async fn deferred_environment_roots_refresh_plugin_availability() {
    ...
    impl xedoc_extension_api::McpServerContributor<Config> for ReadyPluginContributor {
        ...
        fn contribute<'a>(
            &'a self,
            context: xedoc_extension_api::McpServerContributionContext<'a, Config>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, Vec<xedoc_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                available
                    .then(
                        || xedoc_extension_api::McpServerContribution::SelectedPluginPackage {
    ...
    let (mut session, turn_context) = make_session_and_context().await;
    let mut registry = xedoc_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root = xedoc_protocol::capabilities::SelectedCapabilityRoot {
        id: "skill-only".to_string(),
        location: xedoc_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    registration
        .complete(Ok(xedoc_exec_server::EnvironmentReadyInfo {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let environment_manager = session.services.turn_environments.environment_manager();
    let selected_root =
        |environment_id: &str, path: &str| xedoc_protocol::capabilities::SelectedCapabilityRoot {
    ...
    for selected_root in &selected_roots {
        let xedoc_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
...
impl xedoc_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a xedoc_extension_api::ExtensionData,
        thread_store: &'a xedoc_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<xedoc_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    xedoc_extension_api::PromptFragment::developer_policy(
...
-> Arc<xedoc_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = xedoc_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl xedoc_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: xedoc_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<xedoc_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<xedoc_thread_store::InMemoryThreadStore> {
    let store = Arc::new(xedoc_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn xedoc_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &xedoc_thread_store::InMemoryThreadStore,
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = xedoc_protocol::user_input::TextElement::new(
        xedoc_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl xedoc_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: xedoc_extension_api::ThreadIdleInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn thread_idle_lifecycle_waits_for_trigger_turn_mailbox_work() {
    ...
    impl xedoc_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: xedoc_extension_api::ThreadIdleInput<'a>,
        ) -> xedoc_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            })
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(xedoc_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use xedoc_protocol::protocol::AskForApproval;
    use xedoc_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: xedoc_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: xedoc_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let xedoc_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_xedoc = project_root.join(".xedoc");
    let nested_dot_xedoc = nested.join(".xedoc");

    std::fs::create_dir_all(&xedoc_home)?;
    std::fs::create_dir_all(&nested_dot_xedoc)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_xedoc)?;
    write_project_hooks(&nested_dot_xedoc)?;
    write_project_trust_config(&xedoc_home, &[(&nested, TrustLevel::Trusted)]).await?;
    ...
    let expected_source_path = xedoc_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_xedoc.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_xedoc = project_root.join(".xedoc");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_xedoc)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let xedoc_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&xedoc_home)?;
        write_project_trust_config(&xedoc_home, &trust_entries).await?;
