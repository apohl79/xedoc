...
pub(crate) struct SessionConfiguration {
    ...
    /// Optional source classification for this thread.
...
impl SessionConfiguration {
    ...
    pub(crate) fn xedoc_home(&self) -> &AbsolutePathBuf {
        &self.xedoc_home
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: xedoc_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        extensions: Arc<xedoc_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        thread_extension_init.insert(xedoc_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = xedoc_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let auth_and_mcp_fut = async move {
            ...
            let mcp_servers = xedoc_mcp::effective_mcp_servers(mcp_config);
            let tool_plugin_provenance = xedoc_mcp::tool_plugin_provenance(mcp_config);
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let config_path = config.xedoc_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = auth.map(XedocAuth::auth_mode).map(TelemetryAuthMode::from);
            let account_id = auth.and_then(XedocAuth::get_account_id);
            let account_email = auth.and_then(XedocAuth::get_account_email);
            let originator = session_configuration.originator.clone();
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                &session_configuration.provider,
                auth_manager.xedoc_api_key_env_enabled(),
            );
            ...
            let ((), (plugin_skill_errors, plugin_context_cache)) = tokio::join!(
            ...
            let session_extension_data =
                xedoc_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(xedoc_extension_api::ThreadStartInput {
            ...
            let mcp_connection_manager = McpConnectionManager::new(
                ...
                xedoc_mcp::ElicitationRequestRouter::default(),
            )
            .instrument(info_span!(
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => xedoc_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    xedoc_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => xedoc_hooks::SessionStartSource::Clear,
            };
