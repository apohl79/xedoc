...
use core_test_support::test_xedoc::test_env as remote_test_env;
...
use xedoc_exec_server::ExecProcess;
use xedoc_exec_server::ExecProcessEventReceiver;
use xedoc_exec_server::ExecProcessFuture;
use xedoc_exec_server::ProcessId;
use xedoc_exec_server::ProcessSignal;
use xedoc_exec_server::ReadResponse;
use xedoc_exec_server::StartedExecProcess;
use xedoc_exec_server::WriteResponse;
use xedoc_exec_server::WriteStatus;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, xedoc_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, xedoc_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), xedoc_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export XEDOC_INTERACTIVE_SHELL_VAR=xedoc\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $XEDOC_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export XEDOC_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $XEDOC_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = xedoc_exec_server::Environment::default_for_tests();
...
async fn unified_exec_uses_remote_exec_server_when_configured() -> anyhow::Result<()> {
    ...
    let crate::unified_exec::OutputHandles {
        output_buffer,
        output_notify,
        output_closed,
        output_closed_notify,
