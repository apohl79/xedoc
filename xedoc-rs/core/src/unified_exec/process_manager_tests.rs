...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("XEDOC_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("XEDOC_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            XEDOC_PERMISSION_PROFILE_ENV_VAR.to_string(),
            "current-profile".to_string(),
        ),
...
fn exec_env_policy_excludes_runtime_permission_profile() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "xedoc_permission_profile".to_string(),
    ...
    assert_eq!(
        ...
        xedoc_exec_server::ExecEnvPolicy {
            inherit: policy.inherit,
            ignore_default_excludes: policy.ignore_default_excludes,
            exclude: vec![XEDOC_PERMISSION_PROFILE_ENV_VAR.to_string()],
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: xedoc_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        xedoc_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = xedoc_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = xedoc_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            ("XEDOC_NETWORK_PROXY_ACTIVE".to_string(), "1".to_string()),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: xedoc_exec_server::ExecEnvPolicy {
                inherit: xedoc_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: xedoc_sandboxing::SandboxType::None,
        sandbox_policy_cwd: cwd.into(),
    ...
    request.exec_server_sandbox = Some(
        xedoc_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    let first =
        exec_server_params_for_request(/*process_id*/ 123, &request, /*tty*/ true);
    let second =
...
async fn network_denial_fallback_message_names_sandbox_network_proxy() {
    ...
    assert_eq!(
        message,
        "Network access was denied by the Xedoc sandbox network proxy."
    );
}
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: xedoc_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let xedoc_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let xedoc_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        xedoc_protocol::items::CommandExecutionStatus::Failed
    );
