...
use crate::test_xedoc::TestXedoc;
use crate::test_xedoc::TestXedocBuilder;
use crate::test_xedoc::test_xedoc;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = xedoc_utils_cargo_bin::cargo_bin("xedoc-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `xedoc-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_xedoc().with_config(move |config| {
