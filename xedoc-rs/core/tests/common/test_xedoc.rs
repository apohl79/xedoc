...
use xedoc_core::TimeProvider;
use xedoc_core::XedocThread;
use xedoc_core::config::Config;
use xedoc_core::resolve_installation_id;
use xedoc_core::shell::Shell;
use xedoc_core::shell::get_shell_by_model_provided_path;
...
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "XEDOC_TEST_REMOTE_EXEC_SERVER_URL";
...
pub fn local(cwd: AbsolutePathBuf) -> TurnEnvironmentSelection {
    TurnEnvironmentSelection {
        environment_id: xedoc_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
...
#[derive(Debug)]
pub struct TestEnv {
    environment: xedoc_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: xedoc_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            xedoc_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &xedoc_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                xedoc_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestXedocBuilder {
    ...
    pub fn with_auth(mut self, auth: XedocAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestXedoc> {
    ...
    ///
    /// With no remote test configuration, or with `XEDOC_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `XEDOC_TEST_ENVIRONMENT=docker` or
    /// `XEDOC_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `XEDOC_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `XEDOC_TEST_REMOTE_ENV` Docker-container
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let xedoc_linux_sandbox_exe = Some(
            crate::find_xedoc_linux_sandbox_exe()
                .context("should find binary for xedoc-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let xedoc_linux_sandbox_exe = None;
        let local_runtime_paths = xedoc_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            xedoc_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            xedoc_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            xedoc_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<xedoc_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestXedoc> {
        let auth = self.auth.clone();
        let state_db = xedoc_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.xedoc_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(XedocHomeUserInstructionsProvider::new(
        ...
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            xedoc_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    xedoc_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    xedoc_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = xedoc_utils_cargo_bin::cargo_bin("xedoc") {
            config.xedoc_self_exe = Some(path);
        } else if let Ok(path) = xedoc_utils_cargo_bin::cargo_bin("xedoc-exec") {
            // `xedoc-exec` also supports `--xedoc-run-as-apply-patch`, so use it
        ...
        {
            let xedoc = bin_dir.join("xedoc");
            let xedoc_exec = bin_dir.join("xedoc-exec");
            if xedoc.is_file() {
                config.xedoc_self_exe = Some(xedoc);
            } else if xedoc_exec.is_file() {
                config.xedoc_self_exe = Some(xedoc_exec);
            }
        }
...
pub struct TestXedoc {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub xedoc: Arc<XedocThread>,
...
impl TestXedoc {
    ...
    pub fn xedoc_home_path(&self) -> &Path {
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.xedoc, |event| match event {
...
pub struct TestXedocHarness {
    server: MockServer,
    test: TestXedoc,
}

impl TestXedocHarness {
    pub async fn with_builder(mut builder: TestXedocBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestXedocBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_xedoc() -> TestXedocBuilder {
    TestXedocBuilder {
        ...
        auth: XedocAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
