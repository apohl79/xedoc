...
use core_test_support::test_xedoc::TestXedocBuilder;
use core_test_support::test_xedoc::TestXedocHarness;
use core_test_support::test_xedoc::configure_search_capable_model;
use core_test_support::test_xedoc::test_xedoc as base_test_xedoc;
...
use xedoc_core::compact::SUMMARY_PREFIX;
use xedoc_features::Feature;
use xedoc_login::XedocAuth;
use xedoc_login::auth::AgentIdentityAuth;
use xedoc_login::auth::AgentIdentityAuthRecord;
...
use xedoc_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use xedoc_protocol::dynamic_tools::DynamicToolFunctionSpec;
use xedoc_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use xedoc_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
fn test_xedoc() -> TestXedocBuilder {
    base_test_xedoc().with_config(|config| {
...
async fn wait_for_turn_complete(xedoc: &xedoc_core::XedocThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestXedocHarness::with_builder(
        test_xedoc().with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let xedoc = harness.test().xedoc.clone();
    ...
    xedoc
        ...
        .await?;
    wait_for_turn_complete(&xedoc).await;

    xedoc.submit(Op::Compact).await?;
    wait_for_turn_complete(&xedoc).await;
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestXedocHarness::with_builder(
        test_xedoc().with_auth(XedocAuth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let harness = TestXedocHarness::with_builder(builder).await?;
    let xedoc = harness.test().xedoc.clone();
    let image_url =
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_xedoc().with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "xedoc_app".to_string(),
        description: "Xedoc app tools.".to_string(),
        tools: vec![
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "xedoc_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "xedoc_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "xedoc_app", tool_name, "{}"),
    ...
    test.xedoc
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.xedoc, |event| {
    ...
    test.xedoc
        ...
        .await?;
    wait_for_turn_complete(&test.xedoc).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&xedoc, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&xedoc, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestXedocHarness::with_builder(
        test_xedoc()
            .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_xedoc = baseline_harness.test().xedoc.clone();
    ...
    baseline_xedoc
        ...
        .await?;
    wait_for_event(&baseline_xedoc, |event| {
    ...
    baseline_xedoc.submit(Op::Compact).await?;
    wait_for_event(&baseline_xedoc, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestXedocHarness::with_builder(
        test_xedoc()
            .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_xedoc = override_harness.test().xedoc.clone();
    ...
    override_xedoc
        ...
        .await?;
    wait_for_event(&override_xedoc, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_xedoc.submit(Op::Compact).await?;
    wait_for_event(&override_xedoc, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = xedoc.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    xedoc.submit(Op::Shutdown).await?;
    wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    let mut start_builder = test_xedoc()
        .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
    ...
    initial.xedoc.submit(Op::Compact).await?;
    wait_for_event(&initial.xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    wait_for_event(&initial.xedoc, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    .await;

    let mut resume_builder = test_xedoc()
        .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config.base_instructions = None;
            config.include_permissions_instructions = true;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
