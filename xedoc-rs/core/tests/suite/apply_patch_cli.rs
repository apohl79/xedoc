...
use core_test_support::test_xedoc::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestXedocHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestXedocBuilder) -> TestXedocBuilder,
) -> Result<TestXedocHarness> {
    let builder = configure(test_xedoc());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestXedocHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestXedocHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_xedoc_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let xedoc_linux_sandbox_exe = harness
        .test()
        .config
        .xedoc_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include xedoc-linux-sandbox helper");
    assert_eq!(
        xedoc_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(XEDOC_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&xedoc_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(
...
async fn apply_patch_turn_diff_tracks_local_and_remote_environment_paths() -> Result<()> {
    ...
    wait_for_event(&test.xedoc, |event| match event {
        EventMsg::TurnDiff(ev) => {
            last_diff = Some(ev.unified_diff.clone());
            false
        }
