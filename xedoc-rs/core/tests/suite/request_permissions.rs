...
use xedoc_protocol::models::AdditionalPermissionProfile as PermissionProfile;
use xedoc_protocol::models::FileSystemPermissions;
use xedoc_protocol::models::PermissionProfile as CorePermissionProfile;
...
async fn partial_request_permissions_grants_do_not_preapprove_new_permissions() -> Result<()> {
    ...
    let xedoc_protocol::models::LegacyReadWriteRoots {
        read: approval_reads,
...
async fn request_permissions_session_grants_carry_across_turns() -> Result<()> {
    ...
    let completion_event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
        )
