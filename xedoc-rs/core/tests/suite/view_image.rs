...
fn disabled_user_turn(test: &TestXedoc, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestXedoc, rel_path: &str) -> anyhow::Result<PathBuf> {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, xedoc_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        xedoc_protocol::items::TurnItem::ImageView(item) => {
...
async fn view_image_tool_resizes_when_model_lacks_original_detail_support() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_xedoc().with_model("gpt-5.2");
    let test = builder.build_with_auto_env(&server).await?;
    let TestXedoc {
        xedoc,
        session_configured,
