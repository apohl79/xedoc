...
async fn ultra_reasoning_uses_max_and_proactive_mode() -> Result<()> {
    ...
    submit_turn(&test.xedoc, "hello", /*effort*/ None).await?;
...
async fn configured_mode_hint_uses_custom_mode_across_reasoning_efforts() -> Result<()> {
    ...
    submit_turn(&test.xedoc, "explicit", Some(ReasoningEffort::High)).await?;
    submit_turn(&test.xedoc, "proactive", Some(ReasoningEffort::Ultra)).await?;
...
async fn empty_configured_mode_hint_emits_no_mode_message() -> Result<()> {
    ...
    submit_turn(&test.xedoc, "hello", Some(ReasoningEffort::High)).await?;
...
async fn changing_configured_mode_hint_to_empty_emits_no_update() -> Result<()> {
    ...
    submit_turn(&initial.xedoc, "before resume", /*effort*/ None).await?;
    drop(initial);
    ...
    submit_turn(&resumed.xedoc, "after resume", /*effort*/ None).await?;
...
async fn leaving_ultra_after_cold_resume_emits_explicit_mode() -> Result<()> {
    ...
    let mut resume_builder = test_xedoc()
        ...
        .with_config(configure_ultra);
    let resumed = resume_builder.resume(&server, home, rollout_path).await?;
    submit_turn(&resumed.xedoc, "after resume", Some(ReasoningEffort::High)).await?;
