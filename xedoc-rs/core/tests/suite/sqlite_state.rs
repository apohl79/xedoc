...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db_path = xedoc_state::state_db_path(test.config.sqlite_home.as_path());
    ...
    let db = test.xedoc.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn resume_restores_dynamic_tools_from_rollout_with_sqlite_enabled() -> Result<()> {
    ...
    let mut resume_builder = test_xedoc().with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_xedoc()
        .with_pre_build_hook(move |xedoc_home| {
            let rollout_path = xedoc_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = xedoc_state::state_db_path(test.config.sqlite_home.as_path());
    let rollout_path = test.config.xedoc_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = xedoc_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = xedoc_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
