...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let xedoc_home = TempDir::new().expect("create temp dir");
    ...
    let model_info = xedoc_core::test_support::construct_model_info_offline("gpt-5.4", &config);
    assert_eq!(
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_xedoc()
        .with_model("exp-xedoc-personality")
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "xedoc-remote-default-personality";
    ...
    let mut builder = test_xedoc()
        .with_auth(xedoc_login::XedocAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "xedoc-remote-personality";
