...
async fn request_user_input_round_trip_for_mode(
    ...
    let request = wait_for_event_match(&xedoc, |event| match event {
    ...
    wait_for_event(&xedoc, |event| matches!(event, EventMsg::TokenCount(_))).await;
...
async fn request_user_input_interrupt_emits_deferred_token_count() -> anyhow::Result<()> {
    ...
    let token_count = wait_for_event_match(&xedoc, |event| match event {
