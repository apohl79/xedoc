...
use xedoc_core::config::CurrentTimeReminderConfig;
use xedoc_extension_items::ExtensionItem;
use xedoc_extension_items::sleep::SleepItem;
...
async fn build_xedoc(server: &StreamingSseServer) -> Arc<XedocThread> {
    test_xedoc()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Xedoc test session")
        .xedoc
}

async fn submit_user_input(xedoc: &XedocThread, text: &str) {
    xedoc
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestXedoc, text: &str) {
...
async fn steer_user_input(xedoc: &XedocThread, text: &str) {
    xedoc
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(xedoc: &XedocThread, text: &str) {
    xedoc
...
async fn wait_for_reasoning_item_started(xedoc: &XedocThread) {
    wait_for_event(xedoc, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(xedoc: &XedocThread, text: &str) {
...
async fn wait_for_turn_complete(xedoc: &XedocThread) {
    wait_for_event(xedoc, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(xedoc: &XedocThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(xedoc, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(xedoc: &XedocThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(xedoc, |event| {
        matches!(
            event,
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    let xedoc = test_xedoc()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build Xedoc test session")
        .xedoc;

    submit_user_input(&xedoc, INITIAL_PROMPT).await;
    wait_for_event(&xedoc, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&xedoc, STEER_PROMPT).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&xedoc, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&xedoc, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&xedoc, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&xedoc, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&xedoc, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&xedoc, "new mailbox input").await;
    wait_for_sleep_item_completed(&xedoc, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&xedoc).await;
    ...
    xedoc.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&xedoc, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = xedoc.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let xedoc = build_xedoc(&server).await;

    submit_user_input(&xedoc, "first prompt").await;

    wait_for_reasoning_item_started(&xedoc).await;

    submit_queue_only_agent_mail(&xedoc, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&xedoc, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&xedoc, "second prompt").await;
    wait_for_turn_complete(&xedoc).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&xedoc, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&xedoc, "resumed old task").await;
    wait_for_turn_complete(&xedoc).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&xedoc, "processed steered prompt").await;
    wait_for_turn_complete(&xedoc).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&xedoc, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&xedoc, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
