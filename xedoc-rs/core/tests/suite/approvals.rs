...
use xedoc_core::config::ThreadStoreConfig;
use xedoc_core::sandboxing::SandboxPermissions;
use xedoc_features::Feature;
use xedoc_protocol::approvals::NetworkApprovalProtocol;
...
use xedoc_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestXedoc) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Xedoc helper binary
...
impl Expectation {
    fn verify(&self, test: &TestXedoc, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestXedoc) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestXedoc) {
    let event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestXedoc) {
...
async fn wait_for_spawned_thread(test: &TestXedoc) -> Result<Arc<XedocThread>> {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_xedoc().with_model(model).with_config(move |config| {
    ...
    test.xedoc.shutdown_and_wait().await?;
    verification_result
}
...
/// same sandbox with `xedoc sandbox -P`. The nested invocation must also pass
...
printenv XEDOC_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    let approval_policy = AskForApproval::OnRequest;
    let mut builder = test_xedoc().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
    });
    let test = builder.build(&server).await?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit XEDOC_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `xedoc sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Xedoc sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("XEDOC_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("XEDOC_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("XEDOC_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected xedoc sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Xedoc sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/xedoc-fallback-rule-test.txt && echo hello > /tmp/xedoc-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://xedoc-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();
