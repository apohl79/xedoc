...
const REMOTE_MODEL: &str = "xedoc-test-ttl";
const VERSIONED_MODEL: &str = "xedoc-test-versioned";
const MISSING_VERSION_MODEL: &str = "xedoc-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "xedoc-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let xedoc = Arc::clone(&test.xedoc);
    ...
    let cache_path = cache_path(config.xedoc_home.as_path());
    ...
    xedoc
        .submit(Op::UserInput {
            ...
            thread_settings: xedoc_protocol::protocol::ThreadSettingsOverrides {
                ...
                approval_policy: Some(xedoc_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&xedoc, |event| matches!(event, EventMsg::TurnComplete(_))).await;
