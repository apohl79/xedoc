...
//! `just test -p xedoc-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let xedoc_home = home.path().join(".xedoc");
    ...
    let mut cmd = Command::new(xedoc_utils_cargo_bin::cargo_bin("xedoc-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn xedoc-rs");
