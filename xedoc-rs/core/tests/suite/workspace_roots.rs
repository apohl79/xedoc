...
async fn workspace_roots_test(server: &MockServer) -> Result<TestXedoc> {
...
fn outside_workspace_path(test: &TestXedoc, file_name: &str) -> Result<PathUri> {
    let file_name = format!("xedoc-workspace-roots-{}-{file_name}", std::process::id());
...
async fn submit_workspace_turn(test: &TestXedoc, prompt: &str) -> Result<()> {
    test.submit_turn_with_permission_profile(prompt, workspace_roots_profile())
        .await
}

async fn read_file(test: &TestXedoc, path: &PathUri) -> Result<String> {
    Ok(String::from_utf8(
        test.fs().read_file(path, /*sandbox*/ None).await?,
    )?)
}

async fn remove_files(test: &TestXedoc, paths: &[&PathUri]) -> Result<()> {
    for path in paths {
        test.fs()
            .remove(
                path,
