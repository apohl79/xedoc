...
use xedoc_web_search_extension::install as install_web_search_extension;

const RESPONSES_LITE_HEADER: &str = "x-openai-internal-codex-responses-lite";

fn responses_extensions(auth: &XedocAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut xedoc_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_xedoc().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;
...
async fn responses_lite_exposes_standalone_tools_for_actor_authorized_provider() -> Result<()> {
    ...
    let auth = XedocAuth::from_api_key("dummy");
    let extensions = responses_extensions(&auth);
    let mut builder = test_xedoc()
        .with_auth(auth)
        .with_extensions(extensions)
