...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.xedoc);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.xedoc, "after resume").await?;
    shutdown_thread(&resumed.xedoc).await?;
...
async fn submit_user_turn(xedoc: &Arc<XedocThread>, text: &str) -> Result<()> {
    xedoc
        .submit(Op::UserInput {
...
async fn submit_compact_turn(xedoc: &Arc<XedocThread>) -> Result<()> {
    xedoc.submit(Op::Compact).await?;
    let warning_event = wait_for_event(xedoc, |event| {
        matches!(
            event,
...
async fn shutdown_thread(xedoc: &Arc<XedocThread>) -> Result<()> {
