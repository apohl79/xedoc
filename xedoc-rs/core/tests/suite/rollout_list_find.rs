...
use uuid::Uuid;
use xedoc_core::RolloutRecorder;
use xedoc_core::RolloutRecorderParams;
use xedoc_core::config::ConfigBuilder;
use xedoc_core::find_archived_thread_path_by_id_str;
use xedoc_core::find_thread_meta_by_name_str;
use xedoc_core::find_thread_path_by_id_str;
...
fn write_minimal_rollout_with_id_in_subdir(xedoc_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = xedoc_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(xedoc_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(xedoc_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(xedoc_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
    ...
    builder.cwd = xedoc_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_xedoc_home_directory() {
    let repo = TempDir::new().unwrap();
    let xedoc_home = repo.path().join(".xedoc");
    std::fs::create_dir_all(&xedoc_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".xedoc/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&xedoc_home, id);

    let found =
        find_thread_path_by_id_str(&xedoc_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
