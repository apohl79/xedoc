...
use xedoc_core::compact::SUMMARIZATION_PROMPT;
...
use xedoc_protocol::protocol::HookRunStatus;
...
async fn assert_compaction_uses_turn_lifecycle_id(xedoc: &std::sync::Arc<xedoc_core::XedocThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = xedoc.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Xedoc.
    ...
    xedoc.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
    ...
    // Shut down Xedoc to flush rollout entries before inspecting the file.
    xedoc.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    xedoc.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&xedoc, |ev| match ev {
    ...
    let hook_inputs = read_hook_inputs(&test.xedoc_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.xedoc_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_reports_single_pass_lifecycle_progress() {
    ...
    loop {
        let event = xedoc.next_event().await.expect("next compaction event");
        match event.msg {
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let xedoc = builder.build(&server).await.unwrap().xedoc;
    ...
    let first = wait_for_event_match(&xedoc, |ev| match ev {
    ...
    let last = wait_for_event_match(&xedoc, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        xedoc_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let mut resume_builder = test_xedoc().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.xedoc, |event| {
...
async fn pre_sampling_compact_runs_when_comp_hash_changes() {
    ...
    let test = builder.build(&server).await.expect("build test xedoc");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.xedoc).await;
...
async fn pre_sampling_compact_uses_target_model_after_previous_model_rename() {
    ...
    let renamed_model = "renamed-model";
    ...
    let mut initial_builder = test_xedoc()
        .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test xedoc");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_xedoc()
        .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.xedoc).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&xedoc, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&xedoc, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let xedoc = builder.build(&server).await.unwrap();

    xedoc.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    xedoc.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
    let requests = request_log.requests();
    assert_eq!(requests.len(), 3);
    assert!(
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    xedoc.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
-> Result<()> {
    ...
    test.xedoc.flush_rollout().await?;
    ...
    let rollout_path = test.xedoc.rollout_path().expect("rollout path");
    ...
    test.xedoc.submit(Op::Shutdown).await?;
    wait_for_event(&test.xedoc, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    ...
    assert_eq!(
        resumed.xedoc.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
