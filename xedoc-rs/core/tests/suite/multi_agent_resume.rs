...
const COLLABORATION_NAMESPACE: &str = "multi_agent_v2";
...
fn configure_multi_agent_v2_with_role(
    ...
    config
        ...
        .expect("test config should allow feature update");
    let role_path = config.xedoc_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_xedoc().with_config(move |config| {
    ...
    initial.xedoc.flush_rollout().await?;
    drop(worker_thread);
    drop(initial);
