...
use xedoc_core::config::AgentRoleConfig;
...
fn has_agent_message_input_containing(req: &wiremock::Request, text: &str) -> bool {
...
fn write_home_skill(xedoc_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = xedoc_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestXedoc) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_xedoc::TestXedocBuilder,
    ) -> core_test_support::test_xedoc::TestXedocBuilder,
) -> Result<(
    TestXedoc,
    String,
    ...
    let mut builder = configure_test(test_xedoc().with_config(|config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.xedoc_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.xedoc_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.xedoc_home_path(), "stop_hook_log.jsonl")?;
...
async fn spawned_full_history_v2_child_uses_model_precedence_without_dropping_context(
    ...
    test.xedoc
        ...
        .await?;
    let turn_id = wait_for_event_match(&test.xedoc, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    ...
    let activity_metadata = wait_for_event_match(&test.xedoc, |event| match event {
...
async fn openai_multi_agent_v2_spawn_sends_plaintext_agent_message_to_child() -> Result<()> {
    ...
    let mut builder = test_xedoc().with_model("koffing").with_config(|config| {
        config
            .features
            .enable(Feature::Collab)
            .expect("test config should allow feature update");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn non_openai_multi_agent_v2_spawn_sends_plaintext_agent_message_to_child() -> Result<()> {
    let server = start_mock_server().await;
    let message = "plain child task";
    let spawn_args = serde_json::to_string(&json!({
        "message": message,
        "task_name": "worker",
    }))?;
    ...
    let mut builder = test_xedoc().with_model("koffing").with_config(|config| {
        ...
        config.model_provider.name = "Claude".to_string();
    });
    let test = builder.build(&server).await?;
...
async fn multi_agent_v2_followup_restarts_parent_activity_tracking() -> Result<()> {
    ...
    const ACTIVITY_SUMMARY: &str = "Reviewing the continued work";
    const SLEEP_CALL_ID: &str = "sleep-call-2";
    ...
    mount_sse_once_match(
        ...
        sse(vec![
            ev_response_created("resp-child-1"),
            ev_assistant_message("msg-child-1", "initial work complete"),
            ev_completed("resp-child-1"),
        ]),
    )
    .await;
    mount_sse_once_match(
        ...
        sse(vec![
            ev_response_created("resp-parent-2"),
            ev_assistant_message("msg-parent-2", "initial child done"),
            ev_completed("resp-parent-2"),
        ]),
    )
    .await;
    ...
    mount_sse_once_match(
        ...
        sse(vec![
            ev_response_created("resp-parent-4"),
            ev_assistant_message("msg-parent-4", "follow-up sent"),
            ev_completed("resp-parent-4"),
        ]),
    )
    .await;
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.xedoc_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.xedoc_home.join("instructions-only-role.toml");
    ...
    let test = test_xedoc()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.xedoc_home.join("model-only-role.toml");
