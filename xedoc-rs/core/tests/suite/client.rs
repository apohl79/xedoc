...
use core_test_support::TestXedocResponsesRequestKind;
...
use xedoc_core::ModelClient;
use xedoc_core::NewThread;
use xedoc_core::Prompt;
use xedoc_core::ResponseEvent;
use xedoc_core::ThreadManager;
use xedoc_core::resolve_installation_id;
use xedoc_core::thread_store_from_config;
...
use xedoc_protocol::models::LocalShellAction;
use xedoc_protocol::models::LocalShellExecAction;
use xedoc_protocol::models::LocalShellStatus;
...
use xedoc_protocol::models::WebSearchAction;
...
use xedoc_protocol::protocol::RolloutLine;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> xedoc_core::XedocResponsesMetadata {
...
fn assert_xedoc_client_metadata(
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_xedoc().build(&server).await.unwrap();
...
async fn non_openai_responses_requests_omit_item_passthrough_metadata() {
    ...
    xedoc
        ...
        .await
        .unwrap();
    wait_for_event(&xedoc, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.xedoc.submit(Op::Shutdown).await?;
    wait_for_event(&initial.xedoc, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let xedoc_home = Arc::new(TempDir::new()?);
    let mut builder = test_xedoc().with_config(|config| {
        let _ = config.features.enable(Feature::ItemIds);
    });
    let first = builder
        .resume(&server, Arc::clone(&xedoc_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.xedoc.submit(Op::Shutdown).await?;
    wait_for_event(&first.xedoc, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, xedoc_home, session_path).await?;
...
/// Writes an `auth.json` into the provided `xedoc_home` with the specified parameters.
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: xedoc_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = xedoc_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = xedoc_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![xedoc_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = xedoc_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Xedoc to resume from our file
    let xedoc_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_xedoc()
        .with_home(xedoc_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, xedoc_home, session_path.clone())
        .await
        .expect("resume conversation");
    let xedoc = test.xedoc.clone();
    ...
    xedoc
        ...
        .await
        .unwrap();
    wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let xedoc_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_xedoc().with_model("gpt-5.4");
    let test = builder
        .resume(&server, xedoc_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_xedoc().with_auth(XedocAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.xedoc_home_path().join(INSTALLATION_ID_FILENAME))
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    let xedoc_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&xedoc_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = xedoc_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        xedoc_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(XedocAuth::from_api_key(
            "unused-api-key",
        ))),
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_xedoc()
        .with_auth(XedocAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut builder = test_xedoc()
        .with_auth(create_dummy_xedoc_auth())
        .with_config(move |config| {
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = XedocAuth::from_auth_storage(
        xedoc_home.path(),
    ...
    .await
    .expect("Failed to load XedocAuth")
    .expect("No XedocAuth found in xedoc_home");
    let auth_manager = xedoc_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.xedoc_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        xedoc_core::build_models_manager(&config, auth_manager),
        SessionSource::Exec,
        Arc::new(xedoc_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(xedoc_core::test_support::EmptyUserInstructionsProvider),
        thread_store_from_config(&config, /*state_db*/ None),
        /*agent_graph_store*/ None,
        installation_id,
        /*external_time_provider*/ None,
    );
    let NewThread { thread: xedoc, .. } = thread_manager
...
async fn skills_append_to_developer_message() {
    ...
    let xedoc_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = xedoc_home.path().join("skills/demo");
    ...
    let xedoc_home_path = xedoc_home.path().to_path_buf();
    let mut builder = test_xedoc()
        .with_home(xedoc_home.clone())
        .with_auth(XedocAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = xedoc_home_path.abs();
        });
    let xedoc = builder
        .build(&server)
        .await
    ...
    let _xedoc_home_guard = xedoc_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let xedoc_home_parent = TempDir::new().unwrap();
    let long_home_parent = xedoc_home_parent
        .path()
        .join("xedoc-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let xedoc_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = xedoc_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_xedoc()
        ...
        .with_config(move |config| {
            config.cwd = xedoc_home_path.abs();
            let user_config_path = xedoc_home_path.join("config.toml").abs();
    ...
    let _xedoc_home_guard = xedoc_home;
    let _xedoc_home_parent_guard = xedoc_home_parent;
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestXedoc { xedoc, .. } = test_xedoc()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestXedoc { xedoc, .. } = test_xedoc().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestXedoc { xedoc, config, .. } = test_xedoc().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        xedoc_core::test_support::auth_manager_from_auth(XedocAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let mut builder = test_xedoc()
        .with_auth(XedocAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let xedoc = builder
    ...
    wait_for_event(&xedoc, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let mut builder = test_xedoc();
    let xedoc_fixture = builder.build(&server).await?;
    ...
    let token_event = wait_for_event(&xedoc, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&xedoc, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&xedoc, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = XedocErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
async fn repeated_empty_max_output_tokens_responses_emit_error() -> anyhow::Result<()> {
    ...
    let error_event = wait_for_event(&xedoc, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error_event else {
        unreachable!("wait_for_event returned a non-error event")
    };
    assert_eq!(error.message, XedocErr::RepeatedOutputLimit.to_string());
    assert_eq!(error.xedoc_error_info, Some(XedocErrorInfo::Other));
...
fn create_dummy_xedoc_auth() -> XedocAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Xedoc sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
