...
use xedoc_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use xedoc_protocol::protocol::EventMsg;
use xedoc_protocol::protocol::ExitedReviewModeEvent;
use xedoc_protocol::protocol::Op;
use xedoc_protocol::protocol::ReviewCodeLocation;
use xedoc_protocol::protocol::ReviewFinding;
use xedoc_protocol::protocol::ReviewLineRange;
use xedoc_protocol::protocol::ReviewOutputEvent;
...
use xedoc_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Xedoc sandbox network restrictions.
    skip_if_no_network!();
    ...
    let xedoc = new_conversation_for_server(&server, xedoc_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = xedoc.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match xedoc.next_event().await.expect("review event").msg {
    ...
    xedoc.submit(Op::Interrupt).await.unwrap();
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    let review = match closed {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let xedoc = new_conversation_for_server(&server, xedoc_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&xedoc, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&xedoc, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = xedoc_home.path().join("resume.jsonl");
    {
        ...
        let user = xedoc_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = xedoc_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let xedoc =
        resume_conversation_for_server(&server, xedoc_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let xedoc = new_conversation_for_server(&server, xedoc_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, xedoc_home, resume_path)
        .await
