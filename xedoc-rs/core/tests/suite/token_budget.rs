...
use xedoc_core::config::TokenBudgetConfig;
...
use xedoc_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
use xedoc_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
use xedoc_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_OPEN_TAG;
use xedoc_protocol::protocol::CONTEXT_WINDOW_OPEN_TAG;
...
async fn assert_context_compaction_item_lifecycle(xedoc: &std::sync::Arc<xedoc_core::XedocThread>) {
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_xedoc()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.xedoc, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn get_context_remaining_returns_token_budget_remaining_fragment() -> Result<()> {
    ...
    let thread_id = test.session_configured.thread_id;
    let remaining_context = "You have 2130 tokens left in this context window.".to_string();
...
async fn token_budget_context_uses_new_window_after_compaction() -> Result<()> {
    ...
    assert_context_compaction_item_lifecycle(&test.xedoc).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.xedoc, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.xedoc, |event| match event {
        EventMsg::HookCompleted(completed)
