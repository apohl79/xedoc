...
//! request payload that Xedoc would send to the model and assert that the
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<XedocThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.xedoc)
}

async fn user_turn(conversation: &Arc<XedocThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<XedocThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<XedocThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<XedocThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = xedoc_core::test_support::auth_manager_from_auth(
        xedoc_login::XedocAuth::from_api_key("dummy"),
    );
