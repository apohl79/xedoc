...
use xedoc_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use xedoc_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use xedoc_core::ModelClient;
use xedoc_core::ModelClientSession;
use xedoc_core::Prompt;
use xedoc_core::ResponseEvent;
use xedoc_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use xedoc_core::XedocResponsesMetadata;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> XedocResponsesMetadata {
    responses_metadata(harness, turn_id, TestXedocResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestXedocResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> XedocResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestXedocResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(xedoc_login::default_client::get_xedoc_user_agent())
    );
    assert_eq!(
    ...
    let stream_request_start_ms = body["client_metadata"]
        [X_XEDOC_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let token_event =
        wait_for_event(&test.xedoc, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.xedoc, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = xedoc_core::test_support::construct_model_info_offline(MODEL, &config);
