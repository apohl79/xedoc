...
use xedoc_exec_server::EnvironmentReadyInfo;
use xedoc_exec_server::ExecServerError;
use xedoc_exec_server::FileSystemSandboxContext;
use xedoc_exec_server::LOCAL_ENVIRONMENT_ID;
use xedoc_exec_server::NoiseChannelPublicKey;
use xedoc_exec_server::NoiseRendezvousConnectBundle;
use xedoc_exec_server::NoiseRendezvousConnectProvider;
use xedoc_exec_server::REMOTE_ENVIRONMENT_ID;
...
use xedoc_protocol::models::SandboxPermissions;
...
use xedoc_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
use xedoc_protocol::request_user_input::RequestUserInputAnswer;
...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestXedoc> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = xedoc_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn wait_for_completion_without_patch_approval(test: &TestXedoc) {
    let event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn remote_test_env_exposes_target_shell_to_model() -> Result<()> {
    ...
    .await;
    let test = test_xedoc().build_with_auto_env(&server).await?;
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/xedoc-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\xedoc-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    test.xedoc
        ...
        .await?;
    let request = wait_for_event_match(&test.xedoc, |event| match event {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    assert_eq!(test.xedoc.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.xedoc.ensure_rollout_materialized().await;
    test.xedoc.flush_rollout().await?;
    let rollout_path = test.xedoc.rollout_path().context("rollout path")?;
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/xedoc-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/xedoc-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/xedoc-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/xedoc-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/xedoc-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/xedoc-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/xedoc-remote-dotdot-{}", std::process::id()));
