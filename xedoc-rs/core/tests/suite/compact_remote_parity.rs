...
const FIXED_CWD: &str = "/tmp/xedoc_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> XedocAuth {
        match self {
            AuthCase::ChatGpt => XedocAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => XedocAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&xedoc, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().xedoc_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestXedocHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestXedocHarness> {
...
async fn build_harness_inner(
    ...
    TestXedocHarness::with_builder(builder.with_config(move |config| {
        config.cwd = xedoc_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
fn rollout_path(harness: &TestXedocHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn submit_user_input(xedoc: &xedoc_core::XedocThread, items: Vec<UserInput>) -> Result<()> {
    xedoc
        .submit(Op::UserInput {
            items,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<XEDOC_HOME>");
            search_start = start_index + "<XEDOC_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <XEDOC_HOME>/skills/.system/imagegen/SKILL.md and \
         <XEDOC_HOME>/skills/custom/SKILL.md"
    );
}
