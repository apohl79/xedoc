...
use xedoc_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "xedoc-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://xedoc-network-test.invalid:80";
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert_eq!(
        approval.call_id,
        "network#local#http#xedoc-network-test.invalid#80"
    );
    ...
    assert_eq!(
        ...
        Some("xedoc-network-test.invalid is not in the allowed_domains")
    );
    test.xedoc
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="xedoc-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): xedoc-network-test.invalid",
        )
    }));
...
async fn unattributed_network_request_uses_active_turn_environment_fallback() -> Result<()> {
    ...
    test.xedoc.submit(Op::Interrupt).await?;
    wait_for_event(&test.xedoc, |event| {
        matches!(event, EventMsg::TurnAborted(_))
    })
    .await;
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    .await;
    test.xedoc.submit(Op::CleanBackgroundTerminals).await?;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.xedoc.list_background_terminals().await.is_empty() {
                break;
            }
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestXedoc> {
...
async fn wait_for_completion_without_network_prompt(test: &TestXedoc) {
...
async fn wait_for_turn_complete(test: &TestXedoc) {
    wait_for_event(&test.xedoc, |event| {
