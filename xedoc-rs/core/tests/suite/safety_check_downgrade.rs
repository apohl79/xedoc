...
fn disabled_text_turn(test: &TestXedoc, text: &str) -> Op {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_xedoc().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.xedoc, |event| {
    ...
    let warning = wait_for_event(&test.xedoc, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.xedoc, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.xedoc, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.xedoc_error_info, Some(XedocErrorInfo::CyberPolicy));
...
async fn response_model_field_mismatch_emits_warning_when_header_matches_requested() -> Result<()> {
    ...
    let warning = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
...
async fn openai_model_header_mismatch_only_emits_one_warning_per_turn() -> Result<()> {
    ...
    loop {
        let event = wait_for_event(&test.xedoc, |_| true).await;
        match event {
            EventMsg::Warning(warning)
                if warning
                    .message
...
async fn model_verification_emits_structured_event_without_reroute_or_warning() -> Result<()> {
    ...
    let mut builder = test_xedoc().with_model(SERVER_MODEL);
    let test = builder.build(&server).await?;
