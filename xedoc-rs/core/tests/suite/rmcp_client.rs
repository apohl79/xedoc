...
use xedoc_config::types::McpServerAuth;
...
use xedoc_exec_server::Environment;
use xedoc_exec_server::HttpRedirectPolicy;
use xedoc_exec_server::HttpRequestParams;
use xedoc_login::XedocAuth;
use xedoc_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use xedoc_protocol::openai_models::ConfigShellToolType;
...
use xedoc_protocol::openai_models::TruncationPolicyConfig;
...
use xedoc_utils_cargo_bin::cargo_bin;
...
fn read_only_user_turn(fixture: &TestXedoc, text: impl Into<String>) -> Op {
...
fn auto_approved_user_turn(fixture: &TestXedoc, text: impl Into<String>) -> Op {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        xedoc_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/xedoc-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.xedoc, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.xedoc, |ev| {
    ...
    wait_for_event(&fixture.xedoc, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn openai_form_capability_updates_for_loaded_thread() -> anyhow::Result<()> {
    ...
    let fixture = test_xedoc()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&fixture.xedoc, server_name).await?;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.xedoc, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn shutdown_cancels_startup_prewarm_waiting_for_mcp_startup() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.xedoc.shutdown_and_wait())
        .await
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            xedoc_linux_sandbox_exe: fixture.config.xedoc_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.xedoc, |ev| {
            matches!(
                ev,
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: xedoc_protocol::openai_models::ReasoningEffort::Medium,
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Xedoc should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Xedoc can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Xedoc will call the MCP echo tool
    ...
    // Phase 3: configure Xedoc with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Xedoc begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.xedoc, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_xedoc()
        .with_auth(XedocAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.xedoc, "configured_auth").await?;
...
/// This test writes to a fallback credentials file in XEDOC_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire XEDOC_HOME through the code.
#[test]
#[serial(xedoc_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Xedoc will call the OAuth-backed
    ...
    // Phase 3: seed an isolated XEDOC_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _xedoc_home_guard = EnvVarGuard::set("XEDOC_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Xedoc with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Xedoc begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.xedoc, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Xedoc fixture.
