...
async fn explicit_plugin_mentions_expose_plugin_mcp_tools() -> Result<()> {
    ...
    write_plugin_skill_plugin(xedoc_home.as_ref());
    write_plugin_mcp_plugin(xedoc_home.as_ref(), &rmcp_test_server_bin);

    let mut builder = test_xedoc()
        .with_home(xedoc_home)
        .with_auth(XedocAuth::from_api_key("Test API Key"));
    let test_xedoc = builder
        .build(&server)
        .await
        .expect("create new conversation");
    let xedoc = Arc::clone(&test_xedoc.xedoc);
    wait_for_mcp_server(&xedoc, "sample").await?;

    xedoc
        .submit(Op::UserInput {
            items: vec![xedoc_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
