...
fn read_only_user_turn(test: &TestXedoc, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-xedoc-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_xedoc().build(&server).await?;
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.xedoc, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
