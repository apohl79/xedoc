...
use xedoc_app_server_protocol::InitializeCapabilities;
use xedoc_app_server_protocol::InitializeParams;
use xedoc_app_server_protocol::InitializeResponse;
use xedoc_app_server_protocol::JSONRPCMessage;
...
use xedoc_app_server_protocol::SandboxPolicy;
...
use xedoc_otel::OtelProvider;
use xedoc_otel::current_span_w3c_trace_context;
use xedoc_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "xedoc-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $XEDOC_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Xedoc app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Xedoc", version, about = "Bootstrap Xedoc app-server", long_about = None)]
struct Cli {
    /// Path to the `xedoc` CLI binary. When set, requests use stdio by
    /// spawning `xedoc app-server` as a child process.
    #[arg(long, env = "XEDOC_BIN", global = true)]
    xedoc_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--xedoc-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "XEDOC_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `xedoc` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `xedoc app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/xedoc-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `xedoc app-server --listen`.
    ...
    /// Send a user message through the Xedoc app-server.
    SendMessage {
        /// User message to send to Xedoc.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Xedoc-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// List the available models from the Xedoc app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Xedoc app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "XEDOC_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let xedoc_bin = xedoc_bin.unwrap_or_else(|| PathBuf::from("xedoc"));
            serve(&xedoc_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(xedoc_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(xedoc_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(xedoc_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(xedoc_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if xedoc_bin.is_some() && url.is_some() {
        bail!("--xedoc-bin and --url are mutually exclusive");
    }
    if let Some(xedoc_bin) = xedoc_bin {
        return Ok(Endpoint::SpawnXedoc(xedoc_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if xedoc_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --xedoc-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(xedoc_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(xedoc_bin);
        if let Some(xedoc_bin_parent) = xedoc_bin.parent() {
            let mut path = OsString::from(xedoc_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", xedoc_bin.display()))?;
...
fn serve(xedoc_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/xedoc-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,xedoc_=trace {}",
        shell_quote(&xedoc_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started xedoc app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnXedoc(xedoc_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: xedoc_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut XedocClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = XedocClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = XedocClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (xedoc_bin, url) {
        (Some(_), Some(_)) => bail!("--xedoc-bin and --url are mutually exclusive"),
        (Some(xedoc_bin), None) => {
            let server = BackgroundAppServer::spawn(&xedoc_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve xedoc-app-server-test-client binary path")?;
...
impl XedocClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnXedoc(xedoc_bin) => Self::spawn_stdio(xedoc_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(xedoc_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(xedoc_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let xedoc_bin_display = xedoc_bin.display();
        let mut cmd = Command::new(xedoc_bin);
        if let Some(xedoc_bin_parent) = xedoc_bin.parent() {
            let mut path = OsString::from(xedoc_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut xedoc_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{xedoc_bin_display}` app-server"))?;

        let stdin = xedoc_app_server
            .stdin
            .take()
            .context("xedoc app-server stdin unavailable")?;
        let stdout = xedoc_app_server
            .stdout
            .take()
            .context("xedoc app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: xedoc_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `xedoc-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "xedoc-toy-app-server".to_string(),
                    title: Some("Xedoc Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: xedoc_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: xedoc_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to xedoc app-server")?;
                    return Ok(());
                }
                bail!("xedoc app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from xedoc app-server")?;
                if bytes == 0 {
                    bail!("xedoc app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = xedoc_core::otel_init::build_provider(
            &config,
...
impl Drop for XedocClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[xedoc app-server exited: {status}]");
            return;
        }
