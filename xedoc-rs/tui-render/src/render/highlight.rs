...
//! | `XEDOC_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static XEDOC_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and xedoc home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `xedoc_home` in
...
pub fn set_theme_override(name: Option<String>, xedoc_home: Option<PathBuf>) -> Option<String> {
    let warning = validate_theme_name(name.as_deref(), xedoc_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let xedoc_home_set_ok = XEDOC_HOME.set(xedoc_home.clone()).is_ok();
    ...
    if !override_set_ok || !xedoc_home_set_ok {
...
pub fn validate_theme_name(name: Option<&str>, xedoc_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = xedoc_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$XEDOC_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, xedoc_home: &Path) -> PathBuf {
    xedoc_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{xedoc_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, xedoc_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, xedoc_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, xedoc_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 3. Try loading {XEDOC_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = xedoc_home
...
fn build_default_theme() -> Theme {
    ...
    let xedoc_home = XEDOC_HOME
        .get()
        .and_then(|xedoc_home| xedoc_home.as_deref());
    resolve_theme_with_override(name, xedoc_home)
}
...
pub fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = XEDOC_HOME.get()
...
pub fn resolve_theme_by_name(name: &str, xedoc_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
...
/// `.tmTheme` file under `{XEDOC_HOME}/themes/`.
...
/// found in `{xedoc_home}/themes/`.
pub fn list_available_themes(xedoc_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*xedoc_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*xedoc_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*xedoc_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*xedoc_home*/ None).is_none());
    }
