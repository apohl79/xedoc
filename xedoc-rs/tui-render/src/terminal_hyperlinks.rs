...
impl TerminalHyperlink {
    pub fn web(columns: Range<usize>, destination: String) -> Self {
        Self {
            columns,
            destination,
    ...
    pub fn retarget_to_trusted_file(&mut self, destination: &Url) {
...
pub fn visible_lines_ref(lines: &[HyperlinkLine]) -> Vec<Line<'_>> {
    lines
        .iter()
...
mod tests {
    ...
    fn trusted_file_destination_receives_osc8_without_enabling_plain_file_links() {
        ...
        let mut link = TerminalHyperlink::web(
            /*columns*/ 0..4,
            "https://xedoc.invalid/viewer".to_string(),
        );
