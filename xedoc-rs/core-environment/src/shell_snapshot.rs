...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("xedoc.shell_snapshot.duration_ms", &[]);
            ...
            config
                .session_telemetry
                .counter("xedoc.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_create(
        ...
        let temp_path = xedoc_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let xedoc_home = xedoc_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&xedoc_home, cleanup_session_id, state_db).await
            {
...
async fn run_script_with_timeout_and_env(
    ...
    unsafe {
        handler.pre_exec(|| {
            xedoc_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(xedoc_home, session_id, state_db.as_deref()).await?;
