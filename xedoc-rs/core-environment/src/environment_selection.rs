...
use xedoc_protocol::protocol::EnvironmentConnectionEvent;
...
impl TurnEnvironmentSnapshot {
    ...
    #[cfg(test)]
    pub fn primary_environment(&self) -> Option<Arc<xedoc_exec_server::Environment>> {
        self.primary()
...
"#,
        ...
        let cwd_uri = PathUri::from_abs_path(&cwd);
        let manager =
            EnvironmentManager::from_xedoc_home(temp_dir.path(), Some(test_runtime_paths()))
                .await
