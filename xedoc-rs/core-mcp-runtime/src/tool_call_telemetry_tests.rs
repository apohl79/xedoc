...
async fn mcp_result_telemetry_span_logs(meta: Option<serde_json::Value>) -> String {
    ...
    let span = tracing::info_span!(
        "mcp.tools.call",
        xedoc.mcp.target.id = Empty,
        xedoc.mcp.server_user_flow.triggered = Empty,
    );
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("xedoc.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("xedoc.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("xedoc.mcp.target.id=")
            && !invalid_logs.contains("xedoc.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("xedoc.mcp.target.id=")
            && !missing_logs.contains("xedoc.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("xedoc.mcp.target.id=")
            && !no_meta_logs.contains("xedoc.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("xedoc.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("xedoc.mcp.error.code=\"RATE_LIMITED\""),
