...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = xedoc_home.clone();

    let opts = ServerOptions {
        xedoc_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: xedoc_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = xedoc_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_xedoc_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let xedoc_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Xedoc"),
        "error body should clearly explain the Xedoc access denial"
    );
    assert!(
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Xedoc to retry"),
    ...
    assert!(
        !body.contains("get access to Xedoc"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not xedoc"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        xedoc_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_xedoc_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        xedoc_home: first_xedoc_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: xedoc_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_xedoc_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        xedoc_home: second_xedoc_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: xedoc_login::CLIENT_ID.to_string(),
        issuer,
