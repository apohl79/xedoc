...
use xedoc_utils_template::Template;

pub(super) const DEFAULT_ISSUER: &str = "https://auth.openai.com";
const DEFAULT_PORT: u16 = 1455;
// Keep in sync with the Xedoc CLI Hydra redirect URI allow-list.
                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Xedoc failed.",
                            io::ErrorKind::Other,
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("xedoc_cli_simplified_flow".to_string(), "true".to_string()),
...
pub(crate) async fn persist_tokens_async(
    ...
    let xedoc_home = xedoc_home.to_path_buf();
...
/// Returns true when the OAuth callback represents a missing Xedoc entitlement.
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_codex_entitlement_error(error_code, error_description) {
        return "Xedoc is not enabled for your workspace. Contact your workspace administrator to request access to Xedoc.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_codex_entitlement_error(code, error_description) {
            (
                "You do not have access to Xedoc".to_string(),
                "This account is not currently authorized to use Xedoc in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Xedoc.".to_string(),
                "Contact your workspace administrator to get access to Xedoc, then return to Xedoc and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Xedoc to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        ...
        assert!(body.contains("You do not have access to Xedoc"));
