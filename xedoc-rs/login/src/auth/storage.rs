...
use xedoc_agent_identity::AgentIdentityJwtClaims;
use xedoc_agent_identity::decode_agent_identity_jwt;
use xedoc_config::types::AuthCredentialsStoreMode;
pub use xedoc_config::types::AuthKeyringBackendKind;
use xedoc_keyring_store::DefaultKeyringStore;
use xedoc_keyring_store::KeyringStore;
use xedoc_protocol::account::PlanType as AccountPlanType;
use xedoc_protocol::auth::AuthMode;
use xedoc_secrets::LocalSecretsNamespace;
...
/// Expected structure for $XEDOC_HOME/auth.json.
...
pub(super) fn get_auth_file(xedoc_home: &Path) -> PathBuf {
    xedoc_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(xedoc_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(xedoc_home);
...
impl FileAuthStorage {
    pub(super) fn new(xedoc_home: PathBuf) -> Self {
        Self { xedoc_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `XEDOC_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.xedoc_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.xedoc_home)
    }
}

static XEDOC_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("XEDOC_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("XEDOC_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Xedoc Auth";

// turns xedoc_home path into a stable, short key string
fn compute_store_key(xedoc_home: &Path) -> std::io::Result<String> {
    let canonical = xedoc_home
        .canonicalize()
        .unwrap_or_else(|_| xedoc_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(xedoc_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            xedoc_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.xedoc_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.xedoc_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.xedoc_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(xedoc_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(xedoc_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            xedoc_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::XedocAuth,
        );
        Self {
            xedoc_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &XEDOC_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &XEDOC_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &XEDOC_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(xedoc_home)),
        }
    }
}
...
// A global in-memory store for mapping xedoc_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(xedoc_home: PathBuf) -> Self {
        Self { xedoc_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(xedoc_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(xedoc_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(xedoc_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(xedoc_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(xedoc_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(xedoc_home, keyring_store))
        }
    }
}
