//! Default Xedoc HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`xedoc_login::default_client`] from other crates in this
//! workspace.
...
use std::sync::Mutex;
use std::sync::RwLock;
use xedoc_http_client::BuildCustomCaTransportError;
use xedoc_http_client::BuildRouteAwareHttpClientError;
...
use xedoc_http_client::build_reqwest_client_with_custom_ca;
use xedoc_http_client::with_chatgpt_cloudflare_cookie_store;
...
/// Parenthesis will be added by Xedoc. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "codex_cli_rs";
pub const XEDOC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "XEDOC_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-codex-residency";

pub use xedoc_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(XEDOC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(XEDOC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "xedoc-tui"
        || originator_value == "codex_vscode"
        || originator_value.starts_with("Xedoc ")
}
...
pub fn get_xedoc_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Xedoc user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Xedoc user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Xedoc originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Xedoc's route-aware system/PAC resolution.
pub fn create_client() -> HttpClient {
    let inner = build_reqwest_client();
    HttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Xedoc HTTP traffic.
///
/// This starts from the standard Xedoc user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `XEDOC_CA_CERTIFICATE` /
...
///
/// This supported default path preserves reqwest's existing proxy behavior and does not opt into
/// Xedoc's route-aware system/PAC resolution. Auth callers with route settings must use
...
/// Tries to build the default reqwest client used for ordinary Xedoc HTTP traffic.
///
...
/// Builds the default Xedoc reqwest client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Xedoc
...
/// Builds the default Xedoc reqwest client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Xedoc default headers.
...
/// Builds the default Xedoc reqwest client for an auth endpoint.
...
/// Builds the default Xedoc HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_xedoc_user_agent()) {
