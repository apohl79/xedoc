...
use xedoc_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let XedocAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(xedoc_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(xedoc_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("XEDOC_AUTHAPI_BASE_URL", &server.uri());
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(xedoc_home.path().to_path_buf());
    let auth_path = get_auth_file(xedoc_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = XedocAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = XedocAuth::from_auth_dot_json(
        xedoc_home.path(),
...
#[derive(Clone)]
struct StaticExternalAuth(XedocAuth);
...
impl ExternalAuth for StaticExternalAuth {
    ...
    fn refresh(&self, _context: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, XedocAuth> {
        Box::pin(async { Ok(self.0.clone()) })
    }
}
...
async fn external_auth_provider_can_install_headers() {
    ...
    let auth = XedocAuth::Headers(AuthHeaders::new(headers));
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_xedoc_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
fn write_auth_file(params: AuthFileParams, xedoc_home: &Path) -> std::io::Result<String> {
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(XEDOC_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(XEDOC_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    let XedocAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(xedoc_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("XEDOC_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(XEDOC_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(XEDOC_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("XEDOC_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(XEDOC_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            xedoc_home.path().to_path_buf(),
            /*enable_xedoc_api_key_env*/ false,
...
#[tokio::test]
#[serial(xedoc_auth_env)]
async fn load_auth_keeps_xedoc_api_key_env_precedence() {
    let xedoc_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(XEDOC_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(XEDOC_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(XEDOC_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let xedoc_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        xedoc_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = XedocAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        /*auth_route_config*/ None,
