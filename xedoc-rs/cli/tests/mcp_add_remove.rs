...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = xedoc_command(xedoc_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = xedoc_command(xedoc_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = xedoc_command(xedoc_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
"#,
    ...
    let mut list_cmd = xedoc_command(xedoc_home.path())?;
    list_cmd
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!xedoc_home.path().join(".credentials.json").exists());
    assert!(!xedoc_home.path().join(".env").exists());
