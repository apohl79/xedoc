...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    std::fs::write(xedoc_home.path().join("config.toml"), "not valid toml = [")?;
...
"#
    ...
    let xedoc_bin = xedoc_utils_cargo_bin::cargo_bin("xedoc")?;
    let xedoc_home = xedoc_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(xedoc_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    let mut child = std::process::Command::new(xedoc_utils_cargo_bin::cargo_bin("xedoc")?)
