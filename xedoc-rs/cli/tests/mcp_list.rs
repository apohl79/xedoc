...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = xedoc_command(xedoc_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(xedoc_home.path()).await?;
    ...
    let mut list_json_cmd = xedoc_command(xedoc_home.path())?;
    ...
    let mut get_cmd = xedoc_command(xedoc_home.path())?;
    ...
    assert!(stdout.contains("remove: xedoc mcp remove docs"));

    let mut get_json_cmd = xedoc_command(xedoc_home.path())?;
    get_json_cmd
