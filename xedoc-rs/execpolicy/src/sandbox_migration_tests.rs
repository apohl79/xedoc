...
async fn removes_banned_allow_rules_once() {
    ...
    let xedoc_home = tempdir().expect("create xedoc home");
    let policy_path = xedoc_home.path().join("rules/default.rules");
...
"#,
    ...
    prefix_rule_migration(xedoc_home.path(), &policy_path, BANNED_PREFIXES)
        .await
...
"#
    ...
    assert_eq!(
        std::fs::read_to_string(xedoc_home.path().join(MIGRATION_MARKER_FILENAME))
