...
- From the Xedoc CLI, run `xedoc execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
xedoc execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
xedoc execpolicy check \
...
```bash
cargo run -p xedoc-execpolicy -- check --rules path/to/policy.rules git status
```
