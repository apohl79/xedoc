//! The main Xedoc TUI chat surface.
//!
...
use tracing::debug;
use xedoc_app_server_protocol::CollabAgentTool;
use xedoc_app_server_protocol::CollabAgentToolCallStatus;
use xedoc_app_server_protocol::CommandExecutionRequestApprovalParams;
use xedoc_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use xedoc_app_server_protocol::CreditsSnapshot;
...
use xedoc_app_server_protocol::ModelVerification as AppServerModelVerification;
use xedoc_app_server_protocol::RateLimitReachedType;
use xedoc_app_server_protocol::RateLimitSnapshot;
...
use xedoc_app_server_protocol::ThreadGoal as AppThreadGoal;
use xedoc_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use xedoc_app_server_protocol::XedocErrorInfo as AppServerXedocErrorInfo;
use xedoc_config::ConfigLayerStackOrdering;
use xedoc_config::Constrained;
use xedoc_config::ConstraintResult;
use xedoc_config::types::Notifications;
use xedoc_features::FEATURES;
use xedoc_features::Feature;
#[cfg(test)]
use xedoc_git_utils::CommitLogEntry;
use xedoc_git_utils::current_branch_name;
use xedoc_git_utils::get_git_repo_root;
use xedoc_git_utils::local_git_branches;
use xedoc_git_utils::recent_commits;
use xedoc_otel::RuntimeMetricsSummary;
...
use xedoc_terminal_detection::Multiplexer;
use xedoc_terminal_detection::TerminalInfo;
use xedoc_terminal_detection::TerminalName;
use xedoc_terminal_detection::terminal_info;
use xedoc_tui_events::ResolvedAppServerRequest;
use xedoc_tui_overlays::pager_overlay::ActiveCellTranscriptKey;
use xedoc_utils_absolute_path::AbsolutePathBuf;
use xedoc_utils_cli::resume_hint;
...
use xedoc_tui_status::status::remote_connection::RemoteConnectionStatus;
...
pub use self::rate_limits::limit_label_for_window;
...
pub use self::user_messages::ThreadInputState;
pub use self::user_messages::ThreadInputStateRestoreMode;
pub use self::user_messages::UserMessage;
...
pub use self::user_messages::create_initial_user_message;
pub use self::user_messages::mention_bindings_from_user_inputs;
...
pub use crate::branch_summary::StatusLineGitSummary;
...
use xedoc_utils_approval_presets::builtin_approval_presets;
...
/// and by sending requests back to xedoc-core.
///
...
pub struct ChatWidget {
    ...
    pub remote_connection: Option<RemoteConnectionStatus>,
    ...
    xedoc_spend_control_reached: Option<bool>,
...
pub enum InterruptedTurnNoticeMode {
    #[default]
    Default,
    Suppress,
}
...
fn request_permissions_from_params(
    params: xedoc_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub fn set_collab_agent_metadata(
        &mut self,
        thread_id: ThreadId,
    ...
    pub fn handle_history_entry_response(&mut self, event: HistoryLookupResponse) {
    ...
    pub fn clear_thread_rename_block(&mut self) {
        self.thread_rename_block_message = None;
    }

    pub fn set_thread_rename_block_message(&mut self, message: impl Into<String>) {
        self.thread_rename_block_message = Some(message.into());
    }

    pub fn set_interrupted_turn_notice_mode(&mut self, mode: InterruptedTurnNoticeMode) {
    ...
    pub fn add_mcp_output(&mut self, detail: McpServerStatusDetail) {
        self.flush_answer_stream_with_separator();
        self.flush_active_cell();
    ...
    pub fn clear_mcp_inventory_loading(&mut self) {
    ...
    pub fn history_render_mode(&self) -> HistoryRenderMode {
    ...
    pub fn raw_output_mode_notice(enabled: bool) -> &'static str {
        if enabled {
    ...
    pub fn set_raw_output_mode_and_notify(&mut self, enabled: bool) {
    ...
    pub fn toggle_raw_output_mode_and_notify(&mut self) -> bool {
    ...
    pub fn on_terminal_resize(&mut self, width: u16) {
    ...
    pub fn has_active_agent_stream(&self) -> bool {
        self.stream_controller.is_some()
    }

    /// Whether a proposed-plan stream is active.
    pub fn has_active_plan_stream(&self) -> bool {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn is_task_running_for_test(&self) -> bool {
        self.bottom_pane.is_task_running()
    }

    pub fn toggle_vim_mode_and_notify(&mut self) {
    ...
    /// Forward a command directly to xedoc.
    pub fn submit_op<T>(&mut self, op: T) -> bool
    where
        T: Into<AppCommand>,
    {
        ...
        match &self.xedoc_op_target {
            XedocOpTarget::Direct(xedoc_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = xedoc_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            XedocOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::XedocOp(op));
            }
        }
        true
    }
    ...
    pub fn prepare_local_op_submission(&mut self, op: &AppCommand) {
    ...
    pub fn refresh_plugin_mentions(&mut self) {
        if !self.config.features.enabled(Feature::Plugins) {
    ...
    pub fn on_plugin_mentions_loaded(&mut self, plugins: Option<Vec<PluginCapabilitySummary>>) {
    ...
    pub fn active_cell_transcript_hyperlink_lines(&self, width: u16) -> Option<Vec<HyperlinkLine>> {
...
#[cfg(any(test, feature = "test-support"))]
pub mod dependency_test_support;
