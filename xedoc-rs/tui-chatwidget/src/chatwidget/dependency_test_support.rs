...
pub(crate) async fn test_config() -> Config {
    let xedoc_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_xedoc_home(xedoc_home.clone(), Vec::new())
            .await
            .expect("config");
    config.xedoc_home = xedoc_home.abs();
    config.sqlite_home = xedoc_home.clone();
    config.log_dir = xedoc_home.join("log");
...
pub(crate) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, XedocOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Xedoc to do anything".to_string();
...
pub fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_xedoc_backend_auth = true;
