...
impl ChatWidget {
    ...
    /// Clears the terminal title Xedoc most recently wrote, if any.
    ///
    ...
    /// OSC write.
    pub fn clear_managed_terminal_title(&mut self) -> std::io::Result<()> {
    ...
    pub fn refresh_status_surfaces(&mut self) {
    ...
    pub(super) fn status_line_command_payload(&mut self) -> String {
        ...
        let reasoning_level = reasoning
            .as_ref()
            .map(xedoc_protocol::openai_models::ReasoningEffort::as_str)
            .unwrap_or("default");
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_xedoc_folder } => {
                    dot_xedoc_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::XedocVersion => Some(xedoc_cli_version().to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("xedoc".to_string()),
            ...
            StatusSurfacePreviewItem::XedocVersion => StatusLineItem::XedocVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("xedoc".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::XedocVersion => self
                .status_line_value_for_item(StatusLineItem::XedocVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_xedoc_window(snapshot, "5h")
...
fn find_primary_xedoc_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
