...
impl ChatWidget {
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("xedoc-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "xedoc-auto-fast" => 0,
            "xedoc-auto-balanced" => 1,
            "xedoc-auto-thorough" => 2,
            _ => 3,
        }
    }

    pub fn open_all_models_popup(&mut self, mut presets: Vec<ModelPreset>) {
        if presets.is_empty() {
            self.add_info_message(
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running xedoc -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub fn open_advanced_reasoning_popup(&mut self, preset: ModelPreset) {
