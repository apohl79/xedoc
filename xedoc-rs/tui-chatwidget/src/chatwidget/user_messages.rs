...
use xedoc_app_server_protocol::TextElement as AppServerTextElement;
...
pub use xedoc_tui_input::UserMessage;
use xedoc_utils_plugins::mention_syntax::PLUGIN_TEXT_MENTION_SIGIL;
...
pub struct ThreadInputState {
    ...
    pub(super) pending_steer_ids: VecDeque<Option<u64>>,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ThreadInputStateRestoreMode {
    pub preserve_in_flight_turn: bool,
}
...
pub fn mention_bindings_from_user_inputs(
    items: &[UserInput],
    message: &str,
) -> Vec<MentionBinding> {
...
impl ChatWidget {
    ...
    pub fn user_message_display_from_inputs(items: &[UserInput]) -> UserMessageDisplay {
        if items
            .iter()
