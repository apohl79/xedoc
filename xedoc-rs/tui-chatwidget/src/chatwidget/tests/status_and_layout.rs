...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(XedocErrorInfo::CyberPolicy),
    );
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    let xedoc = chat
        .rate_limit_snapshots_by_limit_id
        .get("codex")
        .expect("xedoc snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("codex_other")
        .expect("codex_other snapshot should exist");

    assert_eq!(xedoc.primary.as_ref().map(|w| w.used_percent), Some(20.0));
    assert_eq!(
        xedoc
            .credits
            .as_ref()
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_xedoc_limit() {
...
async fn rate_limit_usage_warnings_keep_workspace_limit_after_rolling_credits() {
    for rate_limit_reached_type in [
        ...
        for credits in [
            ...
            assert_eq!(
                chat.xedoc_rate_limit_reached_type,
...
async fn rate_limit_usage_warnings_preserve_and_clear_spend_control_state() {
    ...
    blocked_snapshot.rate_limit_reached_type =
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached);
    chat.on_rate_limit_snapshot(Some(blocked_snapshot));
    assert_eq!(chat.xedoc_spend_control_reached, Some(true));
    ...
    assert_eq!(chat.xedoc_spend_control_reached, Some(false));
    assert_eq!(
        chat.xedoc_rate_limit_reached_type,
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached)
    );
...
async fn rate_limit_usage_warnings_clear_workspace_limit_from_authoritative_snapshot() {
    ...
    assert_eq!(chat.xedoc_rate_limit_reached_type, None);
}
...
async fn account_update_clears_derived_usage_limit_state_and_prompt() {
    ...
    assert!(chat.xedoc_rate_limit_reached_type.is_some());
    assert_eq!(chat.xedoc_spend_control_reached, Some(true));
    assert!(matches!(
        chat.rate_limit_switch_prompt,
        RateLimitSwitchPromptState::Shown
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            ...
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Xedoc.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using xedoc.",
        ),
    ];
...
async fn esc_interrupt_pauses_active_goal_turn() {
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::XedocOp(Op::Interrupt)));
    assert_goal_paused_event(&mut rx, thread_id);
...
async fn status_line_command_payload_uses_claude_compatible_shape() {
    ...
    assert_eq!(payload["harness"], "xedoc");
    ...
    assert_eq!(payload["version"], xedoc_cli_version());
}
...
#[tokio::test]
async fn pending_status_line_command_hides_builtin_status_line_items() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-test")).await;
    chat.config.tui_status_line = Some(vec!["model".to_string()]);
    chat.config.tui_status_line_command = Some(StatusLineCommand::Args(vec![
        "missing-statusline-command".to_string(),
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            xedoc_app_server_protocol::HookEventName::PreToolUse,
...
async fn completed_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            xedoc_app_server_protocol::HookEventName::PostToolUse,
            /*status_message*/ None,
        ),
    );
    reveal_running_hooks(&mut chat);
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        xedoc_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            xedoc_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            xedoc_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: xedoc_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> xedoc_app_server_protocol::ThreadGoal {
    xedoc_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![xedoc_app_server_protocol::HookOutputEntry {
                kind: xedoc_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![xedoc_app_server_protocol::HookOutputEntry {
                kind: xedoc_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            xedoc_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![xedoc_app_server_protocol::HookOutputEntry {
                kind: xedoc_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            xedoc_app_server_protocol::HookEventName::SessionStart,
...
fn hook_run_summary(
    ...
    xedoc_app_server_protocol::HookRunSummary {
        ...
        started_at: 1,
        completed_at: (status != xedoc_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != xedoc_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
