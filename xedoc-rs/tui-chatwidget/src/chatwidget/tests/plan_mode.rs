...
async fn submit_user_message_emits_structured_plugin_mentions_from_bindings() {
    ...
    chat.bottom_pane
        .set_plugin_mentions(Some(vec![xedoc_plugin::PluginCapabilitySummary {
...
#[tokio::test]
async fn plan_update_does_not_render_history_cell() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    assert!(cells.is_empty(), "expected no plan update history cell");
}
