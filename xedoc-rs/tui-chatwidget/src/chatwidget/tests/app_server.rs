...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> xedoc_app_server_protocol::ThreadSettingsUpdatedNotification {
    xedoc_app_server_protocol::ThreadSettingsUpdatedNotification {
        thread_id: thread_id.to_string(),
        thread_settings: xedoc_app_server_protocol::ThreadSettings {
...
async fn invalid_url_elicitation_is_declined() {
    ...
    chat.handle_elicitation_request_now(
        xedoc_app_server_protocol::RequestId::Integer(9),
        xedoc_app_server_protocol::McpServerElicitationRequestParams {
            thread_id: request_thread_id.to_string(),
            turn_id: Some("turn-auth".to_string()),
            server_name: "payments".to_string(),
            request: xedoc_app_server_protocol::McpServerElicitationRequest::Url {
                meta: None,
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: xedoc_app_server_protocol::RequestId::Integer(9),
...
async fn thread_settings_updated_updates_visible_state_without_transcript() {
    ...
    assert_eq!(
        chat.config_ref()
            ...
            .id,
        xedoc_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
    );
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                xedoc_error_info: Some(XedocErrorInfo::ServerOverloaded),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                xedoc_error_info: Some(XedocErrorInfo::CyberPolicy),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
async fn app_server_safety_access_errors_render_dedicated_notice() {
    ...
    for (case, message) in cases {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        chat.handle_non_retry_error(message, /*xedoc_error_info*/ None);
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            xedoc_app_server_protocol::ThreadNameUpdatedNotification {
                ...
                source: xedoc_app_server_protocol::ThreadNameUpdateSource::Generated,
            },
        ),
        /*replay_kind*/ None,
    );
