pub(super) use super::super::dependency_test_support::make_chatwidget_manual;
pub(super) use super::super::dependency_test_support::make_chatwidget_manual_with_auth;
pub use super::super::dependency_test_support::make_chatwidget_manual_with_sender;
pub use super::super::dependency_test_support::render_bottom_popup;
...
use xedoc_app_server_protocol::ImageGenerationItem;
...
fn token_usage_breakdown(usage: TokenUsage) -> xedoc_app_server_protocol::TokenUsageBreakdown {
    xedoc_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    xedoc_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: xedoc_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    xedoc_error_info: Option<XedocErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = xedoc_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            xedoc_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(xedoc_app_server_protocol::HookEventName, usize)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    xedoc_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: xedoc_app_server_protocol::HookEventName,
    status: xedoc_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<xedoc_app_server_protocol::HookOutputEntry>,
) -> xedoc_app_server_protocol::HookRunSummary {
    xedoc_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: xedoc_app_server_protocol::HookHandlerType::Command,
        execution_mode: xedoc_app_server_protocol::HookExecutionMode::Sync,
        scope: xedoc_app_server_protocol::HookScope::Turn,
        ...
        completed_at: matches!(
            status,
            xedoc_app_server_protocol::HookRunStatus::Completed
                | xedoc_app_server_protocol::HookRunStatus::Failed
                | xedoc_app_server_protocol::HookRunStatus::Blocked
                | xedoc_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            xedoc_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                xedoc_app_server_protocol::HookOutputEntry {
                    kind: xedoc_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                xedoc_app_server_protocol::HookOutputEntry {
                    kind: xedoc_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: xedoc_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        xedoc_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        xedoc_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        xedoc_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        xedoc_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        xedoc_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        xedoc_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        xedoc_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        xedoc_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        xedoc_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        xedoc_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        xedoc_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
