...
impl ChatWidget {
    pub fn show_goal_summary(&mut self, goal: AppThreadGoal) {
        self.add_plain_history_lines(goal_summary_lines(&goal));
    }

    pub fn show_goal_edit_prompt(&mut self, thread_id: ThreadId, goal: AppThreadGoal) {
    ...
    pub fn show_resume_paused_goal_prompt(&mut self, thread_id: ThreadId, objective: String) {
    ...
    pub fn on_thread_goal_cleared(&mut self, thread_id: &str) {
        if self
            .thread_id
