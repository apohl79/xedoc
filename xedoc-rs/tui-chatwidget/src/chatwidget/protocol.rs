...
impl ChatWidget {
    pub fn handle_server_notification(
        ...
        match notification {
            ...
            ServerNotification::Error(notification) => {
                if notification.will_retry {
                    ...
                    self.handle_non_retry_error(
                        notification.error.message,
                        notification.error.xedoc_error_info,
                    );
                }
            }
            ...
            ServerNotification::SessionExtensionMessage(notification) => match notification.level {
                xedoc_app_server_protocol::SessionExtensionMessageLevel::Info => self
                    .add_to_history(crate::history_cell::new_session_extension_info_event(
                        notification.extension_name,
                        notification.message,
                    )),
                xedoc_app_server_protocol::SessionExtensionMessageLevel::Warning => self
                    .add_to_history(crate::history_cell::new_warning_event(format!(
                        "{}: {}",
                        notification.extension_name, notification.message
                    ))),
            ...
            TurnStatus::Failed => {
                if let Some(error) = notification.turn.error {
                        ...
                        == Some(&(notification.turn.id.clone(), error.message.clone()))
                    {
                        ...
                        self.handle_non_retry_error(error.message, error.xedoc_error_info);
                    }
                } else {
                    self.last_non_retry_error = None;
                    self.finalize_turn();
