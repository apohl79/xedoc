...
impl ChatWidget {
    ...
    pub fn replay_thread_turns(&mut self, turns: Vec<Turn>, replay_kind: ReplayKind) {
        let hidden_nested_review_turns = std::iter::once(/*value*/ false)
            .chain(turns.windows(/*size*/ 2).map(|turns| {
                xedoc_tui_transcript::replay::is_hidden_nested_review_turn(&turns[0], &turns[1])
            }))
            .collect::<Vec<_>>();
            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: xedoc_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
            ...
            item @ ThreadItem::CommandExecution {
                status: xedoc_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
