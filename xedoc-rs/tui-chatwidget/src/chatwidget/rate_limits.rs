...
pub(super) const NUDGE_MODEL_SLUG: &str = "gpt-6-luna";
...
pub use xedoc_tui_status::rate_limit_labels::get_limits_duration;
pub use xedoc_tui_status::rate_limit_labels::limit_label_for_window;
...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerXedocErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerXedocErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerXedocErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerXedocErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerXedocErrorInfo) -> bool {
    matches!(info, AppServerXedocErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    pub fn on_rolling_rate_limit_snapshot(&mut self, snapshot: RateLimitSnapshot) {
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            {
                self.xedoc_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_xedoc_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.xedoc_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.xedoc_spend_control_reached == Some(true)
                || matches!(
                    self.xedoc_rate_limit_reached_type,
                    Some(
            ...
            if is_xedoc_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_xedoc_limit && !has_workspace_credits;
            ...
            self.xedoc_rate_limit_reached_type = None;
            self.xedoc_spend_control_reached = None;
        }
