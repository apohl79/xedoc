...
use xedoc_app_server_protocol::SkillsListEntry;
...
use xedoc_utils_plugins::mention_syntax::TOOL_MENTION_SIGIL;

impl ChatWidget {
    pub fn open_skills_list(&mut self) {
    ...
    pub fn open_skills_menu(&mut self) {
    ...
    pub fn open_manage_skills_popup(&mut self) {
    ...
    pub fn update_skill_enabled(&mut self, path: AbsolutePathBuf, enabled: bool) {
    ...
    pub fn handle_manage_skills_closed(&mut self) {
    ...
    pub fn set_skills_from_response(&mut self, response: &SkillsListResponse) {
        let skills = skills_for_cwd(&self.config.cwd, &response.data);
        self.skills_all = skills;
        self.set_skills(Some(enabled_skills_for_mentions(&self.skills_all)));
    }

    pub fn annotate_skill_reads_in_parsed_cmd(
        &self,
...
pub fn collect_tool_mentions(text: &str, mention_paths: &HashMap<String, String>) -> ToolMentions {
...
pub fn find_skill_mentions_with_tool_mentions(
