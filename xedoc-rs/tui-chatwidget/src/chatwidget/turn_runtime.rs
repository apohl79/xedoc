...
impl ChatWidget {
    ...
    pub(super) fn update_task_running_state(&mut self) {
        ...
        self.bottom_pane
            .set_active_turn_running(self.turn_lifecycle.agent_turn_running);
        self.bottom_pane.set_task_running(task_running);
        self.bottom_pane
            .set_active_turn_started_at(self.turn_lifecycle.active_turn_started_at);
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        xedoc_error_info: &AppServerXedocErrorInfo,
    ) -> bool {
        matches!(
            xedoc_error_info,
            AppServerXedocErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Xedoc is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.xedoc_rate_limit_reached_type.map(|kind| {
        ...
        self.xedoc_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        xedoc_error_info: Option<AppServerXedocErrorInfo>,
    ) {
        if xedoc_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if xedoc_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = xedoc_error_info
            .as_ref()
