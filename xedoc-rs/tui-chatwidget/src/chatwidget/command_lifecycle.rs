...
impl ChatWidget {
    ...
    pub fn handle_command_execution_started_now(&mut self, item: ThreadItem) {
        let ThreadItem::CommandExecution {
            id,
            command,
            source,
    ...
    pub fn handle_command_execution_completed_now(&mut self, item: ThreadItem) {
        enum ExecEndTarget {
        ...
        let event_parsed = command_actions
            .into_iter()
            .map(xedoc_app_server_protocol::CommandAction::into_core)
            .collect();
