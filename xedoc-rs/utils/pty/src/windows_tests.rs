...
const READY_MARKER: &str = "__XEDOC_CHILD_READY__";
const VALUE_MARKER: &str = "__XEDOC_CHILD_VALUE__";
...
async fn conpty_delivers_input_to_foreground_children() -> anyhow::Result<()> {
    ...
    let code = format!(
        "print('__XEDOC_CHILD_'+'READY__', flush=True); value=input(); print('{VALUE_MARKER}'+value.encode('utf-8').hex(), flush=True)"
    );
