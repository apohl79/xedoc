...
use xedoc_config::permissions_toml::NetworkUnixSocketPermissionToml;
use xedoc_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use xedoc_features::NetworkProxyDomainPermissionToml;
use xedoc_features::NetworkProxyModeToml;
use xedoc_features::NetworkProxyUnixSocketPermissionToml;
use xedoc_network_proxy::NetworkMode;
use xedoc_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use xedoc_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use xedoc_protocol::permissions::project_roots_glob_pattern;
...
pub fn builtin_permission_profile(
    profile_name: &str,
...
pub fn network_proxy_config_for_profile_selection(
    permissions: Option<&PermissionsToml>,
    profile_name: &str,
...
pub fn compile_permission_profile_selection(
    permissions: Option<&PermissionsToml>,
    profile_name: &str,
    workspace_write: Option<&SandboxWorkspaceWrite>,
    startup_warnings: &mut Vec<String>,
...
pub fn compile_permission_profile_workspace_roots(
    permissions: Option<&PermissionsToml>,
    profile_name: &str,
    policy_cwd: &Path,
...
/// for Xedoc to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub fn get_readable_roots_required_for_xedoc_runtime(
    xedoc_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(xedoc_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Xedoc versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Xedoc. Filesystem access will remain restricted. Upgrade Xedoc if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Xedoc and will be ignored. Upgrade Xedoc if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Xedoc and will be ignored. Upgrade Xedoc if this path is required."
            ),
        },
    );
}
