...
use xedoc_network_proxy::BlockedRequestObserver;
use xedoc_network_proxy::ConfigReloader;
use xedoc_network_proxy::ConfigReloaderFuture;
use xedoc_network_proxy::ConfigState;
use xedoc_network_proxy::NetworkDecision;
use xedoc_network_proxy::NetworkPolicyDecider;
...
use xedoc_network_proxy::host_and_port_from_network_addr;
use xedoc_network_proxy::normalize_host;
use xedoc_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    xedoc_network_proxy::NetworkDomainPermission::Allow
                } else {
                    xedoc_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
