//! Current-time provider contracts and resolution.
...
use chrono::DateTime;
use chrono::Utc;
use xedoc_core_config::config::CurrentTimeReminderConfig;
...
/// Provides the host system's UTC clock.
...
/// Resolves the configured time provider.
///
/// # Errors
///
/// Returns an error when external time is configured without a host provider.
pub fn resolve_time_provider(
