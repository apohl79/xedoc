...
This repository uses Bazel to build the Rust workspace under `xedoc-rs`.
...
- `rules_rs` imports third-party crates from `xedoc-rs/Cargo.toml` and
  `xedoc-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `xedoc_rust_crate`, which wraps `rust_library`,
...
- Each crate in `xedoc-rs/*/BUILD.bazel` typically uses `xedoc_rust_crate` and
...
Xedoc uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/xedoc \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //xedoc-rs/cli:xedoc
```
...
2. Create a `BUILD.bazel` that calls `xedoc_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `xedoc_rust_crate` to configure it.
