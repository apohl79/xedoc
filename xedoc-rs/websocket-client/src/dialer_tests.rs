...
use xedoc_http_client::OutboundProxyRoute;
...
async fn no_proxy_subprocess_probe() {
    let Ok(url) = std::env::var("XEDOC_WEBSOCKET_NO_PROXY_PROBE_URL") else {
        return;
    };
    let proxy_url = std::env::var("XEDOC_WEBSOCKET_NO_PROXY_PROBE_PROXY")
    ...
    let request = url
        ...
        .expect("websocket request should build");
    let tls_config =
        if let Ok(certificate_hex) = std::env::var("XEDOC_WEBSOCKET_NO_PROXY_PROBE_CA_DER") {
...
async fn assert_no_proxy_subprocess(no_proxy: &str, expect_proxy: bool, proxy_tls: bool) {
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        command
            ...
            .env("XEDOC_WEBSOCKET_NO_PROXY_PROBE_URL", target_url)
            .env("XEDOC_WEBSOCKET_NO_PROXY_PROBE_PROXY", proxy_url);
        command.env_remove("XEDOC_WEBSOCKET_NO_PROXY_PROBE_CA_DER");
        if let Some(certificate) = certificate {
            let certificate_hex = certificate
                ...
                .collect::<String>();
            command.env("XEDOC_WEBSOCKET_NO_PROXY_PROBE_CA_DER", certificate_hex);
        }
        command
            .output()
