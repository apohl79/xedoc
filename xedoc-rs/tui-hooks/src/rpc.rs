...
pub use xedoc_tui_events::HookTrustUpdate;

pub async fn fetch_hooks_list(
...
pub fn hooks_list_entry_for_cwd(response: HooksListResponse, cwd: &Path) -> HooksListEntry {
    response
        .data
        .into_iter()
...
pub fn hook_needs_review(hook: &HookMetadata) -> bool {
    matches!(
        hook.trust_status,
...
pub async fn write_hook_trusts(
    request_handle: AppServerRequestHandle,
