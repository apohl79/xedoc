...
use xedoc_tui_events::AppEvent;
use xedoc_tui_events::AppEventSender;
use xedoc_tui_events::HookTrustUpdate;
use xedoc_tui_input::key_hint;
use xedoc_tui_input::key_hint::KeyBindingListExt;
use xedoc_tui_input::keymap::ListKeymap;
use xedoc_tui_overlays::bottom_pane::CancellationEvent;
use xedoc_tui_overlays::bottom_pane::bottom_pane_view::BottomPaneView;
use xedoc_tui_overlays::bottom_pane::popup_consts::MAX_POPUP_ROWS;
use xedoc_tui_overlays::bottom_pane::scroll_state::ScrollState;
use xedoc_tui_overlays::bottom_pane::selection_popup_common::render_menu_surface;
use xedoc_tui_render::city_lights::CityLightsStylize;
use xedoc_tui_render::render::renderable::Renderable;
use xedoc_tui_render::style::accent_style;
use xedoc_tui_status::status::format_directory_display;
use xedoc_tui_transcript::line_truncation::truncate_line_with_ellipsis_if_overflow;

use crate::rpc::hook_needs_review;
...
impl HooksBrowserView {
    ...
    pub(crate) fn new(
        ...
        warnings: Vec<String>,
        errors: Vec<xedoc_app_server_protocol::HookErrorInfo>,
        app_event_tx: AppEventSender,
    ) -> Self {
        Self::from_entry(
            HooksListEntry {
            ...
            app_event_tx,
            xedoc_tui_input::keymap::RuntimeKeymap::defaults().list,
        )
    }
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        xedoc_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| xedoc_protocol::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => xedoc_protocol::protocol::HookEventName::iter().count(),
...
fn event_description(event_name: HookEventName) -> &'static str {
    match event_name {
        ...
        HookEventName::Stop => "Right before Xedoc ends its turn",
    }
}
...
mod tests {
    ...
    use xedoc_tui_overlays::bottom_pane::bottom_pane_view::BottomPaneView;
    use xedoc_tui_render::render::renderable::Renderable;
    use xedoc_tui_test_support::PathBufExt;
    use xedoc_tui_test_support::test_path_buf;
    use xedoc_tui_test_support::test_path_display;
        ...
        HooksBrowserView::new(
            vec![
                hook(
                    ...
                    "${XEDOC_PLUGIN_ROOT}/hooks/pre-tool-use-check.sh",
                    /*enabled*/ true,
                    /*is_managed*/ false,
                    /*display_order*/ 0,
                ),
