...
use xedoc_file_system::find_nearest_native_ancestor_with_markers;
...
/// directory. If you need Xedoc to work from such a checkout simply pass the
...
mod tests {
    ...
    async fn local_git_branches_excludes_detached_head_entry() {
        ...
        run_git(&[
            "-c",
            "user.name=Xedoc Tests",
            "-c",
            "user.email=xedoc-tests@example.com",
            "commit",
            "--allow-empty",
            "-q",
            "-m",
