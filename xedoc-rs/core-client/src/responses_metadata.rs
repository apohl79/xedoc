...
use xedoc_utils_string::to_ascii_json_string;

use crate::client::X_OPENAI_SUBAGENT_HEADER;
use crate::client::X_XEDOC_INSTALLATION_ID_HEADER;
use crate::client::X_XEDOC_PARENT_THREAD_ID_HEADER;
use crate::client::X_XEDOC_TURN_METADATA_HEADER;
use crate::client::X_XEDOC_WINDOW_ID_HEADER;
...
pub const WINDOW_ID_KEY: &str = "window_id";
pub const REQUEST_KIND_KEY: &str = "request_kind";
pub const COMPACTION_KEY: &str = "compaction";
pub const TURN_STARTED_AT_UNIX_MS_KEY: &str = "turn_started_at_unix_ms";

pub const FORKED_FROM_THREAD_ID_KEY: &str = "forked_from_thread_id";
pub const PARENT_THREAD_ID_KEY: &str = "parent_thread_id";
pub const SUBAGENT_KIND_KEY: &str = "subagent_kind";
pub const THREAD_SOURCE_KEY: &str = "thread_source";
pub const SANDBOX_KEY: &str = "sandbox";
pub const WORKSPACES_KEY: &str = "workspaces";
...
impl CompactionTurnMetadata {
    ...
    pub fn implementation(self) -> CompactionImplementation {
        self.implementation
    }
...
pub enum XedocResponsesRequestKind {
    Turn,
    Prewarm,
...
impl XedocResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            XedocResponsesRequestKind::Turn => ("turn", None),
    ...
    fn has_turn_identity(self) -> bool {
        !matches!(self, XedocResponsesRequestKind::SessionName)
    }
}

#[derive(Clone, Debug, Serialize, Default)]
pub struct TurnMetadataWorkspace {
...
/// Caller-owned snapshot of Xedoc metadata sent to ResponsesAPI.
///
/// The full Xedoc turn metadata blob is transported canonically as
...
/// truth.
#[derive(Clone, Debug)]
pub struct XedocResponsesMetadata {
    pub installation_id: String,
    pub session_id: String,
    pub thread_id: String,
    pub turn_id: Option<String>,
    pub window_id: String,
    pub request_kind: Option<XedocResponsesRequestKind>,
    ...
    pub workspaces: BTreeMap<String, TurnMetadataWorkspace>,
    pub turn_started_at_unix_ms: Option<i64>,
    pub extra: BTreeMap<String, String>,
}

impl XedocResponsesMetadata {
    pub fn new(
    ...
    pub fn has_turn_metadata(&self) -> bool {
        self.request_kind.is_some()
    }

    pub fn turn_metadata_json(&self) -> Option<String> {
        to_ascii_json_string(&self.turn_metadata_payload()).ok()
    }

    pub fn turn_metadata_value(&self) -> Option<Value> {
    ...
    pub fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_XEDOC_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_XEDOC_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_XEDOC_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_XEDOC_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }

    pub fn compatibility_headers(&self) -> ApiHeaderMap {
        let mut headers = ApiHeaderMap::new();
        insert_header(&mut headers, X_XEDOC_WINDOW_ID_HEADER, &self.window_id);
    ...
    fn turn_metadata_payload(&self) -> XedocTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(XedocResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(XedocResponsesRequestKind::has_turn_identity);
        XedocTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the Xedoc turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved Xedoc-owned keys are filtered when
...
pub fn subagent_header_value(session_source: &SessionSource) -> Option<String> {
...
pub fn subagent_metadata_kind(session_source: &SessionSource) -> Option<String> {
    match session_source {
...
pub fn filter_extra_metadata(extra: HashMap<String, String>) -> BTreeMap<String, String> {
    extra
        .into_iter()
...
#[derive(Serialize)]
struct XedocTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
