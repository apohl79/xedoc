...
//!
//! `ModelClient` is intended to live for the lifetime of a Xedoc session and holds the stable
...
use xedoc_api::CompactClient as ApiCompactClient;
use xedoc_api::CompactionInput as ApiCompactionInput;
use xedoc_api::Compression;
use xedoc_api::Provider as ApiProvider;
use xedoc_api::Reasoning;
use xedoc_api::ReasoningContext;
use xedoc_api::RequestTelemetry;
use xedoc_api::ReqwestTransport;
use xedoc_api::ResponseCreateWsRequest;
use xedoc_api::ResponsesApiRequest;
use xedoc_api::ResponsesClient as ApiResponsesClient;
use xedoc_api::ResponsesOptions as ApiResponsesOptions;
use xedoc_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use xedoc_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use xedoc_api::ResponsesWsRequest;
use xedoc_api::SharedAuthProvider;
use xedoc_api::SseTelemetry;
use xedoc_api::StreamOptions;
use xedoc_api::TransportError;
use xedoc_api::WebsocketTelemetry;
use xedoc_api::auth_header_telemetry;
use xedoc_api::build_session_headers;
use xedoc_api::create_text_param_for_request;
use xedoc_api::response_create_client_metadata;
use xedoc_http_client::ClientRouteClass;
use xedoc_http_client::HttpClientFactory;
use xedoc_login::AuthManager;
use xedoc_login::RefreshTokenError;
use xedoc_login::UnauthorizedRecovery;
use xedoc_login::XedocAuth;
use xedoc_login::default_client::add_originator_header;
use xedoc_login::default_client::build_default_reqwest_client_for_route;
...
use xedoc_model_provider::AgentIdentitySessionFallback;
...
pub const X_XEDOC_INSTALLATION_ID_HEADER: &str = "x-codex-installation-id";
pub const X_XEDOC_TURN_STATE_HEADER: &str = "x-codex-turn-state";
pub const X_XEDOC_TURN_METADATA_HEADER: &str = "x-codex-turn-metadata";
pub const X_XEDOC_PARENT_THREAD_ID_HEADER: &str = "x-codex-parent-thread-id";
pub const X_XEDOC_WINDOW_ID_HEADER: &str = "x-codex-window-id";
...
const X_XEDOC_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-codex-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_codex_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_XEDOC_RESPONSES_LITE_HEADER: &str =
...
#[doc(hidden)]
pub const WEBSOCKET_CONNECT_TIMEOUT: Duration =
...
///
/// This holds configuration and state that should be shared across turns within a Xedoc session
...
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Xedoc turn. Reusing it across turns would replay
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Xedoc session. Per-turn values
    ...
    pub fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "xedoc.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub async fn compact_conversation_history(
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_XEDOC_INSTALLATION_ID_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: xedoc_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: xedoc_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &XedocResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                xedoc_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn prepare_websocket_request(
        ...
        let incremental_items = self.get_incremental_items(
            request,
            Some(&last_response),
    ...
    fn responses_request_compression(&self, auth: Option<&XedocAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(XedocAuth::uses_xedoc_backend)
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_XEDOC_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
            match self
            ...
            let incremental_request = self.prepare_websocket_request(&request);
            ...
            let stream_result =
                stream_result.map_err(|err| self.client.state.provider.map_api_error(err))?;
    ...
    /// Permanently disables WebSockets for this Xedoc session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_XEDOC_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Xedoc-specific conventions:
///
...
fn build_responses_headers(
    ...
    {
        headers.insert(X_XEDOC_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_XEDOC_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
/// the mapped `XedocErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: xedoc_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a XedocResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(XedocErr::RefreshTokenFailed(failed))
            }
