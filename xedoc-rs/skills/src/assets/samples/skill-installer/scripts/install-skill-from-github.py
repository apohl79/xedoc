#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $XEDOC_HOME/skills."""
...
def _xedoc_home() -> str:
    return os.environ.get("XEDOC_HOME", os.path.expanduser("~/.xedoc"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "xedoc")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "xedoc-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_xedoc_home(), "skills")
