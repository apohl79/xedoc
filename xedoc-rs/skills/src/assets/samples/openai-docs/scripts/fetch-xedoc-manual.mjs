...
const CACHE_FILE_NAME = "xedoc-manual.md";
const OUTLINE_FILE_NAME = "xedoc-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "xedoc-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-xedoc-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Xedoc Manual Outline\n\n${outline.text}\n`;
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchXedocManual(args);
...
export { DEFAULT_MANUAL_URL, fetchXedocManual };
