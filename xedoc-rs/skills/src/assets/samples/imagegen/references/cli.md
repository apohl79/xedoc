...
Set a stable path to the skill CLI (default `XEDOC_HOME` is `~/.xedoc`):

```
export XEDOC_HOME="${XEDOC_HOME:-$HOME/.xedoc}"
export IMAGE_GEN="$XEDOC_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/xedoc-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$XEDOC_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
