...
use xedoc_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
...
use xedoc_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use xedoc_protocol::items::CommandExecutionItem;
...
use xedoc_protocol::items::FileChangeItem;
use xedoc_protocol::items::ImageViewItem;
use xedoc_protocol::items::McpToolCallItem;
use xedoc_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use xedoc_protocol::items::ReasoningItem;
use xedoc_protocol::items::SubAgentActivityItem;
use xedoc_protocol::items::TurnItem;
use xedoc_protocol::items::UserMessageItem;
use xedoc_protocol::items::WebSearchItem as CoreWebSearchItem;
use xedoc_protocol::mcp::CallToolResult;
use xedoc_protocol::mcp::McpServerInfo;
...
use xedoc_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use xedoc_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use xedoc_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use xedoc_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use xedoc_protocol::protocol::AgentStatus as CoreAgentStatus;
use xedoc_protocol::protocol::AskForApproval as CoreAskForApproval;
use xedoc_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use xedoc_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use xedoc_protocol::protocol::NetworkAccess as CoreNetworkAccess;
...
use xedoc_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: xedoc_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        xedoc_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        xedoc_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        xedoc_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.xedoc/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![xedoc_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            xedoc_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = xedoc_protocol::ThreadId::default();
    let receiver_thread_id = xedoc_protocol::ThreadId::default();
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            xedoc_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(xedoc_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
#[test]
fn xedoc_error_info_serializes_http_status_code_in_camel_case() {
    let value = XedocErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn xedoc_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(XedocErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn xedoc_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = XedocErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(xedoc_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(xedoc_protocol::config_types::CollaborationMode {
            mode: xedoc_protocol::config_types::ModeKind::Plan,
