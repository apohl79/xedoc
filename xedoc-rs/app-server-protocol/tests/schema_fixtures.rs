...
use xedoc_app_server_protocol::generate_json_with_experimental;
use xedoc_app_server_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use xedoc_app_server_protocol::read_schema_fixture_subtree;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let json_bundle =
        xedoc_utils_cargo_bin::find_resource!("schema/json/xedoc_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/xedoc_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
