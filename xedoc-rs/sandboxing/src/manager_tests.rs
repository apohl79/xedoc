...
fn transform_linux_seccomp_request(
    xedoc_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            xedoc_linux_sandbox_exe: Some(xedoc_linux_sandbox_exe),
            use_legacy_landlock: false,
        })
        .expect("transform")
}
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let xedoc_linux_sandbox_exe = std::path::PathBuf::from("/tmp/xedoc-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&xedoc_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(xedoc_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let xedoc_linux_sandbox_exe = std::path::PathBuf::from("/tmp/xedoc");
    let exec_request = transform_linux_seccomp_request(&xedoc_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("xedoc-linux-sandbox".to_string()));
}
