...
/// This structure is used to add some limits on the multi-agent capabilities for Xedoc. In
...
impl AgentRegistry {
    pub fn reserve_spawn_slot(
        ...
        if let Some(max_threads) = max_threads {
            if !self.try_increment_spawned(max_threads) {
                return Err(XedocErr::AgentLimitReached { max_threads });
            }
        } else {
    ...
    pub fn clear_last_task_message(&self, thread_id: ThreadId) {
        let mut active_agents = self
            .active_agents
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
            ...
            if let Some(name) = available_names.choose(&mut rand::rng()) {
                ...
                if let Some(metrics) = xedoc_otel::global() {
                    let _ = metrics.counter(
                        "xedoc.multi_agent.nickname_pool_reset",
                        /*inc*/ 1,
                        &[],
                    );
                }
    ...
    fn reserve_agent_path(&self, agent_path: &AgentPath) -> Result<()> {
        ...
        match active_agents.agent_tree.entry(agent_path.to_string()) {
            Entry::Occupied(_) => Err(XedocErr::UnsupportedOperation(format!(
...
impl SpawnReservation {
    pub fn reserve_agent_nickname_with_preference(
        ...
        let agent_nickname = self
            ...
            .ok_or_else(|| {
                XedocErr::UnsupportedOperation("no available agent nicknames".to_string())
            })?;
