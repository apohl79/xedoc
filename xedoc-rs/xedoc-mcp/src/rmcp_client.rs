...
//! initializing the server, listing raw tools, and applying per-server tool
//! filters.
...
use xedoc_config::McpServerTransportConfig;
...
use xedoc_exec_server::HttpClient;
use xedoc_exec_server::ReqwestHttpClient;
use xedoc_protocol::mcp::McpServerInfo;
use xedoc_protocol::protocol::Event;
use xedoc_rmcp_client::ExecutorStdioServerLauncher;
use xedoc_rmcp_client::LocalStdioServerLauncher;
use xedoc_rmcp_client::RmcpClient;
use xedoc_rmcp_client::StdioServerLauncher;
use xedoc_rmcp_client::ToolWithConnectorId;
use xedoc_rmcp_client::is_authentication_required_error;

/// MCP server capability indicating that Xedoc should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "xedoc/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "xedoc/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub const OPENAI_FORM_CAPABILITY: &str = "openai/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "xedoc.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "xedoc.mcp.tools.fetch_uncached.duration_ms";
...
pub(crate) async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        .tools
        .into_iter()
        .map(|tool| regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool))
        .collect();
    emit_duration(
...
async fn start_server_task(
    ...
    let has_shared_tool_catalog = tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("xedoc-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Xedoc"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(xedoc_utils_path_uri::LegacyAppPathString::into_string);
