...
use xedoc_config::McpServerEnvVar;
...
use xedoc_rmcp_client::InProcessTransportFactory;
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        xedoc_rmcp_client::Elicitation::Mcp(
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let elicitation = xedoc_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
    ...
    let (
        xedoc_protocol::mcp::RequestId::String(request_a_id),
        xedoc_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected Xedoc-owned string request IDs");
    };
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<xedoc_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = XEDOC_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `xedoc mcp login {server_name}`."
    );
