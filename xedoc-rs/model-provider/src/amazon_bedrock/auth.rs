...
use http::HeaderMap;
use xedoc_api::AuthError;
use xedoc_api::AuthProvider;
use xedoc_api::SharedAuthProvider;
use xedoc_aws_auth::AwsAuthContext;
use xedoc_aws_auth::AwsAuthError;
use xedoc_aws_auth::AwsRequestToSign;
use xedoc_http_client::Request;
use xedoc_http_client::RequestBody;
use xedoc_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_xedoc_error)?;
...
fn aws_auth_error_to_xedoc_error(error: AwsAuthError) -> XedocErr {
    XedocErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Xedoc agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockMantleSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> xedoc_api::AuthProviderFuture<'_> {
