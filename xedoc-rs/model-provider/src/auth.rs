...
use xedoc_agent_identity::AgentIdentityKey;
use xedoc_agent_identity::authorization_header_for_agent_task;
use xedoc_api::AgentIdentityTelemetry;
use xedoc_api::AuthProvider;
use xedoc_api::SharedAuthProvider;
use xedoc_login::AuthHeaders;
...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(XedocAuth::uses_xedoc_backend)
        else {
            return;
        };
...
) -> xedoc_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(XedocAuth::AgentIdentity(agent_identity_auth)) = auth {
...
pub fn auth_provider_from_auth(auth: &XedocAuth) -> SharedAuthProvider {
    match auth {
        XedocAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        XedocAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        XedocAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        XedocAuth::ApiKey(_)
        | XedocAuth::Chatgpt(_)
        | XedocAuth::ChatgptAuthTokens(_)
        | XedocAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
mod tests {
    ...
    use xedoc_agent_identity::generate_agent_key_material;
    ...
    use xedoc_login::auth::login_with_chatgpt_auth_tokens;
    ...
    fn test_xedoc_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "xedoc-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp xedoc home");
        path
    }

    fn write_chatgpt_auth_json(xedoc_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    ) -> (PathBuf, Arc<AuthManager>, XedocAuth) {
        let xedoc_home = test_xedoc_home();
        write_chatgpt_auth_json(&xedoc_home);
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = XedocAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn openai_provider_rejects_bedrock_api_key_auth() {
        ...
        let auth = XedocAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "bedrock-api-key-test".to_string(),
            region: "us-east-1".to_string(),
        });

        match resolve_provider_auth(Some(&auth), &provider) {
            Err(XedocErr::UnsupportedOperation(message)) => {
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = XedocAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&XedocAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_xedoc_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
