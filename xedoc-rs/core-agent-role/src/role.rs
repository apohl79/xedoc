...
use xedoc_config::loader::resolve_relative_paths_in_config_toml;
use xedoc_core_config::config::AgentRoleConfig;
use xedoc_core_config::config::Config;
use xedoc_core_config::config::ConfigOverrides;
use xedoc_core_config::config::agent_roles::parse_agent_role_file_contents;
use xedoc_core_config::config::deserialize_config_toml_with_base;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.xedoc_home.as_path())
    } else {
...
pub fn resolve_role_config<'a>(config: &'a Config, role_name: &str) -> Option<&'a AgentRoleConfig> {
    config
        .agent_roles
        .get(role_name)
...
mod reload {
    ...
    fn reload_overrides(
        ...
        ConfigOverrides {
            ...
            xedoc_linux_sandbox_exe: config.xedoc_linux_sandbox_exe.clone(),
