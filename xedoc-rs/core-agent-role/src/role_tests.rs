...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.xedoc_linux_sandbox_exe = Some(PathBuf::from("/tmp/xedoc-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/xedoc-execve-wrapper"));
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.xedoc_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/xedoc-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/xedoc-execve-wrapper"))
    );
}
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&xedoc_exec_server::LOCAL_FS)),
        )
        .await;
