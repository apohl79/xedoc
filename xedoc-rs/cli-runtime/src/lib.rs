...
use xedoc_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use xedoc_arg0::Arg0DispatchPaths;
use xedoc_arg0::arg0_dispatch_or_else;
use xedoc_exec::Cli as ExecCli;
use xedoc_exec::Command as ExecCommand;
use xedoc_exec::ReviewArgs;
use xedoc_execpolicy::ExecPolicyCheckCommand;
use xedoc_responses_api_proxy::Args as ResponsesApiProxyArgs;
use xedoc_tui::AppExitInfo;
use xedoc_tui::Cli as TuiCli;
use xedoc_tui::ExitReason;
use xedoc_tui::UpdateAction;
...
use xedoc_terminal_detection::TerminalName;
...
#[clap(
    author,
    name = "xedoc-cli",
    version = crate::version::xedoc_cli_version(),
    // If a sub‑command is given, ignore requirements of the default args.
    subcommand_negates_reqs = true,
    // The executable is sometimes invoked via a platform‑specific name like
    // `xedoc-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `xedoc` command name that users run.
    bin_name = "xedoc",
    override_usage = "xedoc [OPTIONS] [PROMPT]\n       xedoc [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Xedoc.
    Mcp(McpCli),

    /// Manage Xedoc plugins.
    Plugin(PluginCli),

    /// Start Xedoc as an MCP server (stdio).
    ...
    /// Update Xedoc to the latest version.
    Update,

    /// Run commands within a Xedoc-provided sandbox.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Xedoc.
...
struct UnsupportedSandboxArgs {
    /// Layer $XEDOC_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | xedoc login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv XEDOC_ACCESS_TOKEN | xedoc login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = xedoc_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: xedoc_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: xedoc_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from XEDOC_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Xedoc tooling.
    #[clap(hide = true)]
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`xedoc update` is not available in debug builds. Install a release build of Xedoc to use this command."
        );
    }
...
async fn run_session_archive_cli_command(
    action: xedoc_tui::SessionArchiveAction,
    ...
    xedoc_tui::run_session_archive_command(
        action,
        target,
        xedoc_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<xedoc_tui::SessionArchiveAction> {
    if force && xedoc_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => xedoc_tui::DeleteConfirmation::Skip,
        false => xedoc_tui::DeleteConfirmation::Prompt,
    };
    Ok(xedoc_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let xedoc_bin = std::env::current_exe()?;
            xedoc_app_server_test_client::send_message_v2(&xedoc_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
pub fn run(xedoc_version: &'static str) -> anyhow::Result<()> {
    version::set_xedoc_cli_version(xedoc_version);
    xedoc_tui::set_xedoc_cli_version(xedoc_version);
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            xedoc_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["xedoc", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        xedoc_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = xedoc_app_server::AppServerRuntimeOptions::default();
                    xedoc_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        loader_overrides,
                        strict_config,
                        transport,
                        xedoc_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    ...
                    xedoc_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = xedoc_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    xedoc_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    xedoc_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    xedoc_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                xedoc_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                xedoc_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | xedoc login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            crate::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.xedoc_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            crate::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.xedoc_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux")))]
            {
                ...
                anyhow::bail!("`xedoc sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || xedoc_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `xedoc mcp`: `xedoc`, `xedoc exec`, `xedoc review`, `xedoc resume`, `xedoc archive`, `xedoc delete`, `xedoc unarchive`, `xedoc fork`, `xedoc mcp`, `xedoc sandbox`, `xedoc app-server`, and `xedoc debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let xedoc_self_exe = arg0_paths
        .xedoc_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Xedoc executable path is not configured"))?;
    let runtime_paths = xedoc_exec_server::ExecServerRuntimePaths::new(
        xedoc_self_exe,
        arg0_paths.xedoc_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = xedoc_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
        )?;
        ...
        exec_server_telemetry::run_until_shutdown(async move {
            xedoc_exec_server::run_remote_environment(remote_config, runtime_paths).await
        })
        .await?;
        Ok(())
    } else {
        ...
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| xedoc_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(async move {
            xedoc_exec_server::run_main_with_telemetry(&listen_url, runtime_paths, telemetry).await
        })
        .await
...
async fn load_exec_server_remote_auth_provider(
    ...
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<xedoc_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        let agent_identity_jwt = read_xedoc_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("XEDOC_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth_route_config = config.auth_route_config();
        let auth = XedocAuth::from_agent_identity_jwt(
            &agent_identity_jwt,
            Some(&config.chatgpt_base_url),
            auth_route_config.as_ref(),
        )
        .await?;
        return Ok(xedoc_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `xedoc login` or set XEDOC_API_KEY",
    )
    .await?;
    ...
    Ok(xedoc_model_provider::auth_provider_from_auth(&auth))
}

fn is_supported_exec_server_remote_auth(auth: &XedocAuth) -> bool {
...
async fn load_exec_server_config(
    root_config_overrides: &CliConfigOverrides,
    strict_config: bool,
) -> anyhow::Result<xedoc_core::config::Config> {
...
) -> anyhow::Result<xedoc_login::XedocAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_xedoc_api_key_env*/ true).await;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    maybe_print_under_development_feature_warning(&xedoc_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let xedoc_home = find_xedoc_home()?;
            Ok(LoaderOverrides {
                user_config_path: Some(resolve_profile_v2_config_path(&xedoc_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(xedoc_home: &std::path::Path, feature: &str) {
    ...
    let config_path = xedoc_home.join(xedoc_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(xedoc_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        xedoc_self_exe: arg0_paths.xedoc_self_exe,
        xedoc_linux_sandbox_exe: arg0_paths.xedoc_linux_sandbox_exe,
    ...
    let prompt_input = xedoc_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `xedoc {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `xedoc {subcommand}`"
        );
    }
    Ok(())
}
...
/// `xedoc --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `xedoc {subcommand}`");
    }
    Ok(())
}
...
fn app_server_subcommand_name(subcommand: Option<&AppServerSubcommand>) -> &'static str {
    match subcommand {
        ...
        Some(AppServerSubcommand::Daemon(daemon)) => match daemon.subcommand {
            AppServerDaemonSubcommand::Bootstrap => "app-server daemon bootstrap",
...
async fn run_interactive_tui(
    ...
    let terminal_info = xedoc_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Xedoc's interactive TUI may not work in this terminal."
        );
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Xedoc local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<xedoc_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(xedoc_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !xedoc_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let xedoc_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `xedoc resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `xedoc` without additional flags.
...
/// Build the final `TuiCli` for a `xedoc fork` invocation.
...
mod tests {
    ...
    use xedoc_tui::TokenUsage;

    #[test]
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = XedocAuth::from_api_key("sk-test");
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["xedoc", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["xedoc", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["xedoc", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["xedoc", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["xedoc", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["xedoc", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(xedoc_exec::Command::Resume(args)) = exec.command else {
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let xedoc_home = find_xedoc_home().expect("xedoc home");
        xedoc_app_server::app_server_control_socket_path(&xedoc_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["xedoc", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: xedoc plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: xedoc plugin marketplace add"),
            ("list", "Usage: xedoc plugin marketplace list"),
            ("upgrade", "Usage: xedoc plugin marketplace upgrade"),
            ("remove", "Usage: xedoc plugin marketplace remove"),
        ] {
            let help = help_from_args(&["xedoc", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["xedoc", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["xedoc", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["xedoc", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["xedoc", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["xedoc", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["xedoc", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["xedoc", "sandbox", "--full-auto", "--"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: xedoc_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run xedoc resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run xedoc resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["xedoc", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["xedoc", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["xedoc", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["xedoc", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["xedoc", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["xedoc", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["xedoc", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["xedoc", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["xedoc", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(xedoc_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(xedoc_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["xedoc", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["xedoc", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["xedoc", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["xedoc", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["xedoc", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["xedoc", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["xedoc", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["xedoc", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_defaults_to_stdio_transport() {
        let app_server = app_server_from_args(["xedoc", "app-server"].as_ref());
        assert_eq!(
            app_server.listen,
            xedoc_app_server::AppServerTransport::Stdio
        );
    ...
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["xedoc", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["xedoc", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `xedoc mcp`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["xedoc", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `xedoc app-server proxy`"
        );
    }

    #[test]
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "--remote", "unix://xedoc.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://xedoc.sock"));
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["xedoc", "resume", "--remote", "unix://xedoc.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://xedoc.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("XEDOC_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("XEDOC_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["xedoc", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            xedoc_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["xedoc", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            xedoc_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["xedoc", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["xedoc", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            xedoc_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["xedoc", "app-server", "--listen", "unix:///tmp/xedoc.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            xedoc_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/xedoc.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["xedoc", "app-server", "--listen", "off"].as_ref());
        assert_eq!(app_server.listen, xedoc_app_server::AppServerTransport::Off);
    }

    #[test]
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["xedoc", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["xedoc", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        assert!(matches!(
            app_server_from_args(["xedoc", "app-server", "daemon", "bootstrap"].as_ref())
                .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Bootstrap
            }))
        ));
        assert!(matches!(
            app_server_from_args(["xedoc", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["xedoc", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Restart
            }))
        ));
        assert!(matches!(
            app_server_from_args(["xedoc", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["xedoc", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["xedoc", "app-server", "proxy", "--sock", "xedoc.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("xedoc.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(xedoc_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/xedoc-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(xedoc_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/xedoc-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["xedoc", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("xedoc")
