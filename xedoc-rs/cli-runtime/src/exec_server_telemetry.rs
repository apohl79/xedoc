...
const OTEL_SERVICE_NAME: &str = "xedoc-exec-server";

pub(crate) fn init(
    config: Option<&xedoc_core::config::Config>,
) -> (impl Send + Sync, xedoc_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => xedoc_core::otel_init::build_provider(
            config,
    ...
    xedoc_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(xedoc_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();
