...
use xedoc_core_plugins::ConfiguredMarketplace;
use xedoc_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use xedoc_core_plugins::PluginInstallOutcome;
...
use xedoc_core_plugins::allowed_configured_marketplace_names;
use xedoc_core_plugins::installed_marketplaces::marketplace_install_root;
use xedoc_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use xedoc_core_plugins::marketplace::MarketplaceListError;
use xedoc_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use xedoc_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use xedoc_core_plugins::marketplace::MarketplacePluginSource;
use xedoc_core_plugins::marketplace::find_marketplace_manifest_path;
use xedoc_login::XedocAuth;
use xedoc_login::auth::read_xedoc_api_key_from_env;
use xedoc_plugin::PluginId;
use xedoc_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin add",
    after_help = "Examples:\n  xedoc plugin add sample@debug\n  xedoc plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin list",
    after_help = "Examples:\n  xedoc plugin list\n  xedoc plugin list --marketplace debug\n  xedoc plugin list --json\n  xedoc plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin remove",
    after_help = "Examples:\n  xedoc plugin remove sample@debug\n  xedoc plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplaces = outcome
        ...
        .collect::<Vec<_>>();
    let marketplace_sources = configured_marketplace_sources(&plugins_input, xedoc_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    xedoc_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    xedoc_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    xedoc_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<xedoc_core_plugins::ConfiguredMarketplace>,
...
impl JsonPluginListEntry {
    fn from_configured_plugin(
        ...
        marketplace_source: Option<JsonMarketplaceSource>,
        plugin: xedoc_core_plugins::ConfiguredMarketplacePlugin,
    ) -> Self {
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, xedoc_home);
...
async fn load_plugin_command_context(
    ...
    let manager = PluginsManager::new(xedoc_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_xedoc_api_key_from_env() {
        return Some(XedocAuth::from_api_key(&api_key).api_auth_mode());
    }

    let auth_route_config = config.auth_route_config();
    XedocAuth::from_auth_storage(
        &config.xedoc_home,
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(xedoc_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, xedoc_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
