...
use xedoc_core::McpManager;
use xedoc_core::config::Config;
use xedoc_core::config::ConfigBuilder;
use xedoc_core::config::LoaderOverrides;
use xedoc_core::config::edit::ConfigEditsBuilder;
use xedoc_core::config::find_xedoc_home;
use xedoc_core::config::load_global_mcp_servers;
use xedoc_core_plugins::PluginsManager;
use xedoc_exec_server::EnvironmentManager;
use xedoc_login::AuthManager;
use xedoc_mcp::McpOAuthLoginSupport;
use xedoc_mcp::McpRuntimeContext;
use xedoc_mcp::ResolvedMcpOAuthScopes;
use xedoc_mcp::compute_auth_statuses;
use xedoc_mcp::discover_supported_scopes;
use xedoc_mcp::oauth_login_support;
use xedoc_mcp::resolve_oauth_scopes;
use xedoc_mcp::should_retry_without_scopes;
use xedoc_protocol::protocol::McpAuthStatus;
use xedoc_rmcp_client::delete_oauth_tokens;
use xedoc_rmcp_client::perform_oauth_login;
use xedoc_utils_cli::CliConfigOverrides;
use xedoc_utils_cli::format_env_display;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.xedoc/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "xedoc mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: xedoc_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: xedoc_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&xedoc_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", xedoc_home.display()))?;
    ...
    ConfigEditsBuilder::new(&xedoc_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", xedoc_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `xedoc mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `xedoc mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: xedoc mcp remove {}", get_args.name);
