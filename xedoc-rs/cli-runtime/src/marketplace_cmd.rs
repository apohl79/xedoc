...
use xedoc_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use xedoc_core_plugins::marketplace::marketplace_root_dir;
use xedoc_core_plugins::marketplace_add::MarketplaceAddOutcome;
use xedoc_core_plugins::marketplace_add::MarketplaceAddRequest;
use xedoc_core_plugins::marketplace_add::add_marketplace;
use xedoc_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use xedoc_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "xedoc plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Xedoc is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin marketplace add",
    after_help = "Examples:\n  xedoc plugin marketplace add ./path/to/marketplace\n  xedoc plugin marketplace add owner/repo --ref main\n  xedoc plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "xedoc plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin marketplace upgrade",
    after_help = "Examples:\n  xedoc plugin marketplace upgrade\n  xedoc plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "xedoc plugin marketplace remove",
    after_help = "Example:\n  xedoc plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.xedoc_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.xedoc_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<xedoc_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, xedoc_home);
