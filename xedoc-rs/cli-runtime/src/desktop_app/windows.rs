...
const XEDOC_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const XEDOC_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";
...
pub async fn run_windows_app_open_or_install(
    ...
    if xedoc_app_is_installed().await? {
        eprintln!("Opening workspace {display_workspace} in the Desktop app...");
        open_url(&xedoc_new_thread_url(&workspace_path)).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(XEDOC_WINDOWS_INSTALLER_URL);
    if open_url(download_url).await.is_err() && download_url_override.is_none() {
        open_url(XEDOC_MICROSOFT_STORE_WEB_URL).await?;
    }
    eprintln!("After installing the Desktop app, open workspace {display_workspace}.");
    Ok(())
}

async fn xedoc_app_is_installed() -> anyhow::Result<bool> {
    // This package identity is stable across Xedoc- and ChatGPT-branded builds.
...
fn xedoc_new_thread_url(workspace: &str) -> String {
...
mod tests {
    ...
    fn display_workspace_path_removes_windows_extended_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\C:\Users\fcoury\code\xedoc")),
            r"C:\Users\fcoury\code\xedoc"
        );
    }
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\xedoc")),
            r"\\server\share\xedoc"
        );
    }
    ...
    #[test]
    fn xedoc_new_thread_url_encodes_windows_workspace_path() {
        assert_eq!(
            xedoc_new_thread_url(r"C:\Users\akuma\repos\koba"),
            r"codex://threads/new?path=C%3A%5CUsers%5Cakuma%5Crepos%5Ckoba"
        );
    }

    #[test]
    fn xedoc_new_thread_url_preserves_verbatim_workspace_path() {
        assert_eq!(
            xedoc_new_thread_url(r"\\?\C:\Users\akuma\repos\koba"),
