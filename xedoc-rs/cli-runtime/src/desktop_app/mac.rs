...
const XEDOC_BUNDLE_IDENTIFIER: &str = "com.openai.codex";
const XEDOC_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/codex-app-prod/Codex.dmg";
const XEDOC_DMG_URL_X64: &str =
...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_xedoc_app_path(&xedoc_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_xedoc_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let installed_app = download_and_install_xedoc_to_user_applications(&download_url)
        .await
    ...
    open_xedoc_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_xedoc_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Codex.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_xedoc_app_bundle(candidate))
}

fn xedoc_app_search_dirs() -> Vec<PathBuf> {
...
fn is_xedoc_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == XEDOC_BUNDLE_IDENTIFIER
        })
}

async fn open_xedoc_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = xedoc_new_thread_url(workspace);
...
fn xedoc_new_thread_url(workspace: &Path) -> String {
...
async fn download_and_install_xedoc_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("xedoc-app-installer-")
        .tempdir()
    ...
    let result = async {
        let app_in_volume = find_xedoc_app_in_mount(&mount_point)
            .context("failed to locate Codex.app in mounted dmg")?;
        install_xedoc_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_xedoc_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
...
fn find_xedoc_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
...
#[cfg(test)]
mod tests {
    use super::find_existing_xedoc_app_path;
    use super::parse_hdiutil_attach_mount_point;
    use super::xedoc_new_thread_url;
    ...
    #[test]
    fn finds_chatgpt_app_with_xedoc_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.codex");

        assert_eq!(
            find_existing_xedoc_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let xedoc_app_path = temp_dir.path().join("Codex.app");
        write_app_bundle(&xedoc_app_path, "com.openai.codex");
    ...
    #[test]
    fn xedoc_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&xedoc_new_thread_url(Path::new("/tmp/xedoc workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/xedoc workspace/#1".to_string())],
            )
        );
    }
}
