...
//!
//! This crate wraps [`xedoc_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](xedoc_app_server::in_process::InProcessClientHandle),
...
use tracing::warn;
pub use xedoc_app_server::app_server_control_socket_path;
pub use xedoc_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use xedoc_app_server::in_process::InProcessServerEvent;
use xedoc_app_server::in_process::InProcessStartArgs;
use xedoc_app_server::in_process::LogDbLayer;
pub use xedoc_app_server::in_process::StateDbHandle;
...
use xedoc_app_server_protocol::Result as JsonRpcResult;
...
use xedoc_config::NoopThreadConfigLoader;
use xedoc_config::RemoteThreadConfigLoader;
use xedoc_config::ThreadConfigLoader;
use xedoc_core::config::Config;
pub use xedoc_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `xedoc-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use xedoc_core::config::*;

        pub mod edit {
            pub use xedoc_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// SQLite tracing layer capturing recently emitted logs.
    ...
    /// Whether auth loading should honor the `XEDOC_API_KEY` environment variable.
    pub enable_xedoc_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_xedoc_api_key_env: self.enable_xedoc_api_key_env,
            initialize,
...
/// - [`xedoc_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            xedoc_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn xedoc_home(&self, local_xedoc_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_xedoc_home.display().to_string(),
            )),
            Self::Remote(client) => client.xedoc_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use xedoc_app_server_protocol::ServerNotification;
    use xedoc_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use xedoc_core::init_state_db;
    use xedoc_uds::UnixListener;
    use xedoc_utils_absolute_path::AbsolutePathBuf;
    ...
    async fn build_test_config_for_xedoc_home(xedoc_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_xedoc_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let xedoc_home = TempDir::new().expect("temp dir");
        let config = Arc::new(build_test_config_for_xedoc_home(xedoc_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "xedoc-app-server-client-test".to_string(),
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            xedoc_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            xedoc_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(xedoc_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: xedoc_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
            },
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(xedoc_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: xedoc_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: xedoc_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: xedoc_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: xedoc_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<xedoc_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                xedoc_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == xedoc_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.xedoc_home(), Some("/server/.xedoc"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: xedoc_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("xedoc.sock"))
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == xedoc_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                xedoc_app_server_protocol::ServerNotification::TurnCompleted(
                    xedoc_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: xedoc_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: xedoc_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                xedoc_app_server_protocol::ServerNotification::AgentMessageDelta(
                    xedoc_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                xedoc_app_server_protocol::ServerNotification::ItemCompleted(
                    xedoc_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: xedoc_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                xedoc_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    xedoc_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            xedoc_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
