...
use xedoc_utils_path_uri::PathUriParseError;
...
/// Special argv[1] flag used when the Xedoc executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `xedoc-apply-patch` (to avoid forcing
/// `xedoc-arg0` to depend on `xedoc-core`), it remains part of the "xedoc core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const XEDOC_CORE_APPLY_PATCH_ARG1: &str = "--xedoc-run-as-apply-patch";
