//! Canonicalizes and resolves skill-declared MCP server requirements.
...
use xedoc_core_skills::model::SkillMetadata;
use xedoc_core_skills::model::SkillToolDependency;

/// Formats the names of missing MCP servers for a user-facing prompt.
#[doc(hidden)]
pub fn format_missing_mcp_dependencies(missing: &HashMap<String, McpServerConfig>) -> String {
...
/// Returns the stable identity used to match configured MCP servers.
#[doc(hidden)]
...
/// Returns the distinct skill-declared MCP servers absent from the configuration.
#[doc(hidden)]
