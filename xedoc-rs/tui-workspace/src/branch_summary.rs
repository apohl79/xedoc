...
pub use xedoc_tui_events::GitBranchDiffStats;
pub use xedoc_tui_events::StatusLineGitSummary;
pub use xedoc_tui_events::StatusLinePullRequest;
...
pub async fn current_branch_name(
...
pub async fn status_line_git_summary(
    runner: &dyn WorkspaceCommandExecutor,
    cwd: &Path,
...
mod tests {
    ...
    async fn open_pull_request_falls_back_to_parent_repo_commit_lookup() {
        let runner = FakeRunner::new(vec![
            ...
            response(
                ...
                /*exit_code*/ 0,
                r#"{"nameWithOwner":"fcoury/xedoc","parent":{"nameWithOwner":"openai/codex"}}"#,
            ),
            response(
                &[
                    "gh",
    ...
    fn status_line_pr_fallback_searches_parent_repo_first() {
        assert_eq!(
            ...
            Some(vec!["openai/codex".to_string(), "fcoury/xedoc".to_string()])
        );
    }
