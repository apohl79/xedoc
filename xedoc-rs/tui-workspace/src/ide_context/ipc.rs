...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "xedoc-tui";
#[cfg(any(unix, windows))]
const OPEN_IDE_HINT: &str =
    "Open this project in VS Code or Cursor with the Xedoc extension active.";
#[cfg(any(unix, windows))]
const IDE_DID_NOT_PROVIDE_CONTEXT_HINT: &str = "The IDE extension did not provide context.";
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "Xedoc will keep trying on future messages.";

#[derive(Debug, Error)]
pub enum IdeContextError {
    #[cfg(any(unix, windows))]
    ...
    pub fn user_facing_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Send(_) => {
                "Xedoc could not request IDE context. Try /ide again.".to_string()
            }
            IdeContextError::Read(_) | IdeContextError::InvalidResponse(_) => {
                "Xedoc could not read IDE context. Try /ide again.".to_string()
            }
        }
    }

    #[cfg(any(unix, windows))]
    pub fn prompt_skip_hint(&self) -> String {
        match self {
            IdeContextError::ResponseTooLarge => {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "Xedoc timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            IdeContextError::RequestFailed(error) if error == "client-disconnected" => {
                hint_with_retry("The IDE connection changed while Xedoc was requesting context.")
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("Xedoc lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("Xedoc received an unexpected IDE context response.")
            }
            IdeContextError::RequestFailed(_) => hint_with_retry(IDE_DID_NOT_PROVIDE_CONTEXT_HINT),
            IdeContextError::Read(_) => hint_with_retry("Xedoc could not read IDE context."),
        }
    }
...
#[cfg(unix)]
fn primary_ipc_socket_path(xedoc_home: &Path) -> PathBuf {
    xedoc_home.join("ipc").join("ipc.sock")
}

#[cfg(unix)]
fn legacy_ipc_socket_paths(temp_dir: &Path, uid: libc::uid_t) -> Vec<PathBuf> {
    let ipc_dir = temp_dir.join("xedoc-ipc");
    if uid == 0 {
...
mod tests {
    ...
    #[test]
    fn primary_ipc_socket_path_uses_xedoc_home() {
        let xedoc_home = Path::new("/home/test/.xedoc");

        assert_eq!(
            primary_ipc_socket_path(xedoc_home),
            xedoc_home.join("ipc").join("ipc.sock")
        );
    }
    ...
    fn validate_unix_socket_path_rejects_unsafe_parent_directory() {
        ...
        let socket_path = tempdir.path().join("xedoc-ipc.sock");
