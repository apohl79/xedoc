...
pub struct McpToolCallCell {
    ...
    result: Option<Result<xedoc_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub fn complete(
        &mut self,
        duration: Duration,
        result: Result<xedoc_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(xedoc_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`xedoc-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `xedoc mcp add`.
...
pub fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, xedoc_protocol::mcp::Tool>,
