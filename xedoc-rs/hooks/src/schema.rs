...
pub(crate) struct PreToolUseCommandInput {
    ...
    /// Xedoc extension: expose the active turn id to internal turn-scoped hooks.
    pub turn_id: String,
...
fn session_start_source_schema(_gen: &mut SchemaGenerator) -> Schema {
    string_enum_schema(&["startup", "resume", "clear", "compact", "model_change"])
}
...
mod tests {
    ...
    #[test]
    fn turn_scoped_hook_inputs_include_xedoc_turn_id_extension() {
        // Xedoc intentionally diverges from Claude's public hook docs here so
