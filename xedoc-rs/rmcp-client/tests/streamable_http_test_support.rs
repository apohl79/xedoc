...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    xedoc_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("xedoc-test", "0.0.0-test").with_title("Xedoc rmcp recovery test"),
    )
...
pub(crate) async fn spawn_exec_server() -> anyhow::Result<ExecServerProcess> {
    ...
    let mut child = Command::new(xedoc_utils_cargo_bin::cargo_bin("xedoc")?)
...
/// Reads the websocket URL printed by `xedoc exec-server --listen`.
