...
const LOCK_CONTENTION_EVENT_TARGET: &str = "xedoc_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are XEDOC_HOME-backed today: if XEDOC_HOME is unset they use
        // the default home (`~/.xedoc`), and if an embedder has no local home/filesystem authority
        ...
        let xedoc_home = find_xedoc_home()
            .map_err(|source| OAuthStoreLockFailure::XedocHome { store, source })?;
        Self::acquire_in(&xedoc_home, store, STORE_LOCK_ACQUIRE_TIMEOUT)
    }
    ...
    pub(super) fn acquire_in(
        ...
        let path = oauth_store_lock_path(xedoc_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve XEDOC_HOME for MCP OAuth {store} aggregate-store lock")]
    XedocHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(xedoc_home: &Path, store: OAuthStore) -> PathBuf {
    xedoc_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}
