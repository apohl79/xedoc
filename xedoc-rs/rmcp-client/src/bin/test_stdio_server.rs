...
const MEMO_URI: &str = "memo://xedoc/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "xedoc/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Xedoc TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Xedoc TUI):
    /// - Build this binary: `cargo build -p xedoc-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `xedoc mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Xedoc TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://xedoc/{slug}".to_string(),
            name: "xedoc-memo".to_string(),
            title: Some("Xedoc Memo".to_string()),
            description: Some(
                "Template for memo://xedoc/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Xedoc TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("xedoc-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("xedoc/encryptedContent".to_string(), json!(true));
