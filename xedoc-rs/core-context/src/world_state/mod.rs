...
use xedoc_extension_api::RenderedWorldStateFragment;
...
#[serde(transparent)]
pub struct WorldStateHash(String);

impl WorldStateHash {
    pub fn from_fragment(fragment: &(impl ContextualUserFragment + ?Sized)) -> Self {
        let mut hasher = Sha1::new();
        hasher.update(b"xedoc-world-state-fragment-v1\0");
...
impl WorldStateSnapshot {
    ...
    pub fn merge_patch_from(&self, previous: &Self) -> Option<Value> {
    ...
    pub fn apply_merge_patch(&mut self, patch: &Value) -> serde_json::Result<()> {
...
impl WorldState {
    pub fn add_section<S: WorldStateSection>(&mut self, section: S) {
        let id = S::ID;
        assert!(
    ...
    pub fn add_extension_section(&mut self, section: WorldStateSectionContribution) {
        let id = section.id();
        assert!(
            !self.sections.contains_key(id),
            "duplicate world-state section ID: {id}"
    ...
    pub fn render_full(&self) -> Vec<Box<dyn ContextualUserFragment>> {
