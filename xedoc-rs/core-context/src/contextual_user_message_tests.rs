...
use crate::InternalContextSource;
use crate::InternalModelContextFragment;
use crate::SubagentNotification;
...
fn detects_internal_model_context_fragment() {
    ...
    assert_eq!(
        text,
        "<xedoc_internal_context source=\"extension\">\nInternal steering.\n</xedoc_internal_context>"
    );
    assert!(is_contextual_user_fragment(&ContentItem::InputText {
        text
    }));
...
fn rejects_invalid_internal_model_context_source() {
    assert!(!is_contextual_user_fragment(&ContentItem::InputText {
        text: "<xedoc_internal_context source=\"Extension\">\nbody\n</xedoc_internal_context>"
            .to_string(),
    }));
}
