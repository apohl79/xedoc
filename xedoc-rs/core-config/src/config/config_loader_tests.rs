...
use xedoc_config::ConfigLoadOptions;
use xedoc_config::ConfigRequirements;
use xedoc_config::ConfigRequirementsToml;
use xedoc_config::ConfigRequirementsWithSources;
use xedoc_config::FilesystemDenyReadPattern;
use xedoc_config::LoaderOverrides;
use xedoc_config::RequirementSource;
use xedoc_config::RequirementsLayerEntry;
use xedoc_config::SessionThreadConfig;
use xedoc_config::StaticThreadConfigLoader;
use xedoc_config::ThreadConfigSource;
use xedoc_config::compose_requirements;
use xedoc_config::config_error_from_ignored_toml_fields;
use xedoc_config::config_error_from_toml;
...
use xedoc_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_xedoc_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_xedoc_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_xedoc_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let xedoc_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when XEDOC_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = xedoc_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        xedoc_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = xedoc_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = xedoc_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when XEDOC_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![xedoc_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(xedoc_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(xedoc_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_includes_cloud_config_bundle() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let xedoc_home = tmp.path().join("home");
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(xedoc_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_xedoc_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let xedoc_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&xedoc_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", xedoc_home.display()))
                .expect("bundle requirements path should resolve against xedoc_home")
        ])
    );
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            xedoc_config::SandboxModeRequirement::ReadOnly,
            xedoc_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    xedoc_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".xedoc")).await?;
    tokio::fs::create_dir_all(project_root.join(".xedoc")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".xedoc").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".xedoc").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_xedoc_folder } => Some(dot_xedoc_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".xedoc").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".xedoc").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".xedoc")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".xedoc")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".xedoc"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".xedoc"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".xedoc"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".xedoc"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".xedoc"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_xedoc_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".xedoc").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".xedoc").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".xedoc").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".xedoc").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let xedoc_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&xedoc_home).await?;
    tokio::fs::write(xedoc_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_xedoc_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_xedoc_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".xedoc"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn xedoc_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let xedoc_home = home_dir.join(".xedoc");
    tokio::fs::create_dir_all(&xedoc_home).await?;
    tokio::fs::write(
        xedoc_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn xedoc_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_xedoc = project_root.join(".xedoc");
    let nested_dot_xedoc = nested.join(".xedoc");

    tokio::fs::create_dir_all(&nested_dot_xedoc).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_xedoc.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_xedoc).await?;
    make_config_for_test(
        &project_dot_xedoc,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_xedoc.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_xedoc_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_xedoc)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let xedoc_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&xedoc_home_untrusted).await?;
    make_config_for_test(
        &xedoc_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = xedoc_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let xedoc_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&xedoc_home_unknown).await?;
    tokio::fs::write(
        xedoc_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_xedoc).await?;
    ...
    // `.xedoc` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_xedoc.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_xedoc.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.xedoc` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".xedoc")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".xedoc").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let xedoc_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&xedoc_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let xedoc_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&xedoc_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &xedoc_home,
