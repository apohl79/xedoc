...
use xedoc_config::ConfigRequirements;
...
use xedoc_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use xedoc_config::config_toml::ProjectConfig;
use xedoc_config::config_toml::ThreadStoreToml;
use xedoc_config::config_toml::validate_model_providers;
use xedoc_config::loader::load_config_layers_state;
use xedoc_config::loader::project_trust_key;
use xedoc_config::permissions_toml::PermissionsToml;
use xedoc_config::profile_toml::ConfigProfile;
use xedoc_config::sandbox_mode_requirement_for_permission_profile;
use xedoc_config::types::AuthCredentialsStoreMode;
use xedoc_config::types::AuthKeyringBackendKind;
use xedoc_config::types::History;
use xedoc_config::types::McpServerConfig;
use xedoc_config::types::McpServerDisabledReason;
...
use xedoc_core_plugins::PluginsConfigInput;
use xedoc_exec_server::ExecutorFileSystem;
use xedoc_exec_server::LOCAL_FS;
use xedoc_features::CurrentTimeReminderConfigToml;
use xedoc_features::CurrentTimeReminderDeliveryMode;
use xedoc_features::CurrentTimeSource;
use xedoc_features::Feature;
use xedoc_features::FeatureConfigSource;
use xedoc_features::FeatureOverrides;
use xedoc_features::FeatureToml;
...
use xedoc_login::AuthManagerConfig;
...
use xedoc_protocol::config_types::SandboxMode;
...
use xedoc_protocol::config_types::Verbosity;
use xedoc_protocol::config_types::WebSearchConfig;
...
pub use xedoc_thread_store::ExtraConfig;
...
use crate::config::permissions::get_readable_roots_required_for_xedoc_runtime;
...
mod auth_keyring;
pub mod edit;
use xedoc_core_config_runtime::managed_features;
use xedoc_core_config_runtime::otel;
...
pub use xedoc_config::ConfigLoadOptions;
pub use xedoc_config::Constrained;
pub use xedoc_config::ConstraintError;
pub use xedoc_config::ConstraintResult;
pub use xedoc_config::LoaderOverrides;
pub use xedoc_core_config_runtime::managed_features::ManagedFeatures;
pub use xedoc_core_config_runtime::network_proxy_spec::NetworkProxySpec;
pub use xedoc_core_config_runtime::network_proxy_spec::StartedNetworkProxy;
...
const DEFAULT_MULTI_AGENT_V2_TOOL_NAMESPACE: &str = "multi_agent_v2";
const DEFAULT_MULTI_AGENT_V2_SHARED_USAGE_HINT_TEXT: &str = r#"Note that collaboration tools cannot be called from inside `functions.exec`. Call `spawn_agent`, `send_message`, `followup_task`, `wait_agent`, `interrupt_agent`, and `list_agents` only as direct tool calls using the recipient shown in their tool definitions, such as `to=functions.multi_agent_v2.spawn_agent`, since they are intentionally absent from the `functions.exec` `tools.*` namespace. Available tools in `functions.exec` are explicitly described with a `tools` namespace in the developer message.
...
#[doc(hidden)]
pub const DEFAULT_AGENT_MAX_DEPTH: i32 = 2;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(xedoc_state::SQLITE_HOME_ENV).ok()?;
...
impl Permissions {
    ...
    /// selection. Internal Xedoc-only writable roots are intentionally excluded.
...
// when Xedoc is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<xedoc_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Xedoc will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Xedoc
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.xedoc/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Xedoc"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Xedoc '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Xedoc home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Xedoc can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Xedoc unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/codex/blob/main/codex-rs/rmcp-client/src/oauth.rs#L2
    /// file: XEDOC_HOME/.credentials.json
    ///       This file will be readable to Xedoc and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Xedoc will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Xedoc state (defaults to `~/.xedoc` but can be
    /// overridden by the `XEDOC_HOME` environment variable).
    pub xedoc_home: AbsolutePathBuf,

    /// Directory where Xedoc stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Xedoc writes log files (defaults to `$XEDOC_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Xedoc writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Xedoc version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_xedoc_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.xedoc/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Xedoc executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub xedoc_self_exe: Option<PathBuf>,

    /// Path to the `xedoc-linux-sandbox` executable. This must be set if
    /// [`xedoc_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `xedoc-linux-sandbox`.
    pub xedoc_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `xedoc-execve-wrapper` executable used for shell
    ...
    /// Whether Xedoc-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for Xedoc updates on startup and surfaces update prompts.
    /// Set to `false` only if your Xedoc updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// Whether Xedoc should generate short session names automatically.
    pub auto_session_name: bool,

    /// OTEL configuration (exporter type, endpoint, headers, etc.).
    pub otel: xedoc_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn xedoc_home(&self) -> PathBuf {
        self.xedoc_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn xedoc_home(mut self, xedoc_home: PathBuf) -> Self {
        self.xedoc_home = Some(xedoc_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let xedoc_home = match xedoc_home {
            Some(xedoc_home) => AbsolutePathBuf::from_absolute_path(xedoc_home)?,
            None => find_xedoc_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&xedoc_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = xedoc_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    xedoc_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(xedoc_config::io_error_from_config_error(
        ...
        {
            let allow_xedoc_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_xedoc_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_xedoc_version_mismatch = allow_xedoc_version_mismatch;
...
impl Config {
    #[doc(hidden)]
    pub fn multi_agent_version_override(&self) -> Option<MultiAgentVersion> {
    ...
    #[doc(hidden)]
    pub fn multi_agent_version_from_features(&self) -> MultiAgentVersion {
    ...
    #[doc(hidden)]
    pub fn multi_agent_version_for_model(
        &self,
    ...
    #[doc(hidden)]
    pub fn effective_agent_max_threads(
        &self,
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &xedoc_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    #[doc(hidden)]
    pub fn to_mcp_config_with_loaded_plugins(
        &self,
        ...
        McpConfig {
            xedoc_home: self.xedoc_home.to_path_buf(),
            ...
            xedoc_linux_sandbox_exe: self.xedoc_linux_sandbox_exe.clone(),
    ...
    #[doc(hidden)]
    pub fn prefix_mcp_tool_names(&self) -> bool {
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.xedoc_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_xedoc_home(
    ...
    /// Load a default configuration for a specific Xedoc home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_xedoc_home(
        xedoc_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let cli_layer = xedoc_config::build_cli_overrides_layer(&cli_overrides);
        xedoc_config::merge_toml_values(&mut merged, &cli_layer);
        let xedoc_home = AbsolutePathBuf::from_absolute_path_checked(xedoc_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &xedoc_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
    ...
    /// ignores user settings. For example, the `xedoc exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `xedoc_self_exe`, `xedoc_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(xedoc_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(xedoc_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &xedoc_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, xedoc_home).map_err(|e| {
...
#[doc(hidden)]
pub fn model_catalogs_for_config(config: &Config) -> HashMap<String, ModelsResponse> {
    ...
    {
        let _guard = AbsolutePathBufGuard::new(config.xedoc_home.as_path());
    ...
    let mut profile_paths = std::fs::read_dir(config.xedoc_home.as_path())
        .ok()
        .into_iter()
        .flatten()
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .xedoc/ folders.
...
/// Patch `XEDOC_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(xedoc_home: &Path, provider: &str) -> std::io::Result<()> {
    xedoc_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &xedoc_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            xedoc_protocol::permissions::FileSystemSandboxEntry {
                path: xedoc_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: xedoc_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            xedoc_protocol::permissions::FileSystemSandboxEntry {
                path: xedoc_protocol::permissions::FileSystemPath::Path { path },
                access: xedoc_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&xedoc_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub async fn load_config_with_layer_stack(
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(xedoc_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| xedoc_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| xedoc_home.to_path_buf());
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Xedoc would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let helper_readable_roots = get_readable_roots_required_for_xedoc_runtime(
            &xedoc_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_xedoc_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_xedoc_version_mismatch)
            ...
            tui_status_line_command: cfg
                ...
                .or_else(|| default_xedoc_statusline(xedoc_home.as_path())),
...
/// Returns the path to the Xedoc configuration directory, which can be
/// specified by the `XEDOC_HOME` environment variable. If not set, defaults to
/// `~/.xedoc`.
///
/// - If `XEDOC_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `XEDOC_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_xedoc_home() -> std::io::Result<AbsolutePathBuf> {
    xedoc_utils_home_dir::find_xedoc_home()
}

/// Returns a default statusline command pointing at `$XEDOC_HOME/statusline.sh`
/// when that file exists and the user has not configured an explicit command.
fn default_xedoc_statusline(xedoc_home: &Path) -> Option<StatusLineCommand> {
    let path = xedoc_home.join("statusline.sh");
    path.is_file()
        .then(|| StatusLineCommand::Args(vec![path.to_string_lossy().to_string()]))
}

/// Returns the path to the folder where Xedoc logs are stored. Does not verify
