...
use xedoc_config::ConfigLayerEntry;
use xedoc_config::ConfigLayerSource;
use xedoc_config::ConfigLayerStack;
use xedoc_config::McpServerCommandMatcher;
use xedoc_config::McpServerIdentity;
use xedoc_config::McpServerRequirement;
use xedoc_config::McpServerValueMatcher;
use xedoc_config::ProfileV2Name;
use xedoc_config::RequirementSource;
use xedoc_config::Sourced;
use xedoc_config::config_toml::AgentRoleToml;
use xedoc_config::config_toml::AgentsToml;
use xedoc_config::config_toml::ConfigToml;
use xedoc_config::config_toml::ExperimentalRequestUserInput;
use xedoc_config::config_toml::ProjectConfig;
use xedoc_config::config_toml::ToolsToml;
...
use xedoc_config::permissions_toml::NetworkDomainPermissionToml;
use xedoc_config::permissions_toml::NetworkDomainPermissionsToml;
use xedoc_config::permissions_toml::NetworkMitmActionToml;
use xedoc_config::permissions_toml::NetworkMitmHookToml;
use xedoc_config::permissions_toml::NetworkMitmToml;
...
use xedoc_config::permissions_toml::WorkspaceRootsToml;
use xedoc_config::types::AppToolApproval;
use xedoc_config::types::BundledSkillsConfig;
use xedoc_config::types::HistoryPersistence;
use xedoc_config::types::McpServerEnvVar;
use xedoc_config::types::McpServerOAuthConfig;
...
use xedoc_config::types::NotificationCondition;
use xedoc_config::types::NotificationMethod;
use xedoc_config::types::Notifications;
use xedoc_config::types::OtelConfigToml;
use xedoc_config::types::OtelExporterKind;
...
use xedoc_network_proxy::NetworkMode;
...
use xedoc_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use xedoc_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use xedoc_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use xedoc_protocol::models::ManagedFileSystemPermissions;
...
use xedoc_protocol::protocol::NetworkAccess;
...
use tempfile::TempDir;
use xedoc_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: xedoc_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
"#;
    ...
    assert_eq!(
        cfg.tui
            .expect("tui config should deserialize")
            .status_line_command,
        Some(xedoc_config::types::StatusLineCommand::Command(
...
"#,
    ...
    assert_eq!(
        ...
        xedoc_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    Ok(())
}
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        xedoc_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
#[tokio::test]
async fn permission_profile_override_keeps_xedoc_home_out_of_legacy_projection()
-> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let cwd = TempDir::new()?;
...
-> std::io::Result<()> {
    ...
    let xedoc_home = temp_dir.path().join("xedoc-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".xedoc"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".xedoc".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".xedoc"), cwd.as_path()),
            "expected .xedoc carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".xedoc"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".xedoc"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Xedoc and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Xedoc and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Xedoc."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            xedoc_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, xedoc_home.path());
    let project_dot_xedoc =
        AbsolutePathBuf::resolve_path_against_base("project/.xedoc", xedoc_home.path());
    ...
    let requirements_toml = xedoc_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = xedoc_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_xedoc_folder: project_dot_xedoc.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(xedoc_home.path().to_path_buf()),
            ..Default::default()
        },
        xedoc_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_xedoc_folder: project_dot_xedoc,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let xedoc_home = TempDir::new()?;
    let plugin_root = xedoc_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".xedoc-plugin"))?;
    std::fs::write(
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = PluginsManager::new(xedoc_home.path().to_path_buf());
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            xedoc_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let default_zsh_path = xedoc_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_xedoc_home_for_workspace_write() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, xedoc_home.path().to_path_buf());
...
async fn workspace_write_includes_configured_writable_root_once() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let writable_root = xedoc_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".xedoc");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let xedoc_home = TempDir::new()?;
    let managed_path = xedoc_home.path().join("managed_config.toml");
    let config_path = xedoc_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), xedoc_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(xedoc_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let xedoc_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(xedoc_home.path());
    ...
    let loaded = load_global_mcp_servers(xedoc_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(xedoc_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/xedoc-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/xedoc-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-xedoc-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-xedoc-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-xedoc-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(xedoc_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(xedoc_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let xedoc_home = TempDir::new()?;
    let base_config = xedoc_home.path().join(CONFIG_TOML_FILE);
    let selected_config = xedoc_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(xedoc_home.path(), &profile_name),
        xedoc_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn xedoc_home(&self) -> AbsolutePathBuf {
        self.xedoc_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let workspace = xedoc_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, xedoc_home.abs()).await?;
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let missing_path = xedoc_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let role_config_path = xedoc_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = xedoc_config::ConfigLayerStack::new(
        vec![xedoc_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: xedoc_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        xedoc_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".xedoc").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let researcher_path = xedoc_home.path().join("agents").join("researcher.toml");
    let reviewer_path = xedoc_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = xedoc_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let catalog_path = xedoc_home.path().join("catalog.json");
...
"#;
    ...
    let xedoc_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        xedoc_home: xedoc_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: xedoc_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let xedoc_home = fixture.xedoc_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = xedoc_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![xedoc_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = xedoc_config::RequirementSource::Unknown;
    ...
    let requirements = xedoc_config::ConfigRequirements {
        web_search_mode: xedoc_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        xedoc_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/xedoc4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/xedoc3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/xedoc2");
...
[projects."/Users/mbolin/code/xedoc4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/xedoc3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let xedoc_home = temp_dir.path();
    let config_path = xedoc_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(xedoc_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(xedoc_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(xedoc_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(xedoc_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let cwd = xedoc_home.path().join("workspace");
    let outside_root = xedoc_home.path().join("outside-write");
...
export_dir = "locks"
allow_xedoc_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_xedoc_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let xedoc_home = TempDir::new()?;
    let lock_path = xedoc_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
xedoc_version = "older-version"
...
max_loaded_threads_per_session = 12
...
"#,
    ...
    assert_eq!(config.multi_agent_v2.max_loaded_threads_per_session, 12);
