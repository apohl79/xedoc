...
fn status_line_command_edit_clears_command() {
    ...
    .expect("persist");
    apply_blocking(xedoc_home, &[status_line_command_edit(None)]).expect("persist");
...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(xedoc_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = xedoc_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let xedoc_home = tmp.path();
    let link_a = xedoc_home.join("a.toml");
    let link_b = xedoc_home.join("b.toml");
    let config_path = xedoc_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(xedoc_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(xedoc_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_xedoc_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let xedoc_home = tmp.path();
    std::fs::write(
        xedoc_home.join(CONFIG_TOML_FILE),
...
client_id = \"eci-prd-pub-xedoc-123\"
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(xedoc_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !xedoc_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(xedoc_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(xedoc_home.join(CONFIG_TOML_FILE)).expect("read config");
