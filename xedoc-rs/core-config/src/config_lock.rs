...
pub struct ConfigLockReplayOptions {
    pub allow_xedoc_version_mismatch: bool,
}
...
pub fn config_lockfile(config: ConfigToml) -> ConfigLockfileToml {
    ConfigLockfileToml {
        ...
        xedoc_version: env!("CARGO_PKG_VERSION").to_string(),
        config,
    }
}
...
pub fn validate_config_lock_replay(
    ...
    if !options.allow_xedoc_version_mismatch
        && expected_lock.xedoc_version != actual_lock.xedoc_version
    {
        return Err(config_lock_error(format!(
            "config lock Xedoc version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_xedoc_version_mismatch=true to ignore this",
            expected_lock.xedoc_version, actual_lock.xedoc_version
        )));
    }
