//! Defines the protocol for a Xedoc session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Xedoc:";
...
pub enum Op {
    ...
    /// Request Xedoc to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down xedoc instance.
    Shutdown,
...
/// running the command proposed by Xedoc.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.xedoc`, `.git`, notably `.git/hooks`) under
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.xedoc_error_info
            .as_ref()
            .is_none_or(XedocErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Xedoc),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Xedoc
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(xedoc_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                xedoc_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
pub enum Product {
    #[serde(alias = "CHATGPT")]
    Chatgpt,
    // OpenAI's product identifier stays "codex" on the wire.
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_xedoc() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("xedoc".to_string()).restriction_product(),
            Some(Product::Xedoc)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Xedoc])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Xedoc]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = xedoc_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            xedoc_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".xedoc")).expect("create .xedoc");
        let canonical_cwd = xedoc_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_xedoc = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".xedoc"))
            .expect("canonical .xedoc");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_xedoc.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_xedoc = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".xedoc"))
            .expect("canonical .xedoc");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_xedoc.to_path_buf(),
