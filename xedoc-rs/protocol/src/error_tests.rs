...
fn server_overloaded_maps_to_protocol() {
    let err = XedocErr::ServerOverloaded;
    assert_eq!(
        err.to_xedoc_protocol_error(),
        XedocErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = XedocErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = XedocErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.xedoc_error_info,
        Some(XedocErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Xedoc (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_xedoc_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Xedoc, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Xedoc, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
