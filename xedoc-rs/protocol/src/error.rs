...
use crate::protocol::XedocErrorInfo;
use chrono::DateTime;
use chrono::Datelike;
use chrono::Local;
use chrono::Utc;
...
use xedoc_async_utils::CancelErr;
use xedoc_utils_string::truncate_middle_chars;
use xedoc_utils_string::truncate_middle_with_token_budget;

pub type Result<T> = std::result::Result<T, XedocErr>;
...
pub enum XedocErr {
    ...
    #[error(
        "Xedoc ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `XedocError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use Xedoc with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("xedoc-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for XedocErr {
    fn from(_: CancelErr) -> Self {
        XedocErr::TurnAborted
    }
}
...
impl XedocErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            XedocErr::TurnAborted
            | XedocErr::SessionBudgetExceeded
            | XedocErr::RepeatedOutputLimit
            | XedocErr::Interrupted
            | XedocErr::EnvVar(_)
            | XedocErr::Fatal(_)
            | XedocErr::UsageNotIncluded
            | XedocErr::QuotaExceeded
            | XedocErr::InvalidImageRequest()
            | XedocErr::InvalidRequest(_)
            | XedocErr::RefreshTokenFailed(_)
            | XedocErr::UnsupportedOperation(_)
            | XedocErr::Sandbox(_)
            | XedocErr::LandlockSandboxExecutableNotProvided
            | XedocErr::RetryLimit(_)
            | XedocErr::ContextWindowExceeded
            | XedocErr::ThreadNotFound(_)
            | XedocErr::AgentLimitReached { .. }
            | XedocErr::Spawn
            | XedocErr::SessionConfiguredNotFirstEvent
            | XedocErr::UsageLimitReached(_)
            | XedocErr::ServerOverloaded
            | XedocErr::CyberPolicy { .. } => false,
            XedocErr::Stream(..)
            | XedocErr::Timeout
            | XedocErr::RequestTimeout
            | XedocErr::UnexpectedStatus(_)
            | XedocErr::ResponseStreamFailed(_)
            | XedocErr::ConnectionFailed(_)
            | XedocErr::InternalServerError
            | XedocErr::InternalAgentDied
            | XedocErr::Io(_)
            | XedocErr::Json(_)
            | XedocErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            XedocErr::LandlockRuleset(_) | XedocErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<XedocErr>()` checks continue to compile
    ...
    pub fn to_xedoc_protocol_error(&self) -> XedocErrorInfo {
        match self {
            XedocErr::ContextWindowExceeded => XedocErrorInfo::ContextWindowExceeded,
            XedocErr::SessionBudgetExceeded => XedocErrorInfo::SessionBudgetExceeded,
            XedocErr::UsageLimitReached(_)
            | XedocErr::QuotaExceeded
            | XedocErr::UsageNotIncluded => XedocErrorInfo::UsageLimitExceeded,
            XedocErr::ServerOverloaded => XedocErrorInfo::ServerOverloaded,
            XedocErr::CyberPolicy { .. } => XedocErrorInfo::CyberPolicy,
            XedocErr::RetryLimit(_) => XedocErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            XedocErr::ConnectionFailed(_) => XedocErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            XedocErr::ResponseStreamFailed(_) => XedocErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            XedocErr::RefreshTokenFailed(_) => XedocErrorInfo::Unauthorized,
            XedocErr::SessionConfiguredNotFirstEvent
            | XedocErr::InternalServerError
            | XedocErr::InternalAgentDied => XedocErrorInfo::InternalServerError,
            XedocErr::UnsupportedOperation(_)
            | XedocErr::ThreadNotFound(_)
            | XedocErr::AgentLimitReached { .. } => XedocErrorInfo::BadRequest,
            XedocErr::Sandbox(_) => XedocErrorInfo::SandboxError,
            _ => XedocErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            xedoc_error_info: Some(self.to_xedoc_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            XedocErr::RetryLimit(err) => Some(err.status),
            XedocErr::UnexpectedStatus(err) => Some(err.status),
            XedocErr::ConnectionFailed(err) => err.source.status(),
            XedocErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Xedoc (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
...
pub fn get_error_message_ui(e: &XedocErr) -> String {
    let message = match e {
        XedocErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        XedocErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
