...
use xedoc_core_client::ResponseEvent;
...
impl TurnTimingState {
    pub async fn mark_turn_started(&self, started_at: Instant) -> i64 {
    ...
    pub async fn started_at_unix_secs(&self) -> Option<i64> {
        self.state.lock().await.started_at_unix_secs
    }

    pub async fn completed_at_and_duration_ms(&self) -> (Option<i64>, Option<i64>) {
    ...
    pub async fn time_to_first_token_ms(&self) -> Option<i64> {
        let state = self.state.lock().await;
        state
            .time_to_first_token()
    ...
    pub async fn record_ttft_for_response_event(&self, event: &ResponseEvent) -> Option<Duration> {
    ...
    pub async fn record_ttfm_for_turn_item(&self, item: &TurnItem) -> Option<Duration> {
...
pub fn now_unix_timestamp_ms() -> i64 {
    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                xedoc_protocol::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    xedoc_protocol::models::ReasoningItemContent::ReasoningText { text }
                    | xedoc_protocol::models::ReasoningItemContent::Text { text } => {
                        !text.is_empty()
                    }
                })
            })
        ...
        return Some(
            content
                ...
                .filter_map(|item| match item {
                    xedoc_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                    _ => None,
                })
                .collect(),
        );
