...
- `modelProvider/apiKey/set` and `modelProvider/apiKey/delete` — store or remove an external provider API key through Xedoc's encrypted credential store. Account credentials managed by `account/login/start` remain on that existing API.
- `modelProvider/oauth/start` — start a supported external provider OAuth flow and return the browser authorization URL.
