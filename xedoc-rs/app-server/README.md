...
- `script/read`, `script/unregister`, and `script/respond` — experimental session-script lifecycle methods. `script/read` refreshes the bounded snapshot, `script/unregister` removes the connection’s registration, and `script/respond` resolves a leased `request_user_input` prompt. A registered script can use `turn/start` or `turn/steer` only when `userInput.send` was granted for its registered thread.
