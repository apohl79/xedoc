...
`xedoc app-server` is the interface Xedoc uses to power rich interfaces such as the [Xedoc VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `xedoc app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$XEDOC_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `xedoc app-server proxy`
opens exactly one raw stream connection to `$XEDOC_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `xedoc app-server generate-ts`, or a JSON Schema bundle via `xedoc app-server generate-json-schema`. Each output is specific to the version of Xedoc you used to run the command, so the generated artifacts are guaranteed to match that version.

```
xedoc app-server generate-ts --out DIR
xedoc app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Xedoc:

- **Thread**: A conversation between a user and the Xedoc agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's Xedoc home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
Applications building on top of `xedoc app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new Xedoc integration that is intended for enterprise use, please contact us to get it
...
- `script/register` — experimental; turn an initialized connection into a restricted session-script connection for one loaded root thread. The response contains a bounded current-session snapshot; registered scripts receive only their selected model-output, turn, prompt, and session-update projections rather than the thread’s ordinary notification stream.
- `script/read`, `script/unregister`, `script/respond`, and `script/message` — experimental session-script lifecycle methods. `script/read` refreshes the bounded snapshot, `script/unregister` removes the connection’s registration, `script/respond` resolves a leased prompt, and `script/message` posts an `info`, `warning`, or `error` message to that script’s registered thread. A registered script can use `turn/start` or `turn/steer` only when `userInput.send` was granted for its registered thread. A form `select` field can include `search` with an optional placeholder to request host-rendered filtering for long option lists.
- `script/sessionUpdated`, `script/promptOpened`, `script/promptClosed`, and `script/resyncRequired` — experimental notifications for registered session scripts. Prompt notifications are observational; only the exclusive `prompt.requestUserInput.respond` holder receives a response lease.
- `sessionExtension/list` and `sessionExtension/command/invoke` — experimental; list and invoke the approved plugin-extension slash commands for a loaded root thread. Extensions complete approval and one-shot setup before their commands can start a host-scoped persistent session-script child. The generic `on`, `off`, and `restart` command arguments respectively start, stop, and restart that child for the current root thread.
- `sessionExtension/commandsUpdated` — experimental notification that replaces the approved slash-command set after an extension is activated, disabled, or refreshed following an effective plugin change.
...
- `thread/settings/update` — experimental; queue a partial update to a loaded thread’s next-turn settings without starting a turn or adding transcript items. Omitted fields leave settings unchanged; `serviceTier: null` clears the tier; deprecated `multiAgentMode` is ignored, while Ultra reasoning effort enables proactive multi-agent behavior; `sandboxPolicy` and `permissions` cannot be combined. Experimental `environmentVariables` replaces the in-memory source environment for subsequent tool calls; it is filtered by the thread’s shell-environment policy and is never persisted or echoed in notifications. Returns `{}` when the update is accepted and emits `thread/settings/updated` with the full effective settings only if they actually change. `turn/start` settings overrides emit the same notification when they change the stored settings.
...
- `thread/delete` — hard-delete an active or archived thread and any spawned descendant threads; returns `{}` on success and emits `thread/deleted` for each deleted thread.
 ...
 - `thread/name/set` — set or update a thread’s user-facing name for either a loaded thread or a persisted rollout; returns `{}` on success and emits `thread/name/updated` to initialized, opted-in clients. `thread/name/updated` includes `source: "user" | "generated"` so clients can distinguish manual renames from automatic session-name updates. Thread names are not required to be unique; name lookups resolve to the most recently updated thread.
...
- `turn/start` — add user input to a thread and begin Xedoc generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. `clientUserMessageId` is optional; when supplied, the corresponding `userMessage` item echoes it as `clientId`. Experimental `runtimeWorkspaceRoots` supplies the default roots for newly resolved environment selections. Explicit `environments[].runtimeWorkspaceRoots` override that fallback with environment-native absolute paths. Experimental `environmentVariables` replaces the in-memory source environment for this and subsequent turns; it is filtered by the thread’s shell-environment policy and is never persisted or echoed. Prefer experimental `permissions` profile selection by id for permission overrides; the legacy `sandboxPolicy` field is still accepted but cannot be combined with `permissions`. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode". Deprecated experimental `multiAgentMode` is ignored; Ultra reasoning effort selects proactive behavior.
...
- `process/spawn` — experimental; spawn a standalone process without the Xedoc sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
- `model/list` — list available models (set `includeHidden: true` to include entries with `hidden: true`), including each model's `providerId` for cross-provider selection, model-advertised string reasoning effort options in the catalog's intended progression order, `additionalSpeedTiers`, `serviceTiers`, optional `defaultServiceTier`, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata. Clients should preserve the `supportedReasoningEfforts` array order rather than deriving order from the effort names.
- `modelRouter/activity` — experimental transient notification emitted when local model-router classification starts or finishes. It carries only thread, turn, scope, and lifecycle state; it is not persisted in the transcript.
- `modelRouter/decision` — experimental notification emitted for each local router decision. It includes the bounded four-axis classification, ranking score/range, route, policy, and prompt-hash metadata only; it never includes prompt or output text.
- `item/modelRouter/requestApproval` — experimental server request emitted when the configured model-router approval mode requires a decision. It provides all four predicted routing classifications, their permitted values, and the bounded rating/ranking metadata used to derive the eligible route. Respond with `approve`, `reject` to keep the current route, or `override` with any combination of the four classifications; Xedoc derives the eligible route. Overrides feed the selected classifications back into the local embedding classifier.
...
- `modelRouter/settings/open` and `modelRouter/settings/respond` — experimental script-owned router settings surfaces. The Jev setup form submits its API key only through the optional `jevApiKey` request field; clients must not put it in the generic interaction `response.values`.
...
- `modelManager/update` — persist provider defaults or one model's context, compaction, and base-instruction settings to the model registry under the server's Xedoc home.
- `modelProvider/apiKey/set` and `modelProvider/apiKey/delete` — store or remove an external provider API key through Xedoc's encrypted credential store. Setting an Anthropic API key removes its OAuth credentials. Account credentials managed by `account/login/start` remain on that existing API.
- `modelProvider/oauth/start` and `modelProvider/oauth/delete` — start or remove a supported external provider OAuth login. Completing Anthropic OAuth removes its API key so the two authentication modes remain exclusive.
...
- `plugin/list` — list discovered plugin marketplaces and plugin state, including effective marketplace install/auth policy metadata, the locally materialized `localVersion` when available, and fail-open `marketplaceLoadErrors` entries for marketplace files that could not be parsed or loaded. `interface.category` uses the marketplace category when present; otherwise it falls back to the plugin manifest category (**under development; do not call from production clients yet**).
- `plugin/installed` — list installed plugin rows plus any explicitly requested local install-suggestion plugin names. Mention surfaces can use this narrower view when they need plugin mention payloads rather than plugin-page discovery data (**under development; do not call from production clients yet**).
- `plugin/read` — read one plugin by `marketplacePath` plus `pluginName`, returning marketplace info, a list-style `summary`, manifest descriptions/interface metadata, and bundled skills/hooks/MCP server names. Returned plugin skills include their current `enabled` state after local config filtering; bundled hooks are returned as lightweight declaration summaries keyed for correlation with `hooks/list`. (**under development; do not call from production clients yet**).
- `skills/changed` — notification emitted when watched local skill files change.
- `skills/config/write` — write user-level skill config by name or absolute path.
- `plugin/install` — install a plugin from a discovered marketplace entry, rejecting marketplace entries marked unavailable for install, install MCPs if any, and return the effective plugin auth policy (**under development; do not call from production clients yet**).
- `plugin/uninstall` — uninstall a local plugin by `pluginId` in `<plugin>@<marketplace>` form by removing its cached files and clearing its user-level config entry (**under development; do not call from production clients yet**).
...
Start a fresh thread when you need a new Xedoc conversation.
...
To continue a stored session, call `thread/resume` with the `thread.id` you previously recorded. The response shape matches `thread/start`. When the stored session includes persisted token usage, the server emits `thread/tokenUsage/updated` immediately after the response so clients can render restored usage before the next turn starts. You can also pass the same configuration overrides supported by `thread/start`.
...
Turns attach user input (text, images, or audio) to a thread and trigger Xedoc generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.xedoc/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
While the steered message is still pending, cancel it with the same client message id:

```json
{ "method": "turn/steer/cancel", "id": 33, "params": {
    "threadId": "thr_123",
    "clientUserMessageId": "client_msg_124",
    "expectedTurnId": "turn_456"
} }
{ "id": 33, "result": { "status": "canceled" } }
```

### Example: Request a code review

Use `review/start` to run Xedoc’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Xedoc will run).
...
For an inline review, Xedoc streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set the legacy `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). Xedoc will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `processId` is optional for buffered execution. When omitted, Xedoc generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the Xedoc sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `rawResponseItem/completed` — internal-only; when `thread/start.experimentalRawEvents` is enabled, emits each completed provider response item. Native providers may attach `provider_metadata` to tool-call items; clients that store and later submit raw history must preserve this field unchanged.
...
- `subAgentActivity` — `{id, kind, agentThreadId, agentPath, modelProvider?, model?, reasoningEffort?, currentActivity?}` describes a spawned agent’s lifecycle and latest activity. `reasoningEffort` is the effective effort reported when the agent starts and remains absent when unavailable.
...
- `contextCompaction` — `{id}` emitted when xedoc compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when xedoc compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Xedoc whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`xedoc_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
...
Each function may set `deferLoading`. When omitted, it defaults to `false`. Deferred functions must belong to a namespace. Set it to `true` to keep the function registered while excluding it from the model-facing tool list sent on ordinary turns. When `tool_search` is available, deferred dynamic tools are searchable and can be exposed by a matching search result.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.xedoc/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Xedoc skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Xedoc skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.xedoc/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.xedoc/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.xedoc/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.xedoc/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.xedoc/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
Xedoc supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Xedoc owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Xedoc persists tokens to disk and refreshes them automatically.
- **Xedoc managed Amazon Bedrock auth (`amazonBedrock`, experimental)**: Caller supplies an Amazon Bedrock API key and region via `account/login/start` with `type: "amazonBedrock"`. The client must enable the `experimentalApi` initialization capability for Xedoc-managed Amazon Bedrock login. Xedoc replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: Xedoc uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `xedoc login --with-access-token` or `XEDOC_ACCESS_TOKEN`.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Xedoc can run without OpenAI credentials.
- Amazon Bedrock reports `usesCodexManagedCredentials: true` when it uses a Bedrock API key managed by Xedoc. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether Xedoc-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted Xedoc success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"xedoc"` or `"chatgpt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"xedoc"`.
...
When using a Xedoc-managed Bedrock key, logout removes the key and clears `model_provider` if it is still set to `"amazon-bedrock"`. When using AWS-managed credentials, manage them through AWS or switch providers before logging out.
...
`xedoc app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
xedoc app-server generate-ts --out DIR --experimental
xedoc app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p xedoc-app-server-protocol
   ```
