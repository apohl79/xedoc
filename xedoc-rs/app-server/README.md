...
- `item/modelRouter/requestApproval` — experimental server request emitted when the configured model-router approval mode requires a decision. It provides all four predicted routing classifications, their permitted values, and eligible model routes. Respond with `approve`, `reject` to keep the current route, or `override` with any combination of the four classifications and an eligible route. Overrides feed the selected classifications back into the local embedding classifier.
...
- `modelRouterPolicy/read` — experimental read of the user-editable policy controls. It exposes confidence thresholds and the ranked model/effort ladder, never classifier weights or prompts.
...
- `modelRouterPolicy/write` — experimental update of confidence thresholds and the ranked model/effort ladder. It preserves classifier weights and validates the submitted ranking.
