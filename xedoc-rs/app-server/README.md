...
- `modelRouterPolicy/read` — experimental read of the user-editable policy controls. It exposes confidence thresholds, the ranked model/effort ladder, and the optional reporting baseline selected from that ladder; it never exposes classifier weights or prompts.
- `modelRouterPolicy/bootstrap` — experimental first-run bootstrap. It installs the packaged ranking policy, preserving schema-v1 policy files as sibling backups before migration and leaving other existing policy files unchanged.
- `modelRouterPolicy/write` — experimental update of confidence thresholds, the ranked model/effort ladder, and its optional reporting baseline. It preserves classifier weights and validates that the baseline is a ladder route.
