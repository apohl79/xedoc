...
- `sessionExtension/list` and `sessionExtension/command/invoke` — experimental; list and invoke the approved plugin-extension slash commands for a loaded root thread. Extensions complete an approval and one-shot setup flow before Xedoc starts their host-scoped persistent session-script child.
