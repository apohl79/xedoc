...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(xedoc_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(xedoc_home);
        command.env("XEDOC_HOME", xedoc_home);
        #[cfg(target_os = "linux")]
        command.env(
            XEDOC_LINUX_SANDBOX_EXE_ENV_VAR,
