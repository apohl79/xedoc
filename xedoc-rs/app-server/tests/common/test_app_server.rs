...
use core_test_support::test_xedoc::TestEnv;
use core_test_support::test_xedoc::test_env;
...
use xedoc_app_server_protocol::FsCopyParams;
use xedoc_app_server_protocol::FsCreateDirectoryParams;
use xedoc_app_server_protocol::FsGetMetadataParams;
use xedoc_app_server_protocol::FsReadDirectoryParams;
use xedoc_app_server_protocol::FsReadFileParams;
use xedoc_app_server_protocol::FsRemoveParams;
use xedoc_app_server_protocol::FsUnwatchParams;
use xedoc_app_server_protocol::FsWatchParams;
use xedoc_app_server_protocol::FsWriteFileParams;
...
use xedoc_app_server_protocol::ProcessKillParams;
use xedoc_app_server_protocol::ProcessSpawnParams;
...
use xedoc_exec_server::XEDOC_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use xedoc_exec_server::XEDOC_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use xedoc_exec_server::XEDOC_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use xedoc_exec_server::XEDOC_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use xedoc_exec_server::XEDOC_EXEC_SERVER_URL_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned XEDOC_HOME that may still be its cwd on Windows.
    _delayed_exec_server: Option<(LocalWebsocketExecServer, WebsocketDelayInterposer)>,
    _owned_xedoc_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "xedoc-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "XEDOC_APP_SERVER_DISABLE_MANAGED_CONFIG";
const CONFIG_OVERRIDE_ARG: &str = "-c";
const DISABLE_AUTO_SESSION_NAME_OVERRIDE: &str = "auto_session_name=false";

impl TestAppServer {
    /// Starts building a server with a temporary XEDOC_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            xedoc_home.join("managed_config.toml"),
        );
        cmd.env_remove(XEDOC_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("xedoc-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: xedoc_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing XEDOC_HOME instead of a temporary one.
    pub fn with_xedoc_home(mut self, xedoc_home: &Path) -> Self {
        self.xedoc_home = Some(xedoc_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary XEDOC_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (xedoc_home, owned_xedoc_home) = match xedoc_home {
            Some(xedoc_home) => (xedoc_home, None),
            None => {
                let owned_xedoc_home = TempDir::new()?;
                (
                    owned_xedoc_home.path().to_path_buf(),
                    Some(owned_xedoc_home),
                )
            }
        };
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = xedoc_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            xedoc_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&xedoc_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        XEDOC_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        XEDOC_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        XEDOC_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (XEDOC_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        XEDOC_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let program = match program {
            ...
            None => xedoc_utils_cargo_bin::cargo_bin("xedoc-app-server")
                .context("should find binary for xedoc-app-server")?,
        };
        ...
        app_server._owned_xedoc_home = owned_xedoc_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `xedoc-app-server` child process.
        //
