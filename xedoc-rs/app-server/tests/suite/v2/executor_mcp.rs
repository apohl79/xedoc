...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let xedoc_bin = toml::Value::String(
        xedoc_utils_cargo_bin::cargo_bin("xedoc")?
            .to_string_lossy()
            .into_owned(),
    );
    let http_proxy = toml::Value::String(format!("http://{http_addr}"));
    std::fs::write(
        xedoc_home.path().join("environments.toml"),
        format!(
            r#"
include_local = true
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".xedoc-plugin"))?;
    std::fs::write(
        plugin.path().join(".xedoc-plugin/plugin.json"),
