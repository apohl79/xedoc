...
use xedoc_app_server_protocol::ChatgptAuthTokensRefreshReason;
use xedoc_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use xedoc_login::AuthDotJson;
use xedoc_login::AuthKeyringBackendKind;
use xedoc_login::CLIENT_ID_OVERRIDE_ENV_VAR;
use xedoc_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use xedoc_protocol::auth::AuthMode as DomainAuthMode;

const DEFAULT_READ_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(60);
const LOGIN_ISSUER_ENV_VAR: &str = "XEDOC_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "XEDOC_APP_SERVER_DEV_OPEN_APP_URL";
...
fn create_config_toml(xedoc_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
fn read_config_toml(xedoc_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        xedoc_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(xedoc_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(xedoc_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(xedoc_home.path().join("auth.json").exists());
    ...
    assert!(
        !xedoc_home.path().join("auth.json").exists(),
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    std::fs::write(xedoc_home.path().join("config.toml"), "invalid = [")?;
    ...
    assert_eq!(load_file_auth(xedoc_home.path())?, None);
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(xedoc_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<xedoc_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(xedoc_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![xedoc_app_server_protocol::UserInput::Text {
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    let mut expected_config = read_config_toml(xedoc_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(
        load_file_auth(xedoc_home.path())?,
        Some(AuthDotJson {
    ...
    assert_eq!(read_config_toml(xedoc_home.path())?, expected_config);
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(xedoc_home.path())?;
    let expected_config = read_config_toml(xedoc_home.path())?;
    ...
    assert_eq!(load_file_auth(xedoc_home.path())?, expected_auth);
    assert_eq!(read_config_toml(xedoc_home.path())?, expected_config);
    ...
    assert_eq!(
        ...
        GetAccountResponse {
            account: Some(Account::AmazonBedrock {
                uses_xedoc_managed_credentials: true,
            }),
...
async fn logout_aws_managed_bedrock_errors_without_changing_auth_or_config() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), aws_managed_bedrock_config())?;
    login_with_api_key(
        xedoc_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    ...
    assert_eq!(
        error.error.message,
        "cannot log out while Amazon Bedrock is using AWS-managed credentials; manage those credentials through AWS or switch model providers before logging out Xedoc authentication"
    );
    assert_eq!(load_file_auth(xedoc_home.path())?, expected_auth);
    assert_eq!(read_config_toml(xedoc_home.path())?, expected_config);
    Ok(())
...
"#
    ...
    let expected = GetAccountResponse {
        account: Some(Account::AmazonBedrock {
            uses_xedoc_managed_credentials: false,
        }),
        requires_openai_auth: false,
    };
    assert_eq!(received, expected);
