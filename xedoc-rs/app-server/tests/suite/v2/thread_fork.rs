...
use xedoc_rollout::append_thread_name;
use xedoc_rollout::read_session_meta_line;
use xedoc_state::StateRuntime;
use xedoc_state::ThreadTitleSource;
...
async fn thread_fork_defers_inherited_active_goal_until_next_turn() -> Result<()> {
    ...
    assert_eq!(
        forked_goal.status,
        xedoc_state::ThreadGoalStatus::BudgetLimited
    );
    assert_eq!(
        state_db
            .thread_goals()
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(xedoc_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(xedoc_home.path(), filename_ts, &conversation_id);
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_xedoc_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(xedoc_home_path.clone()),
            ..Default::default()
        })
        .await?;
