...
//! the temporary `xedoc_home` must not contain rollout session files or sqlite
...
use xedoc_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    create_config_toml_with_thread_store(xedoc_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let xedoc_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under xedoc_home. Disable them
    ...
    let mut client = start_in_process_server(xedoc_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(xedoc_home.path())?;
...
async fn start_in_process_server(xedoc_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .xedoc_home(xedoc_home.to_path_buf())
            .fallback_cwd(Some(xedoc_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(xedoc_home: &Path) -> Result<()> {
    ...
    let mut entries = xedoc_home_entries(xedoc_home)?;
    // Bazel test runs may initialize shell snapshot storage under xedoc_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in xedoc_home"
    );

    Ok(())
}

fn xedoc_home_entries(xedoc_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(xedoc_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
