...
use xedoc_app_server_protocol::DynamicToolCallStatus;
use xedoc_app_server_protocol::DynamicToolFunctionSpec;
use xedoc_app_server_protocol::DynamicToolNamespaceSpec;
...
async fn start_function_dynamic_tool_call(call_id: &str) -> Result<PendingDynamicToolCall> {
    ...
    let mut model_info =
        xedoc_core::test_support::construct_model_info_offline("mock-model", &config);
    model_info.input_modalities.push(InputModality::Audio);
    write_models_cache_with_models(xedoc_home.path(), vec![model_info])?;
