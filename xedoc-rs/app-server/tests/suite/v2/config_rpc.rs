...
use xedoc_app_server_protocol::AppConfig;
use xedoc_app_server_protocol::AppToolApproval;
use xedoc_app_server_protocol::AppsConfig;
use xedoc_app_server_protocol::AppsDefaultConfig;
...
use xedoc_app_server_protocol::ConfigLayerSource;
...
use xedoc_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use xedoc_app_server_protocol::ToolsV2;
...
use xedoc_protocol::config_types::WebSearchLocation;
use xedoc_protocol::config_types::WebSearchToolConfig;
...
fn write_config(xedoc_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let xedoc_home_path = xedoc_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(xedoc_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("xedoc")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    write_config(&xedoc_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_xedoc_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let xedoc_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", xedoc_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(xedoc_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let xedoc_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(xedoc_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(xedoc_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[xedoc_app_server_protocol::ConfigLayer],
...
fn first_non_mdm_layer_index(layers: &[xedoc_app_server_protocol::ConfigLayer]) -> usize {
    if matches!(
