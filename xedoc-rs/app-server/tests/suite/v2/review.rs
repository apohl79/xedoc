...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml_with_approval_policy(xedoc_home.path(), &server.uri(), "untrusted")?;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": xedoc_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri())?;
    let colliding_skill_dir = xedoc_home.path().join("skills/review-agent-collision");
    ...
    let canonical_xedoc_home = std::fs::canonicalize(xedoc_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_xedoc_home)
        .join("review-agent")
        .join("SKILL.md");
...
fn create_config_toml(xedoc_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    create_config_toml_with_approval_policy(xedoc_home, server_uri, "never")
}
