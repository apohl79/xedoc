...
use xedoc_app_server_protocol::McpToolCallAppContext;
...
use xedoc_core::ARCHIVED_SESSIONS_SUBDIR;
...
use xedoc_rollout::append_rollout_item_to_path;
use xedoc_rollout::read_session_meta_line;
use xedoc_state::StateRuntime;
...
const XEDOC_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Xedoc, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
#[tokio::test]
async fn thread_resume_preserves_acknowledged_model_effort_update() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri())?;
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let xedoc_home = TempDir::new()?;
    let xedoc_home_path = normalized_existing_path(xedoc_home.path())?;
    create_config_toml(&xedoc_home_path, &server.uri())?;
    let config_path = xedoc_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = xedoc_home_path
        .as_path()
        .to_str()
        .expect("test xedoc home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(&xedoc_home_path)
        .without_managed_config()
        .with_env_overrides(&[("XEDOC_SQLITE_HOME", Some(sqlite_home))])
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in ["xedoc_chatgpt_android_remote", "xedoc_chatgpt_ios_remote"] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(xedoc_home, filename_ts, conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(xedoc_home.path(), filename_ts, &conversation_id);
    let archived_dir = xedoc_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "xedoc unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(xedoc_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            xedoc_state::GoalAccountingMode::ActiveOnly,
...
#[tokio::test]
async fn thread_goal_clear_deletes_goal() -> Result<()> {
    ...
    .await;
    let xedoc_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(xedoc_home.path(), &server.uri(), &server.uri())?;
    let config_path = xedoc_home.path().join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
    ...
    .await??;
    let get: xedoc_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let xedoc_home = TempDir::new()?;
    let config_toml = xedoc_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = xedoc_home.path().join("repo");
    ...
    let rollout_path = rollout_path(xedoc_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let xedoc_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(xedoc_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(xedoc_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
    ...
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(xedoc_home.path())?);
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(xedoc_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&xedoc_home, &server.uri())?;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![xedoc_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(xedoc_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(xedoc_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(xedoc_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(xedoc_home.path(), "seed history").await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(XEDOC_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(xedoc_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
command = "xedoc-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
async fn setup_rollout_fixture(xedoc_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(xedoc_home, server_uri)?;
