...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_xedoc_home = AbsolutePathBuf::try_from(xedoc_home.path().canonicalize()?)?;
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "xedoc_app_server_daemon".to_string(),
            title: Some("Xedoc App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
#[tokio::test]
async fn initialize_xedoc_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "xedoc-backend".to_string(),
            title: Some("Xedoc Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[(
            "XEDOC_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("xedoc_originator_via_env_var"),
        )])
        .build()
        .await?;
    ...
    assert!(user_agent.starts_with("xedoc_originator_via_env_var/"));
    assert_eq!(response_xedoc_home, expected_xedoc_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(xedoc_home.path())
        .without_auto_env()
        .with_env_overrides(&[("XEDOC_INTERNAL_ORIGINATOR_OVERRIDE", None)])
        .build()
        .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let xedoc_home = TempDir::new()?;
    let notify_file = xedoc_home.path().join("notify.json");
    let notify_capture = cargo_bin("xedoc-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(xedoc_home, server_uri, approval_policy, "")
}
