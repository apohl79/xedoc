...
async fn plugin_read_returns_canonical_openai_curated_marketplace_name() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.xedoc-plugin"))?;
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.xedoc-plugin/plugin.json"),
        r#"{
  "name": "demo-plugin",
...
"#,
    ...
    write_installed_plugin(&xedoc_home, "openai-curated", "demo-plugin")?;
...
[plugins."demo-plugin@xedoc-curated"]
enabled = true

[hooks.state."demo-plugin@xedoc-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&xedoc_home, "xedoc-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "xedoc-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@xedoc-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![
            xedoc_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@xedoc-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            xedoc_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@xedoc-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            xedoc_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@xedoc-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&xedoc_home)?;
...
fn write_plugins_enabled_config(xedoc_home: &TempDir) -> Result<()> {
    std::fs::write(
        xedoc_home.path().join("config.toml"),
        r#"[features]
plugins = true
