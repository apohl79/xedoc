...
use xedoc_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use xedoc_app_server::INVALID_PARAMS_ERROR_CODE;
use xedoc_app_server_protocol::AdditionalContextEntry;
use xedoc_app_server_protocol::AdditionalContextKind;
use xedoc_app_server_protocol::ByteRange;
use xedoc_app_server_protocol::ClientInfo;
use xedoc_app_server_protocol::CollabAgentStatus;
...
use xedoc_app_server_protocol::FileChangePatchUpdatedNotification;
...
use xedoc_app_server_protocol::JSONRPCError;
...
use xedoc_app_server_protocol::PatchApplyStatus;
...
use xedoc_app_server_protocol::SubAgentActivityKind;
use xedoc_app_server_protocol::TextElement;
...
use xedoc_core::test_support::all_model_presets;
...
use xedoc_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Xedoc turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = xedoc_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Xedoc VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(xedoc_home.path())?;
    let cache_path = xedoc_home.path().join("models_cache.json");
    ...
    let config_path = xedoc_home.path().join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("model = \"mock-model\"", &format!("model = \"{model}\"")),
    )?;
    write_test_skill(xedoc_home.path(), "alpha-skill")?;
    write_test_skill(xedoc_home.path(), "beta-skill")?;

    let isolated_home = xedoc_home.path().to_string_lossy();
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(xedoc_home.path())
        .with_env_overrides(&[
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: xedoc_app_server_protocol::ServerNotification = notification.try_into()?;
    let xedoc_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
...
async fn turn_start_accepts_managed_network_profile_from_requirements() -> Result<()> {
    ...
    std::fs::write(
        xedoc_home.path().join("requirements.toml"),
        r#"
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: xedoc_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Xedoc turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        xedoc_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            model: Some("exp-xedoc-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let xedoc_home = tmp.path().to_path_buf();
    ...
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(xedoc_home.as_path())
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(xedoc_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(xedoc_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let xedoc_home = tmp.path().join("xedoc_home");
    std::fs::create_dir(&xedoc_home)?;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            runtime_workspace_roots: None,
            approval_policy: Some(xedoc_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(xedoc_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = xedoc_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = xedoc_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&xedoc_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        xedoc_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![xedoc_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&xedoc_home)?;
    let cache_path = xedoc_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        xedoc_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(xedoc_home.path().join("config.toml"))?;
...
fn create_config_toml_with_sandbox(
    ...
    let feature_entries = features
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let config_toml = xedoc_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
...
fn write_test_skill(xedoc_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = xedoc_home.join("skills").join(name);
