...
use xedoc_app_server_protocol::TextPosition;
use xedoc_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = xedoc_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml_without_approval_policy(xedoc_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(xedoc_home.path())
        .build()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml_without_approval_policy(xedoc_home.path(), &server.uri())?;
    let global_agents_path = xedoc_home.path().join("AGENTS.md");
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(xedoc_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(xedoc_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(xedoc_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(xedoc_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml_with_optional_approval_policy(
        xedoc_home, server_uri, /*approval_policy*/ None,
    )
}
