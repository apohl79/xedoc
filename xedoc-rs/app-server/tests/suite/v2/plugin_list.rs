...
use xedoc_app_server_protocol::PluginAuthPolicy;
use xedoc_app_server_protocol::PluginInstallPolicy;
...
use xedoc_core::config::set_project_trust_level;
...
fn write_plugins_enabled_config(xedoc_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(xedoc_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = xedoc_home.path().to_string_lossy().into_owned();
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&xedoc_home, "openai-curated", "linear")?;
    std::fs::write(
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.xedoc-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.xedoc-plugin/plugin.json"),
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    install_policy: PluginInstallPolicy::Available,
                    auth_policy: PluginAuthPolicy::OnInstall,
                    interface: Some(xedoc_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    std::fs::create_dir_all(xedoc_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(xedoc_home.path())?;
    std::fs::write(
        xedoc_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "xedoc-curated",
  "plugins": [
    {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&xedoc_home, "xedoc-curated", "enabled-plugin")?;
    write_installed_plugin(&xedoc_home, "xedoc-curated", "disabled-plugin")?;
    std::fs::write(
...
[plugins."enabled-plugin@xedoc-curated"]
enabled = true

[plugins."disabled-plugin@xedoc-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "xedoc-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@xedoc-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@xedoc-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@xedoc-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    std::fs::create_dir_all(xedoc_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&xedoc_home, "xedoc-curated", "shared-plugin")?;
    std::fs::write(
        xedoc_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "xedoc-curated",
...
[plugins."shared-plugin@xedoc-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".xedoc"))?;
    std::fs::write(
        workspace_enabled.path().join(".xedoc/config.toml"),
        r#"[plugins."shared-plugin@xedoc-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@xedoc-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@xedoc-curated");
...
#[tokio::test]
async fn plugin_list_includes_api_curated_marketplace_for_api_auth() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    write_plugins_enabled_config(xedoc_home.path())?;
    write_openai_api_curated_marketplace(xedoc_home.path(), &["api-plugin"])?;
    login_with_api_key(
        xedoc_home.path(),
        "sk-test-key",
...
#[tokio::test]
async fn plugin_list_returns_no_marketplaces_when_plugins_disabled() -> Result<()> {
    let xedoc_home = TempDir::new()?;
    std::fs::write(
        xedoc_home.path().join("config.toml"),
        r#"
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = xedoc_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".xedoc-plugin"))?;
    std::fs::write(
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(xedoc_home, marketplace_name, plugin_name, "local")
}
...
fn write_curated_marketplace(
    ...
    let curated_root = xedoc_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.xedoc-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(xedoc_home.join(".tmp"))?;
    std::fs::write(
        xedoc_home.join(".tmp/plugins.sha"),
