...
use xedoc_app_server_protocol::CommandExecOutputDeltaNotification;
use xedoc_app_server_protocol::CommandExecOutputStream;
use xedoc_app_server_protocol::CommandExecParams;
use xedoc_app_server_protocol::CommandExecResizeParams;
use xedoc_app_server_protocol::CommandExecResponse;
use xedoc_app_server_protocol::CommandExecTerminalSize;
...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(xedoc_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$XEDOC_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", xedoc_home.path().display()),
            stderr: String::new(),
        }
    );
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${XEDOC_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(xedoc_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_xedoc_home(xedoc_home.path())
        .without_auto_env()
        .build()
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let xedoc_home = TempDir::new()?;
    let command_dir = xedoc_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !xedoc_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(xedoc_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "xedoc-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(xedoc_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(xedoc_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(xedoc_home: &Path, inserted_config: &str) -> Result<()> {
    let config_path = xedoc_home.join("config.toml");
