...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn get_conversation_summary_by_relative_rollout_path_resolves_from_xedoc_home() -> Result<()>
{
    let xedoc_home = TempDir::new()?;
    let conversation_id = create_fake_rollout(
        xedoc_home.path(),
    ...
    let relative_path = rollout_path.strip_prefix(xedoc_home.path())?.to_path_buf();
