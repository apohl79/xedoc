...
const MID_TURN_AUTO_SESSION_NAME_MAX_CHARS: usize = 6_000;
...
pub(crate) struct PendingThreadResumeRequest {
    ...
    pub(crate) thread_summary: xedoc_app_server_protocol::Thread,
    ...
    pub(crate) initial_turns_page:
        Option<xedoc_app_server_protocol::ThreadResumeInitialTurnsPageParams>,
    pub(crate) paginated_turns: Option<Vec<Turn>>,
    pub(crate) paginated_initial_turns_page: Option<xedoc_app_server_protocol::TurnsPage>,
    pub(crate) paginated_initial_turns_page_with_active_slot:
        Option<xedoc_app_server_protocol::TurnsPage>,
    pub(crate) resume_cursor_store: Option<Arc<dyn xedoc_thread_store::ThreadStore>>,
...
pub(crate) struct ThreadState {
    ...
    listener_thread: Option<Weak<XedocThread>>,
    watch_registration: WatchRegistration,
}

impl ThreadState {
    pub(crate) fn listener_matches(&self, conversation: &Arc<XedocThread>) -> bool {
    ...
    pub(crate) fn track_current_turn_event(&mut self, event_turn_id: &str, event: &EventMsg) {
        if let EventMsg::TurnStarted(payload) = event {
            ...
            self.mid_turn_auto_session_name
                .start_turn(payload.turn_id.clone());
        }
...
impl MidTurnAutoSessionNameState {
    fn note_delta(
        ...
        if event.delta.trim().is_empty() || self.requested {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn mid_turn_auto_session_name_request_fires_once_after_first_non_empty_delta() {
        let mut state = ThreadState::default();

        let first_request = state.note_agent_message_delta_for_auto_session_name(
            &agent_message_delta("turn-1", "msg-1", "   "),
        );
        ...
        let request = state
            ...
            .expect("first non-empty delta should request a generated session name");
        assert_eq!(request.turn_id, "turn-1");
        assert_eq!(request.completed_turn_count, 0);
        assert_eq!(request.partial_response, "b");
