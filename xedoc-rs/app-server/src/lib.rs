...
use xedoc_app_server_protocol::TextPosition as AppTextPosition;
use xedoc_app_server_protocol::TextRange as AppTextRange;
use xedoc_config::ConfigLayerSource;
use xedoc_config::ConfigLoadError;
use xedoc_config::TextRange as CoreTextRange;
use xedoc_core::ExecPolicyError;
...
use xedoc_rollout::state_db as rollout_state_db;
use xedoc_state::log_db;

const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Xedoc rebuilt its local database.";
...
pub(crate) use xedoc_app_server_jsonrpc::connection_rpc_gate;
pub(crate) use xedoc_app_server_jsonrpc::error_code;
pub(crate) use xedoc_app_server_jsonrpc::outgoing_message;
pub(crate) use xedoc_app_server_jsonrpc::request_serialization;
pub(crate) use xedoc_app_server_jsonrpc::server_request_error;
...
const OTEL_SERVICE_NAME: &str = "xedoc-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "XEDOC_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_xedoc_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_xedoc_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let otel = xedoc_core::otel_init::build_provider(
        &config,
    ...
    xedoc_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    xedoc_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&xedoc_home)?;
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !xedoc_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Xedoc local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = xedoc_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Xedoc local database file {} to {}",
...
fn loader_overrides_with_test_user_config_file(
    ...
    apply_debug_test_user_config(loader_overrides, test_user_config_file)
}

#[cfg(debug_assertions)]
fn apply_debug_test_user_config(
    ...
    if let Some(path) = test_user_config_file {
        ...
        overrides.user_config_path = Some(path);
    }
    Ok(overrides)
}
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("xedoc-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(
