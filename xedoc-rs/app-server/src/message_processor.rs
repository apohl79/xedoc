...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = xedoc_core::thread_store_from_config(config.as_ref(), state_db.clone());
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                xedoc_core::build_models_manager(config.as_ref(), auth_manager.clone()),
                session_source,
                environment_manager,
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let xedoc_request = deserialize_client_request(&request);
                let result = match xedoc_request {
                    Ok(xedoc_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = xedoc_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = xedoc_request.experimental_reason()
        ...
        let serialization_scope = xedoc_request.serialization_scope();
