...
use xedoc_utils_json_to_toml::json_to_toml;

/// Shared app-server entry point for loading effective Xedoc configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn xedoc_home(&self) -> &Path {
        self.xedoc_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.xedoc_home())
    }
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(xedoc_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_xedoc_home(
            self.xedoc_home.clone(),
            self.current_cli_overrides(),
        )
        .await?;
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path(self.xedoc_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = xedoc_core::config::ConfigBuilder::default()
            .xedoc_home(self.xedoc_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.xedoc_self_exe = self.arg0_paths.xedoc_self_exe.clone();
        config.xedoc_linux_sandbox_exe = self.arg0_paths.xedoc_linux_sandbox_exe.clone();
    ...
    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(xedoc_home: PathBuf) -> Self {
        Self::new_for_tests(
            xedoc_home,
            Vec::new(),
