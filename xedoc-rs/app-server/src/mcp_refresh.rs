...
async fn build_refresh_config(
    ...
    let mcp_servers = xedoc_mcp::configured_mcp_servers(&mcp_config);
...
mod tests {
    ...
    use xedoc_config::ThreadConfigContext;
    use xedoc_config::ThreadConfigLoadError;
    use xedoc_config::ThreadConfigLoadErrorCode;
    ...
    use xedoc_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_uses_latest_auth_keyring_backend() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(xedoc_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let executor_skill_provider: Arc<dyn xedoc_skills_extension::SkillProvider> = Arc::new(
            xedoc_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                xedoc_core::build_models_manager(&good_config, auth_manager.clone()),
                SessionSource::Exec,
                Arc::clone(&environment_manager),
                thread_extensions(ThreadExtensionDependencies {
                    ...
                    goal_service: Arc::new(xedoc_goal_extension::GoalService::new()),
                    environment_manager: Arc::clone(&environment_manager),
                    executor_skill_provider: Arc::clone(&executor_skill_provider),
                    thread_store: Arc::clone(&thread_store),
                }),
                Arc::new(XedocHomeUserInstructionsProvider::new(
                    good_config.xedoc_home.clone(),
                )),
                Arc::clone(&thread_store),
                xedoc_core::local_agent_graph_store_from_state_db(Some(&state_db)),
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> xedoc_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
