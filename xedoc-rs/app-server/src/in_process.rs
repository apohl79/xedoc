...
//!
//! # Relationship to `xedoc-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `xedoc-app-server-client`,
...
/// through `xedoc-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_xedoc_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let config_manager = ConfigManager::new(
            args.config.xedoc_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(xedoc_home.path()).await);
        let state_db = xedoc_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "xedoc-in-process-test".to_string(),
                    title: None,
        ...
        client._test_xedoc_home = Some(xedoc_home);
        client
    }
