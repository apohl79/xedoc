...
use xedoc_app_server_protocol::CommandExecResizeResponse;
...
use xedoc_app_server_protocol::CommandExecTerminateResponse;
use xedoc_app_server_protocol::CommandExecWriteParams;
use xedoc_app_server_protocol::CommandExecWriteResponse;
use xedoc_app_server_protocol::JSONRPCErrorError;
use xedoc_app_server_protocol::ServerNotification;
use xedoc_core::config::StartedNetworkProxy;
use xedoc_core::exec::ExecExpiration;
use xedoc_core::exec::ExecExpirationOutcome;
use xedoc_core::exec::IO_DRAIN_TIMEOUT_MS;
use xedoc_core::sandboxing::ExecRequest;
use xedoc_protocol::exec_output::bytes_to_string_smart;
use xedoc_utils_pty::ProcessHandle;
use xedoc_utils_pty::SpawnedProcess;
use xedoc_utils_pty::TerminalSize;
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let spawned = if tty {
            xedoc_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            xedoc_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0, &[])
                .await
        } else {
            xedoc_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: xedoc_app_server_protocol::RequestId::Integer(100),
        };
        ...
        manager
            .start(StartCommandExecParams {
                ...
                exec_request: ExecRequest::new(
                    ...
                    xedoc_core::exec::ExecCapturePolicy::ShellTool,
                    SandboxType::None,
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: xedoc_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: xedoc_app_server_protocol::RequestId::Integer(3),
        };
