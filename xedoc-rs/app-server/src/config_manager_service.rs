...
use xedoc_app_server_protocol::Config as ApiConfig;
...
use xedoc_config::merge_toml_values;
use xedoc_core::config::deserialize_config_toml_with_base;
use xedoc_core::config::edit::ConfigEdit;
use xedoc_core::config::edit::ConfigEditsBuilder;
use xedoc_core::config::validate_feature_requirements_for_config_toml;
use xedoc_core::path_utils;
use xedoc_core::path_utils::SymlinkWritePaths;
use xedoc_core::path_utils::resolve_symlink_write_paths;
use xedoc_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.xedoc_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .xedoc/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_xedoc_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_xedoc_folder.display(),
        ),
