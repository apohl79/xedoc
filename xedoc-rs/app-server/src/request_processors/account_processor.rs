...
use xedoc_model_provider::is_supported_amazon_bedrock_region;

// Duration before a browser ChatGPT login attempt is abandoned.
const LOGIN_CHATGPT_TIMEOUT: Duration = Duration::from_secs(10 * 60);
// Login overrides are intentionally available only in debug builds.
#[cfg(debug_assertions)]
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "XEDOC_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "XEDOC_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(XedocAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(XedocAuth::account_plan_type),
        }
    }
    ...
    fn maybe_refresh_plugin_caches_for_current_config(
        thread_manager: &ThreadManager,
        auth: Option<XedocAuth>,
    ) {
        thread_manager
            .plugins_manager()
            .set_auth_mode(auth.as_ref().map(XedocAuth::api_auth_mode));
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Xedoc => LoginSuccessPageBrand::Xedoc,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: XEDOC_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Xedoc open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
    ...
    async fn login_chatgpt_common(
        ...
        let opts = {
                ...
                && !open_app_url.trim().is_empty()
            {
                *url = open_app_url
                    .parse()
                    .map_err(|err| internal_error(format!("invalid Xedoc open app URL: {err}")))?;
            }
            opts
        };
    ...
    async fn login_chatgpt_auth_tokens_response(
        ...
        let auth = XedocAuth::from_external_chatgpt_tokens(
            &access_token,
            &chatgpt_account_id,
    ...
    async fn send_chatgpt_login_completion_notifications(
        ...
        if success {
            ...
            Self::maybe_refresh_plugin_caches_for_current_config(&thread_manager, auth.clone());
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        let managed_bedrock_auth = matches!(
            self.auth_manager.auth_cached(),
            Some(XedocAuth::BedrockApiKey(_))
        );
