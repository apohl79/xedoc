...
use xedoc_login::default_client::SetOriginatorError;
...
const NON_ORIGINATING_CLIENT_NAMES: &[&str] = &["xedoc_app_server_daemon", "xedoc-backend"];
...
impl InitializeRequestProcessor {
    ...
    pub(crate) async fn initialize(
        ...
        let xedoc_home = self.config.xedoc_home.clone();
        if session
            ...
            if let Err(error) = set_default_originator(originator) {
                match error {
                    ...
                    SetOriginatorError::AlreadyInitialized => {
                        ...
                        // TODO(owen): Once we remove support for XEDOC_INTERNAL_ORIGINATOR_OVERRIDE,
        ...
        let user_agent = get_xedoc_user_agent();
