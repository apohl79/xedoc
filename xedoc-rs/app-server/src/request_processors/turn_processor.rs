use super::*;
use xedoc_agent_extension::AgentInvocation;
use xedoc_agent_extension::AgentRun;
use xedoc_agent_extension::AgentRunner;
use xedoc_protocol::models::ContentItem;
use xedoc_protocol::models::FunctionCallOutputContentItem;
use xedoc_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use xedoc_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
use xedoc_skills::system_cache_root_dir;
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = xedoc_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<xedoc_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(xedoc_app_server_protocol::AskForApproval::to_core);
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != xedoc_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let message = match turn_kind {
                            xedoc_protocol::protocol::NonSteerableTurnKind::Review => {
                                "cannot steer a review turn".to_string()
                            }
                            xedoc_protocol::protocol::NonSteerableTurnKind::Compact => {
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            xedoc_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.xedoc_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(
