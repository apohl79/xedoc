...
use xedoc_rmcp_client::perform_oauth_login_silent;
...
fn marketplace_entries_from_outcome(
    outcome: xedoc_core_plugins::ConfiguredMarketplaceListOutcome,
    include_plugin: impl Fn(&xedoc_core_plugins::ConfiguredMarketplacePlugin) -> bool,
) -> (
    Vec<PluginMarketplaceEntry>,
    Vec<xedoc_app_server_protocol::MarketplaceLoadErrorInfo>,
) {
    (
        outcome
            .marketplaces
        ...
        outcome
            .errors
            .into_iter()
            .map(|err| xedoc_app_server_protocol::MarketplaceLoadErrorInfo {
...
impl PluginRequestProcessor {
    ...
    async fn plugin_list_response(
        ...
        plugins_manager.set_auth_mode(auth.as_ref().map(XedocAuth::api_auth_mode));
    ...
    async fn load_marketplace_plugins(
        &self,
        plugins_manager: Arc<xedoc_core_plugins::PluginsManager>,
        plugins_input: &xedoc_core_plugins::PluginsConfigInput,
    ...
    async fn plugin_read_response(
        ...
        let plugin = PluginDetail {
            ...
            hooks: outcome
                ...
                .hooks
                .into_iter()
                .map(|hook| xedoc_app_server_protocol::PluginHookSummary {
                    key: hook.key,
