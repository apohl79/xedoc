...
use xedoc_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            xedoc_rollout::find_thread_path_by_id_str(
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => xedoc_rollout::find_thread_path_by_id_str(
                &self.config.xedoc_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
...
pub(super) fn api_thread_goal_from_state(goal: xedoc_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: xedoc_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
