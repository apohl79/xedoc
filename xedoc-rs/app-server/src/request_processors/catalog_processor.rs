...
use xedoc_core::config::permission_profile_catalog;
...
fn skills_to_info(
    skills: &[xedoc_core::skills::SkillMetadata],
    disabled_paths: &HashSet<AbsolutePathBuf>,
) -> Vec<xedoc_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            xedoc_app_server_protocol::SkillMetadata {
                name: skill.name.clone(),
                description: skill.description.clone(),
                short_description: skill.short_description.clone(),
                interface: skill.interface.clone().map(|interface| {
                    xedoc_app_server_protocol::SkillInterface {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    xedoc_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| xedoc_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[xedoc_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[xedoc_core::skills::SkillError],
) -> Vec<xedoc_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| xedoc_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                xedoc_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![xedoc_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let effective_skill_roots = plugins_manager
                        ...
                        .await;
                    let skills_input = xedoc_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(xedoc_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
            ...
            let plugin_hooks = if plugins_enabled {
                ...
                xedoc_core_plugins::PluginHookLoadOutcome {
                ...
                xedoc_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = xedoc_hooks::list_hooks(xedoc_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.xedoc_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {
