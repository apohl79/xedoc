...
use xedoc_app_server_protocol::SelectedCapabilityRoot;
...
const XEDOC_TUI_CLIENT_NAME: &str = "xedoc-tui";
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                xedoc_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = xedoc_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(XEDOC_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<XedocThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            xedoc_home: self.config.xedoc_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(xedoc_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<xedoc_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<xedoc_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                xedoc_core::config::set_project_trust_level(
                    &listener_task_context.xedoc_home,
        ...
        if let Ok(Some(err)) =
            xedoc_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(xedoc_app_server_protocol::ThreadStartSource::Startup)
                {
                    xedoc_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    xedoc_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                session_source: None,
                thread_source,
            ...
            .map_err(|err| match err {
                ...
                XedocErr::UnsupportedOperation(message) => method_not_found(message),
    ...
    fn build_thread_config_overrides(
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(xedoc_app_server_protocol::AskForApproval::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            xedoc_linux_sandbox_exe: self.arg0_paths.xedoc_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = xedoc_core::util::normalize_thread_name(&name) else {
        ...
        self.thread_manager
            .update_thread_metadata(
                ...
                StoreThreadMetadataPatch {
                    ...
                    title_source: Some(xedoc_state::ThreadTitleSource::Manual),
                    ..Default::default()
                },
                /*include_archived*/ false,
            )
    ...
    ) -> Result<xedoc_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let config_snapshot = xedoc_thread.config_snapshot().await;
...
fn deserialize_stored_thread_item(
    item: xedoc_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        xedoc_error_info: error.xedoc_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: XedocErr) -> JSONRPCErrorError {
    match err {
...
) -> (Thread, Option<xedoc_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<xedoc_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(xedoc_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match xedoc_core::parse_turn_item(item) {
                Some(xedoc_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            xedoc_protocol::config_types::SandboxMode::WorkspaceWrite
                | xedoc_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &xedoc_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        xedoc_protocol::models::PermissionProfile::Disabled
        | xedoc_protocol::models::PermissionProfile::External { .. } => true,
        xedoc_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<xedoc_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
