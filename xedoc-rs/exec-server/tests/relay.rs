...
#[path = "../src/proto/xedoc.exec_server.relay.v1.rs"]
mod relay_proto;
...
use xedoc_exec_server::EnvironmentConnectionState;
use xedoc_exec_server::EnvironmentManager;
use xedoc_exec_server::EnvironmentReadyInfo;
use xedoc_exec_server::ExecParams;
use xedoc_exec_server::ExecResponse;
use xedoc_exec_server::ExecServerClient;
use xedoc_exec_server::ExecServerError;
use xedoc_exec_server::ExecServerRuntimePaths;
use xedoc_exec_server::FsReadFileParams;
use xedoc_exec_server::NoiseChannelIdentity;
use xedoc_exec_server::NoiseChannelPublicKey;
use xedoc_exec_server::NoiseRendezvousConnectArgs;
...
use xedoc_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> xedoc_api::SharedAuthProvider {
...
async fn deferred_noise_environment_connects_and_reconnects_with_fresh_bundle() -> Result<()> {
    ...
    let (xedoc_exe, xedoc_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(xedoc_exe, xedoc_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(xedoc_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
