...
use xedoc_exec_server::FsCloseParams;
use xedoc_exec_server::FsOpenParams;
use xedoc_exec_server::FsReadBlockParams;
use xedoc_exec_server::FsReadBlockResponse;
...
async fn file_reads_reject_named_pipes() -> Result<()> {
    ...
    let read_path = format!(r"\\.\pipe\xedoc-fs-read-{}", Uuid::new_v4());
    ...
    let stream_path = format!(r"\\.\pipe\xedoc-fs-stream-{}", Uuid::new_v4());
