...
pub(crate) struct ExecServerHarness {
    _xedoc_home: TempDir,
    _helper_paths: TestXedocHelperPaths,
    child: Child,
...
pub(crate) struct TestXedocHelperPaths {
    pub(crate) xedoc_exe: PathBuf,
    pub(crate) xedoc_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_xedoc_helper_paths() -> anyhow::Result<TestXedocHelperPaths> {
    let (helper_binary, xedoc_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestXedocHelperPaths {
        xedoc_exe: helper_binary,
...
{
    let helper_paths = test_xedoc_helper_paths()?;
    let xedoc_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.xedoc_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("XEDOC_HOME", xedoc_home.path());
    child.envs(env);
