...
use crate::environment::XEDOC_EXEC_SERVER_URL_ENV_VAR;

/// Lists the concrete environments available to Xedoc.
///
...
/// Default provider backed by `XEDOC_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `XEDOC_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `XEDOC_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(XEDOC_EXEC_SERVER_URL_ENV_VAR).ok())
    }
