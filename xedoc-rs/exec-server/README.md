...
`xedoc-exec-server` is the library backing `xedoc exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`xedoc-utils-pty`.

It provides:

- a CLI entrypoint: `xedoc exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `xedoc` binary owns hidden helper dispatch for sandboxed
filesystem operations and `xedoc-linux-sandbox`.

## Transport

The server speaks the exec-specific `xedoc-exec-server-protocol` message
...
must support it.
It uses the standard Xedoc ChatGPT sign-in state; run `xedoc login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `XEDOC_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Xedoc then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `XEDOC_API_KEY`;
Xedoc sends it as a bearer token on the registration request. For example:

```sh
XEDOC_API_KEY="$OPENAI_API_KEY" \
xedoc exec-server \
  --remote ... \
...
`xedoc.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/xedoc.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `xedoc-utils-pty`.
...
hidden helper process launched from the top-level `xedoc` executable and
...
`xedoc exec-server` command builds these paths from the `xedoc` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from Xedoc auth
