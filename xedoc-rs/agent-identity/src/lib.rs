...
use sha2::Sha512;
use xedoc_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "xedoc-app-server";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"xedoc-agent-identity-ed25519-v1";
...
/// single Xedoc run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "xedoc-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "xedoc-cli".to_string(),
        },
...
mod tests {
    ...
    use xedoc_protocol::auth::KnownPlan;
