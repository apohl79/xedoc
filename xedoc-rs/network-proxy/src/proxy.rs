...
#[command(name = "xedoc-network-proxy", about = "Xedoc network sandbox proxy")]
pub struct Args {}
...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_xedoc(mut self, managed_by_xedoc: bool) -> Self {
        self.managed_by_xedoc = managed_by_xedoc;
        self
    }
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "XEDOC_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "XEDOC_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
pub const XEDOC_PROXY_GIT_SSH_COMMAND_MARKER: &str = "XEDOC_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const XEDOC_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "XEDOC_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const XEDOC_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn xedoc_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{XEDOC_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{XEDOC_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_xedoc_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(XEDOC_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(XEDOC_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected Xedoc fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_xedoc_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    xedoc_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_xedoc_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"XEDOC_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/xedoc-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_xedoc_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            xedoc_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&xedoc_proxy_git_ssh_command(SocketAddr::new(
