...
fn managed_ca_dir() -> Result<PathBuf> {
    let xedoc_home =
        find_xedoc_home().context("failed to resolve XEDOC_HOME for managed MITM CA")?;
    Ok(xedoc_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Xedoc-generated MITM CA bundle.
