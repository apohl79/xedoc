...
`xedoc-network-proxy` is Xedoc's local network policy enforcement proxy. It runs:
...
`xedoc-network-proxy` reads from Xedoc's merged `config.toml` (via `xedoc-core` config loading).
...
# bundle env vars pointing at immutable public files under $XEDOC_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p xedoc-network-proxy --
```
...
`xedoc-network-proxy` can be embedded as a library with a thin API:

```rust
use xedoc_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `xedoc-network-proxy` is embedded in managed Xedoc runtime, policy decisions emit structured
OTEL-compatible events with `target=xedoc_otel.network_proxy`.

Event name:

- `xedoc.network_proxy.policy_decision`
...
This section documents the protections implemented by `xedoc-network-proxy`, and the boundaries of
