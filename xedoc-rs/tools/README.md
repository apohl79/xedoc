...
`xedoc-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `xedoc-core`.
...
compatibility-sensitive orchestration in `xedoc-core` until the surrounding
...
- do not move `xedoc-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `xedoc-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `xedoc-core`.
3. Leave compatibility-sensitive adapters in `xedoc-core` while downstream
...
`xedoc-core`, that is a sign to revisit the extraction boundary before adding
more here.
