...
use xedoc_exec_server::ExecutorCapabilityDiscoveryCache;
...
use xedoc_skills_extension::ExecutorSkillProvider;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("xedoc-executor-skill-fs-{}", std::process::id()));
