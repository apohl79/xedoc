...
use xedoc_core_skills::SKILLS_INTRO_WITH_ABSOLUTE_PATHS;
use xedoc_core_skills::SkillLoadOutcome;
use xedoc_core_skills::injection::InjectedHostSkillPrompts;
use xedoc_extension_api::ConversationHistory;
...
use xedoc_extension_api::PreviousWorldStateSection;
...
use xedoc_extension_api::WorldStateContributionInput;
...
use xedoc_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use xedoc_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use xedoc_skills::SkillMetadata;
use xedoc_skills_extension::SkillProviders;
use xedoc_skills_extension::SkillsExtensionConfig;
use xedoc_skills_extension::catalog::SkillAuthority;
use xedoc_skills_extension::catalog::SkillCatalog;
use xedoc_skills_extension::catalog::SkillCatalogEntry;
use xedoc_skills_extension::catalog::SkillPackageId;
use xedoc_skills_extension::catalog::SkillProviderError;
use xedoc_skills_extension::catalog::SkillReadResult;
use xedoc_skills_extension::catalog::SkillResourceId;
use xedoc_skills_extension::catalog::SkillSearchResult;
use xedoc_skills_extension::catalog::SkillSourceKind;
use xedoc_skills_extension::install;
use xedoc_skills_extension::install_with_providers;
use xedoc_skills_extension::provider::SkillListQuery;
use xedoc_skills_extension::provider::SkillProvider;
use xedoc_skills_extension::provider::SkillProviderFuture;
use xedoc_skills_extension::provider::SkillReadRequest;
use xedoc_skills_extension::provider::SkillSearchRequest;
use xedoc_utils_absolute_path::AbsolutePathBuf;
use xedoc_utils_path_uri::PathUri;

type TestResult = Result<(), Box<dyn std::error::Error>>;

static NEXT_XEDOC_HOME_ID: AtomicUsize = AtomicUsize::new(0);
const DEMO_SKILL_CONTENTS: &str =
    "---\nname: demo\ndescription: Demo skill.\n---\n# Demo\n\nUse the demo skill.\n";

#[tokio::test]
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let xedoc_home = test_xedoc_home();
    let skill_path = xedoc_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(xedoc_home)?;
    Ok(())
}
...
fn test_xedoc_home() -> PathBuf {
    let id = NEXT_XEDOC_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "xedoc-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
