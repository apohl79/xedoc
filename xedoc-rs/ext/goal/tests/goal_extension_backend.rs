...
use xedoc_extension_api::ExtensionEventSink;
use xedoc_extension_api::ExtensionRegistryBuilder;
use xedoc_extension_api::FunctionCallError;
use xedoc_extension_api::NoopTurnItemEmitter;
use xedoc_extension_api::ThreadResumeInput;
use xedoc_extension_api::ThreadStartInput;
use xedoc_extension_api::ThreadStopInput;
use xedoc_extension_api::ToolCall;
use xedoc_extension_api::ToolCallOutcome;
use xedoc_extension_api::ToolCallSource;
use xedoc_extension_api::ToolExecutor;
use xedoc_extension_api::ToolFinishInput;
use xedoc_extension_api::ToolPayload;
use xedoc_extension_api::TurnErrorInput;
use xedoc_extension_api::TurnStartInput;
use xedoc_extension_api::TurnStopInput;
use xedoc_goal_extension::GoalObjectiveUpdate;
use xedoc_goal_extension::GoalRuntimeHandle;
use xedoc_goal_extension::GoalService;
use xedoc_goal_extension::GoalSetRequest;
use xedoc_goal_extension::GoalTokenBudgetUpdate;
use xedoc_goal_extension::install_with_backend;
...
use xedoc_protocol::protocol::ThreadGoalStatus;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(xedoc_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", XedocErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(xedoc_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", XedocErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(xedoc_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(xedoc_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            xedoc_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<xedoc_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: xedoc_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = xedoc_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: XedocErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: xedoc_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        xedoc_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: xedoc_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<xedoc_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    xedoc_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    runtime: &xedoc_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = xedoc_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .xedoc_home()
...
fn protocol_status(status: xedoc_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        xedoc_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        xedoc_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        xedoc_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        xedoc_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        xedoc_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        xedoc_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}
