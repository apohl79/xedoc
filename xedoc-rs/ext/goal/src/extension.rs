...
use xedoc_extension_api::ConfigContributor;
...
use xedoc_extension_api::ThreadIdleInput;
use xedoc_extension_api::ThreadLifecycleContributor;
use xedoc_extension_api::ThreadResumeInput;
use xedoc_extension_api::ThreadStartInput;
use xedoc_extension_api::ThreadStopInput;
use xedoc_extension_api::TokenUsageContributor;
use xedoc_extension_api::ToolCallOutcome;
use xedoc_extension_api::ToolContributor;
use xedoc_extension_api::ToolFinishInput;
use xedoc_extension_api::ToolLifecycleContributor;
use xedoc_extension_api::ToolLifecycleFuture;
use xedoc_extension_api::TurnAbortInput;
use xedoc_extension_api::TurnErrorInput;
use xedoc_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                xedoc_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                XedocErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn xedoc_extension_api::ToolExecutor<xedoc_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };
