...
use xedoc_extension_api::JsonToolOutput;
...
use xedoc_protocol::protocol::ThreadGoal;
use xedoc_protocol::protocol::ThreadGoalStatus;
use xedoc_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<xedoc_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> xedoc_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => xedoc_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => xedoc_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: xedoc_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            xedoc_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
            ...
            xedoc_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<xedoc_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &xedoc_state::StateRuntime,
    thread_id: ThreadId,
    goal: &xedoc_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: xedoc_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: xedoc_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> xedoc_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => xedoc_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => xedoc_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => xedoc_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => xedoc_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => xedoc_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => xedoc_state::ThreadGoalStatus::Complete,
    }
}
