...
use xedoc_api::SearchClient;
use xedoc_api::SearchCommands;
use xedoc_api::SearchQuery;
use xedoc_api::SearchRequest;
use xedoc_api::SearchSettings;
use xedoc_core::X_XEDOC_TURN_METADATA_HEADER;
use xedoc_core::web_search_action_detail;
...
use xedoc_extension_api::parse_tool_input_schema_without_compaction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "xedoc.web_search.results.payload_bytes";
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.xedoc_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = xedoc_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_XEDOC_TURN_METADATA_HEADER, header_value);
    }
