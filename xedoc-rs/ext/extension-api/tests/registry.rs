...
use xedoc_extension_api::ApprovalReviewContributor;
use xedoc_extension_api::ConfigContributor;
use xedoc_extension_api::ContextContributor;
use xedoc_extension_api::ContextualUserFragment;
...
use xedoc_extension_api::SkillInvocationContributor;
...
use xedoc_extension_api::TurnInputContributor;
use xedoc_extension_api::TurnItemContributor;
use xedoc_extension_api::TurnLifecycleContributor;
use xedoc_extension_api::empty_extension_registry;
use xedoc_protocol::items::HookPromptItem;
...
#[test]
fn from_registry_preserves_skill_invocation_contributors() {
    let contributor = Arc::new(AllContributors);
    let mut builder = ExtensionRegistryBuilder::<()>::new();
    builder.skill_invocation_contributor(contributor);
    let registry = builder.build();

    let copied_registry = ExtensionRegistryBuilder::from_registry(&registry).build();

    assert_eq!(copied_registry.skill_invocation_contributors().len(), 1);
}
        ...
        fragments.extend(
            contributor
                .contribute_turn_context(TurnContextContributionInput {
                    thread_id: xedoc_protocol::ThreadId::default(),
