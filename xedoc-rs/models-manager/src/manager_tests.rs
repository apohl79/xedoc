...
use xedoc_login::ExternalAuth;
use xedoc_login::ExternalAuthRefreshContext;
use xedoc_login::TokenData;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> xedoc_login::ExternalAuthFuture<'_, XedocAuth> {
        Box::pin(async { Ok(XedocAuth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> xedoc_login::ExternalAuthFuture<'_, XedocAuth> {
        Box::pin(async { Ok(XedocAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_xedoc_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_xedoc_backend })
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(xedoc_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(xedoc_home: &Path) -> XedocAuth {
    let auth_dot_json = xedoc_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: xedoc_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(xedoc_home).expect("xedoc home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(xedoc_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-xedoc/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(xedoc_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(xedoc_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(xedoc_home.path().to_path_buf(), endpoint);
...
#[tokio::test]
async fn refresh_available_models_uses_remote_only_catalog_for_command_auth() {
    let remote_models = vec![remote_model(
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager = openai_manager_for_tests(xedoc_home.path().to_path_buf(), endpoint.clone());
    manager
        .cache_manager
        .as_mut()
        .expect("cached model manager")
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_xedoc_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_xedoc_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(xedoc_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        xedoc_home.path().to_path_buf(),
        endpoint.clone(),
