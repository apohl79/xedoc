...
/// The easiest way to iterate is to add unit tests and have Xedoc fix the implementation.
...
/// Tests are at the top to encourage using TDD + Xedoc to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd xedoc-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/xedoc && rg -n "xedoc_api" xedoc-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n xedoc_api xedoc-rs -S".to_string(),
                query: Some("xedoc_api".to_string()),
                path: Some("xedoc-rs".to_string()),
            }],
        );
    }
