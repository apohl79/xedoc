...
use xedoc_core_environment::TurnEnvironment;
use xedoc_core_environment::TurnEnvironmentSnapshot;
use xedoc_core_environment::TurnEnvironmentState;
...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///xedoc%20runtime").expect("POSIX cwd URI"),
            "/xedoc runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/xedoc%20runtime").expect("Windows cwd URI"),
            r"C:\xedoc runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.xedoc_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".xedoc-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_xedoc_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".xedoc").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".xedoc"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.xedoc_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.xedoc_home).unwrap();
...
fn create_skill(xedoc_home: PathBuf, name: &str, description: &str) {
    let skill_dir = xedoc_home.join(format!("skills/{name}"));
