...
use xedoc_config::default_project_root_markers;
use xedoc_config::merge_toml_values;
use xedoc_config::project_root_markers_from_config;
use xedoc_core_config::config::Config;
use xedoc_core_context::UserInstructions as ContextUserInstructions;
...
use xedoc_file_system::FindUpErrorPolicy;
use xedoc_file_system::find_nearest_ancestor_with_markers;
...
impl LoadedAgentsMd {
    ...
    /// compile `xedoc-core` as a normal dependency without that configuration.
    ...
    /// Converts these instructions to the model-visible contextual fragment.
    pub fn contextual_user_fragment(&self) -> ContextUserInstructions {
