...
pub struct RecentSubAgentActivity {
    items: VecDeque<String>,
    changed: bool,
}

impl RecentSubAgentActivity {
    /// Records a response item when it provides activity suitable for summarization.
    pub fn record_response_item(&mut self, item: &ResponseItem) {
    ...
    /// Returns a summary only after the activity history has changed.
    pub fn snapshot_if_changed(&mut self) -> Option<String> {
        if !self.changed {
            return None;
        }
        self.changed = false;
