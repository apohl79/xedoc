...
use xedoc_core_context_manager::truncate_function_output_payload;
use xedoc_core_exec::unified_exec::format_output_omission_marker;
...
use xedoc_protocol::models::function_call_output_content_items_to_text;
use xedoc_tools::LoadableToolSpec;
use xedoc_tools::sanitize_original_image_detail;
use xedoc_utils_output_truncation::TruncationPolicy;
use xedoc_utils_output_truncation::approx_token_count;
use xedoc_utils_output_truncation::formatted_truncate_text;
use xedoc_utils_output_truncation::truncate_text;
use xedoc_utils_string::take_bytes_at_char_boundary;
...
/// Boxes a concrete output for use by the dynamic tool execution boundary.
pub fn boxed_tool_output<T>(output: T) -> Box<dyn ToolOutput>
where
    T: ToolOutput + 'static,
{
    Box::new(output)
...
/// Model-facing output for tool-search results.
#[derive(Clone)]
...
/// General model-facing function tool output.
...
impl FunctionToolOutput {
    /// Creates a text-only function tool result.
    ...
    /// Creates a function tool result with structured content items.
    pub fn from_content(
    ...
    /// Converts the content items into their text representation.
...
/// Model-facing output for a successful apply-patch tool call.
pub struct ApplyPatchToolOutput {
    pub text: String,
}

impl ApplyPatchToolOutput {
    /// Creates an apply-patch output from its textual result.
...
/// Model-facing output for unified shell execution.
...
impl ExecCommandToolOutput {
    ...
    /// Returns model-visible output truncated to the supplied token budget.
    pub fn truncated_output(&self, max_tokens: usize) -> String {
...
fn telemetry_preview(content: &str) -> String {
    ...
    for index in 0..TELEMETRY_PREVIEW_MAX_LINES {
