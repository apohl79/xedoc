...
pub use crate::text_formatting::fmt_elapsed_compact;
...
impl StatusIndicatorWidget {
    ...
    pub fn set_interrupt_binding(&mut self, binding: Option<KeyBinding>) {
        self.interrupt_binding = binding;
    }

    pub fn set_detach_hint_visible(&mut self, visible: bool) {
...
mod tests {
    ...
    fn details_args_can_disable_capitalization_and_limit_lines() {
        ...
        w.update_details(
            Some("cargo test -p xedoc-core and then cargo test -p xedoc-tui".to_string()),
        ...
        assert_eq!(
            w.details(),
            Some("cargo test -p xedoc-core and then cargo test -p xedoc-tui")
        );
