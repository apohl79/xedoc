...
use crate::rate_limit_labels::fallback_limit_label;
use crate::rate_limit_labels::limit_label_for_window;
...
use chrono::Local;
use chrono::Utc;
use xedoc_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use xedoc_app_server_protocol::RateLimitSnapshot;
use xedoc_app_server_protocol::RateLimitWindow;
use xedoc_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use xedoc_protocol::num_format::format_with_separators;
...
pub const RATE_LIMIT_STALE_THRESHOLD_MINUTES: i64 = 15;
...
pub struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `xedoc` or `codex_other`).
...
#[derive(Debug, Clone)]
pub struct CreditsSnapshotDisplay {
...
#[derive(Debug, Clone)]
pub struct SpendControlLimitSnapshotDisplay {
...
pub fn rate_limit_snapshot_display_for_limit(
    snapshot: &RateLimitSnapshot,
    limit_name: String,
    captured_at: DateTime<Local>,
) -> RateLimitSnapshotDisplay {
...
pub fn compose_rate_limit_data(
...
pub fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let combine_non_xedoc_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_xedoc_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_xedoc_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
pub fn render_status_limit_progress_bar(percent_remaining: f64) -> String {
...
pub fn format_status_limit_summary(percent_remaining: f64) -> String {
...
mod tests {
    ...
    #[test]
    fn non_xedoc_single_limit_renders_combined_row() {
        let now = Local::now();
        let xedoc = RateLimitSnapshotDisplay {
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "xedoc-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[xedoc, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "xedoc-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_xedoc_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "xedoc-other".to_string(),
            captured_at: now,
