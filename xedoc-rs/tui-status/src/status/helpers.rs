...
pub fn compose_agents_summary(config: &Config, paths: &[PathUri]) -> String {
...
pub fn plan_type_display_name(plan_type: PlanType) -> String {
...
mod tests {
    ...
    async fn test_config(xedoc_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let xedoc_home = TempDir::new().expect("temp xedoc home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = xedoc_home.path().join("global.md");
        let config = test_config(&xedoc_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let xedoc_home = TempDir::new().expect("temp xedoc home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = xedoc_home.path().join("override.md");
        let config = test_config(&xedoc_home, &cwd).await;
