...
#[derive(Debug, Clone)]
pub struct StatusTokenUsageData {
    total: i64,
    input: i64,
    output: i64,
...
#[cfg(any(test, feature = "test-support"))]
#[allow(clippy::too_many_arguments)]
pub fn new_status_output_with_rate_limits(
    config: &Config,
    account_display: Option<&StatusAccountDisplay>,
    token_info: Option<&TokenUsageInfo>,
    total_usage: &TokenUsage,
...
#[allow(clippy::too_many_arguments)]
pub fn new_status_output_with_rate_limits_handle(
    config: &Config,
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            Span::from(format!("{}x_ ", FieldFormatter::INDENT)).dim(),
            Span::from("E47 Xedoc").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{})", xedoc_cli_version())).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run xedoc login to use ChatGPT)".to_string()
            }
        });
