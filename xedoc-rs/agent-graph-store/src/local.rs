...
impl std::fmt::Debug for LocalAgentGraphStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LocalAgentGraphStore")
            .field("xedoc_home", &self.state_db.xedoc_home())
...
fn to_state_status(status: ThreadSpawnEdgeStatus) -> xedoc_state::DirectionalThreadSpawnEdgeStatus {
    match status {
        ThreadSpawnEdgeStatus::Open => xedoc_state::DirectionalThreadSpawnEdgeStatus::Open,
        ThreadSpawnEdgeStatus::Closed => xedoc_state::DirectionalThreadSpawnEdgeStatus::Closed,
    }
}
...
mod tests {
    ...
    async fn state_runtime() -> TestRuntime {
        let xedoc_home = TempDir::new().expect("tempdir should be created");
        let state_db =
            StateRuntime::init(xedoc_home.path().to_path_buf(), "test-provider".to_string())
                .await
