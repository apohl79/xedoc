...
const XEDOC_AUTH_SECRETS_FILENAME: &str = "xedoc_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Xedoc authentication credentials used by the CLI, TUI, app server, and other clients.
    XedocAuth,
...
impl LocalSecretsBackend {
    pub fn new(xedoc_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(xedoc_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let xedoc_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(xedoc_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let xedoc_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let xedoc_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        xedoc_auth_backend.set(&scope, &name, "xedoc-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            xedoc_auth_backend.get(&scope, &name)?,
            Some("xedoc-auth-value".to_string())
        );
        assert_eq!(
        ...
        assert!(!xedoc_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }
}
