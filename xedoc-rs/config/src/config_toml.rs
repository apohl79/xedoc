//! Schema-heavy configuration TOML types used by Xedoc.
...
/// Base config deserialized from ~/.xedoc/config.toml.
...
pub struct ConfigToml {
    ...
    /// file: Use a file in the Xedoc home directory.
    ...
    /// Defaults to `$XEDOC_SQLITE_HOME` when set. Otherwise uses `$XEDOC_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Xedoc writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$XEDOC_HOME/log`.
    ...
    /// directories for `.xedoc` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different Xedoc version.
    pub allow_xedoc_version_mismatch: Option<bool>,
