...
use xedoc_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/xedoc/config.toml";
...
///
/// - system    `/etc/xedoc/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Xedoc\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/xedoc/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Xedoc\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${XEDOC_HOME}/config.toml`
/// - profile   `${XEDOC_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.xedoc/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.xedoc/config.toml` (loaded but disabled when untrusted)
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(xedoc_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, xedoc_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(xedoc_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, xedoc_home);
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(xedoc_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/xedoc/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_xedoc_system_dir() -> PathBuf {
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Xedoc to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".xedoc"))
    }
}
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_xedoc_folder: dot_xedoc_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_xedoc_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let xedoc_home_abs = AbsolutePathBuf::from_absolute_path(xedoc_home)?;
    let xedoc_home_normalized =
        normalize_path(xedoc_home_abs.as_path()).unwrap_or_else(|_| xedoc_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_xedoc_abs = dir.join(".xedoc");
        let dot_xedoc_uri = PathUri::from_abs_path(&dot_xedoc_abs);
        if !fs
            .get_metadata(&dot_xedoc_uri, /*sandbox*/ None)
            .await
        ...
        let dot_xedoc_normalized =
            normalize_path(dot_xedoc_abs.as_path()).unwrap_or_else(|_| dot_xedoc_abs.to_path_buf());
        if dot_xedoc_abs == xedoc_home_abs || dot_xedoc_normalized == xedoc_home_normalized {
            continue;
        }
        let config_file = dot_xedoc_abs.join(CONFIG_TOML_FILE);
...
/// from a file like `/etc/xedoc/managed_config.toml` that has the same
...
/// where each specified field is treated as a constraint, allowing only the
/// specified value.
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Xedoc").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Xedoc").join("config.toml"))
        );
    }
}
