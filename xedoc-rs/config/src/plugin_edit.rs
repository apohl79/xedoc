...
pub async fn clear_user_plugin(xedoc_home: &Path, plugin_key: String) -> std::io::Result<()> {
    apply_user_plugin_config_edits(xedoc_home, vec![PluginConfigEdit::Clear { plugin_key }]).await
}
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&xedoc_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(xedoc_home.path());
...
"#,
        ...
        clear_user_plugin(xedoc_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(xedoc_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let xedoc_home = TempDir::new().unwrap();
        let target_path = xedoc_home.path().join("target_config.toml");
        symlink(&target_path, xedoc_home.path().join(CONFIG_TOML_FILE)).unwrap();
...
enabled = true
    ...
    fn read_config(xedoc_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(xedoc_home.join(CONFIG_TOML_FILE)).unwrap()).unwrap()
    }
}
