...
use xedoc_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == xedoc_features::Feature::Artifact {
            continue;
        }
        if feature.id == xedoc_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                removed_code_mode_schema(schema_gen),
            );
            continue;
        }
        if feature.id == xedoc_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<xedoc_features::FeatureToml<
                    xedoc_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == xedoc_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<xedoc_features::FeatureToml<
                    xedoc_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        ...
        if feature.id == xedoc_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<xedoc_features::FeatureToml<
                    xedoc_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == xedoc_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<xedoc_features::FeatureToml<
                    xedoc_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == xedoc_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == xedoc_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<xedoc_features::FeatureToml<
                    xedoc_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
...
fn removed_code_mode_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for key in ["excluded_tool_namespaces", "direct_only_tool_namespaces"] {
        config_validation
            .properties
            .insert(key.to_string(), schema_gen.subschema_for::<Vec<String>>());
    }
