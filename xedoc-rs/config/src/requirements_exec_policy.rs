...
use xedoc_execpolicy::RuleRef;
use xedoc_execpolicy::rule::PatternToken;
use xedoc_execpolicy::rule::PrefixPattern;
use xedoc_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Xedoc merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `xedoc-execpolicy`.
