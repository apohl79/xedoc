...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("xedoc-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, xedoc_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.xedoc/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_xedoc_folder } => Some(dot_xedoc_folder.clone()),
    ...
    /// Returns the `.xedoc/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$XEDOC_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$XEDOC_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_dot_xedoc_folder: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            dot_xedoc_folder: current_project_dot_xedoc_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_xedoc_folder {
                ...
                if previous == current_project_dot_xedoc_folder
                    || !current_project_dot_xedoc_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_xedoc_folder = Some(current_project_dot_xedoc_folder);
        }
    }
