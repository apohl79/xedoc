//! Types used to define loaded and effective Xedoc configuration values.
...
use xedoc_protocol::config_types::EnvironmentVariablePattern;
pub use xedoc_protocol::config_types::ModeKind;
pub use xedoc_protocol::config_types::Personality;
pub use xedoc_protocol::config_types::ServiceTier;
...
pub enum ResumeCwdMode {
    /// Use the directory where Xedoc was launched.
    Current,
...
/// Determine where Xedoc should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in XEDOC_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in XEDOC_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Xedoc should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AppConfig {
    /// When `false`, Xedoc does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Xedoc cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$XEDOC_HOME/themes` for custom themes.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Xedoc's terminal-specific default. Set to `0` to
...
/// (primarily the Xedoc IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Xedoc-managed fast defaults.
    ...
    pub hide_gpt_5_1_xedoc_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Xedoc skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Xedoc successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Xedoc last successfully activated for this marketplace.
    #[serde(default)]
