...
/// OAuth client settings used when Xedoc launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
/// Authentication flow Xedoc attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Xedoc should start this MCP server.
    pub environment_id: String,

    /// When `false`, Xedoc skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `xedoc exec` exits with an error if this MCP server fails to initialize.
