...
- a `xedoc-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Xedoc CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `xedoc-exec` CLI can check if its arg0 is `xedoc-linux-sandbox` and, if so, execute as if it were `xedoc-linux-sandbox`
  - this should also be true of the `xedoc` multitool CLI

On Linux, Xedoc prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `xedoc-resources/bwrap` binary shipped
with Xedoc.
Xedoc also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Xedoc surfaces the same startup warning
...
it cannot create the required user namespaces, so Xedoc rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `xedoc-resources/bwrap` path.
- If `bwrap` is missing, Xedoc also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Xedoc surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Xedoc rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.xedoc`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `xedoc sandbox`; the host OS selects the sandbox backend.
