...
use url::Url;
use xedoc_network_proxy::PROXY_ATTRIBUTION_TOKEN_ENV_KEY;
use xedoc_network_proxy::write_attribution_frame;
...
const PROXY_SOCKET_DIR_PREFIX: &str = "xedoc-linux-sandbox-proxy-";
...
fn proxy_socket_parent_dir() -> PathBuf {
    if let Some(xedoc_home) = std::env::var_os("XEDOC_HOME") {
        let candidate = PathBuf::from(xedoc_home).join("tmp");
...
fn harden_bridge_process() -> io::Result<()> {
    ...
    xedoc_process_hardening::disable_process_dumping()
}
...
mod tests {
    ...
    fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
        ...
        let socket_dir = root.path().join("xedoc-linux-sandbox-proxy-test");
    ...
    fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("xedoc-linux-sandbox-proxy-1234-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("xedoc-linux-sandbox-proxy-1234-1000-0"),
            Some(1234)
        );
        assert_eq!(
            parse_proxy_socket_dir_owner_pid("xedoc-linux-sandbox-proxy-x"),
            None
        );
