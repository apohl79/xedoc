...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/xedoc-arg0-session/xedoc-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/xedoc-arg0-session/xedoc-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--xedoc-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        xedoc_protocol::permissions::FileSystemSandboxEntry {
            path: xedoc_protocol::permissions::FileSystemPath::Special {
                value: xedoc_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: xedoc_protocol::permissions::FileSystemAccessMode::Write,
        },
        xedoc_protocol::permissions::FileSystemSandboxEntry {
            path: xedoc_protocol::permissions::FileSystemPath::Path { path: docs },
            access: xedoc_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn root_write_read_only_carveout_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        xedoc_protocol::permissions::FileSystemSandboxEntry {
            path: xedoc_protocol::permissions::FileSystemPath::Special {
                value: xedoc_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: xedoc_protocol::permissions::FileSystemAccessMode::Write,
        },
        xedoc_protocol::permissions::FileSystemSandboxEntry {
...
fn synthetic_mount_registry_root_is_unique_to_effective_user() {
    ...
    assert_eq!(
        ...
        std::env::temp_dir().join(format!(
            "xedoc-bwrap-synthetic-mount-targets-{effective_uid}"
        ))
    );
}
