...
//! - sensitive subpaths such as `.git`, `.agents`, and `.xedoc` remain
...
use xedoc_protocol::permissions::is_protected_metadata_name;
use xedoc_protocol::protocol::FileSystemAccessMode;
use xedoc_protocol::protocol::FileSystemPath;
use xedoc_protocol::protocol::FileSystemSandboxPolicy;
use xedoc_protocol::protocol::FileSystemSpecialPath;
use xedoc_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.xedoc`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".xedoc")
                {
                    return None;
                }
...
mod tests {
    ...
    use xedoc_protocol::protocol::FileSystemAccessMode;
    use xedoc_protocol::protocol::FileSystemPath;
    use xedoc_protocol::protocol::FileSystemSandboxEntry;
    use xedoc_protocol::protocol::FileSystemSandboxPolicy;
    use xedoc_protocol::protocol::FileSystemSpecialPath;
    use xedoc_utils_absolute_path::AbsolutePathBuf;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_xedoc = temp_dir.path().join("real-xedoc");
        let logical_xedoc = logical_home.join(".xedoc");
        let real_memories = real_xedoc.join("memories");
        let logical_memories = logical_xedoc.join("memories");
        ...
        std::os::unix::fs::symlink(&real_xedoc, &logical_xedoc)
            .expect("create symlinked xedoc home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".xedoc"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_xedoc = path_to_string(&temp_dir.path().join(".xedoc"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_xedoc));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_xedoc)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
