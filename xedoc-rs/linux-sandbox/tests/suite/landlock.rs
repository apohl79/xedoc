...
use xedoc_core::exec::process_exec_tool_call;
...
fn xedoc_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_xedoc-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> xedoc_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<xedoc_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let xedoc_linux_sandbox_exe = Some(xedoc_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &xedoc_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(XedocErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(XedocErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<xedoc_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> xedoc_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(XedocErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let xedoc_linux_sandbox_exe: Option<PathBuf> = Some(xedoc_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_xedoc_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_xedoc = tmpdir.path().join(".xedoc");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_xedoc).expect("create .xedoc");

    let git_target = dot_git.join("config");
    let xedoc_target = dot_xedoc.join("config.toml");
    ...
    let xedoc_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", xedoc_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".xedoc write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(xedoc_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_xedoc_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-xedoc");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_xedoc = tmpdir.path().join(".xedoc");
    symlink(&decoy, &dot_xedoc).expect("create .xedoc symlink");
    ...
    let xedoc_output = expect_denied(
        ...
        .await,
        ".xedoc symlink replacement should be denied",
    );
    assert_ne!(xedoc_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_xedoc_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(XedocErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".xedoc symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|xedoc|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_xedoc_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".xedoc"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .xedoc directory creation should be denied",
    );
    assert_ne!(mkdir_xedoc_output.exit_code, 0);
    assert!(!subdir.join(".xedoc").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = xedoc_linux_sandbox_exe()
        .parent()
