...
use xedoc_rollout::ThreadItem;
use xedoc_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(xedoc_home: &Path, path: &Path) -> bool {
    path.starts_with(xedoc_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = xedoc_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.xedoc_home.as_path(), &legacy_thread_ids).await
    {
...
) -> Option<(String, xedoc_state::ThreadTitleSource)> {
    let title = metadata.title.trim();
    if title.is_empty() {
        None
    } else if metadata.title_source != xedoc_state::ThreadTitleSource::Derived {
...
pub(super) fn set_thread_name_from_title(
    ...
    title: String,
    title_source: Option<xedoc_state::ThreadTitleSource>,
) {
