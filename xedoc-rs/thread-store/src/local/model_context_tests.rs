...
async fn loads_latest_checkpoint_with_required_turn_metadata() {
    ...
    let thread_id = xedoc_protocol::ThreadId::from_string(&uuid.to_string()).expect("thread id");
...
async fn assert_reverse_scan_matches_full_history(path: &Path) {
    let session_meta = xedoc_rollout::read_session_meta_line(path)
        .await
...
fn completed_user_message(turn_id: &str, message: &str) -> RolloutItem {
    RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
        thread_id: xedoc_protocol::ThreadId::from_string("00000000-0000-0000-0000-000000000000")
