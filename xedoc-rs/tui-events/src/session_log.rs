...
pub fn maybe_init(config: &Config) {
    let enabled = std::env::var("XEDOC_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("XEDOC_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
...
pub fn log_inbound_app_event(event: &AppEvent) {
    // Log only if enabled
...
pub fn log_outbound_op(op: &AppCommand) {
    if !LOGGER.is_enabled() {
        return;
    }
