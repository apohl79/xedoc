...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, XedocErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let XedocErr::UnexpectedStatus(err) = err else {
        panic!("expected XedocErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let XedocErr::CyberPolicy { message } = err else {
        panic!("expected XedocErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_maps_prompt_too_long_from_400_body() {
    ...
    assert!(matches!(err, XedocErr::ContextWindowExceeded));
}
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let XedocErr::InvalidRequest(message) = err else {
        panic!("expected XedocErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    let XedocErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected XedocErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
