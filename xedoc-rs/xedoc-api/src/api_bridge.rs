...
use xedoc_protocol::auth::PlanType;
use xedoc_protocol::error::RetryLimitReachedError;
use xedoc_protocol::error::UnexpectedResponseError;
use xedoc_protocol::error::UsageLimitReachedError;
use xedoc_protocol::error::XedocErr;

pub fn map_api_error(err: ApiError) -> XedocErr {
    match err {
        ApiError::ContextWindowExceeded => XedocErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => XedocErr::QuotaExceeded,
        ApiError::UsageNotIncluded => XedocErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => XedocErr::Stream(message, delay),
        ApiError::Stream(msg) => XedocErr::Stream(msg, None),
        ApiError::ServerOverloaded => XedocErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            let user_message = api_error_user_message(status, &message);
            XedocErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                user_message,
                url: None,
        ...
        ApiError::InvalidRequest { message } => XedocErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => XedocErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return XedocErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                        ...
                        .contains("The image data you provided does not represent a valid image")
                    {
                        ...
                        XedocErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return XedocErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return XedocErr::UsageNotIncluded;
                        }
                    }

                    XedocErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => XedocErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => XedocErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                XedocErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => XedocErr::Stream(msg, None),
    }
}
