...
use anyhow::bail;
use clap::Parser;
use xedoc_core_api::AbsolutePathBuf;
use xedoc_core_api::AltScreenMode;
use xedoc_core_api::Arg0DispatchPaths;
use xedoc_core_api::AskForApproval;
use xedoc_core_api::AuthCredentialsStoreMode;
use xedoc_core_api::AuthManager;
use xedoc_core_api::AutoCompactTokenLimitScope;
use xedoc_core_api::Config;
use xedoc_core_api::ConfigLayerStack;
use xedoc_core_api::Constrained;
use xedoc_core_api::EnvironmentManager;
use xedoc_core_api::EventMsg;
use xedoc_core_api::ExecServerRuntimePaths;
use xedoc_core_api::ExtensionRegistryBuilder;
use xedoc_core_api::Features;
use xedoc_core_api::GhostSnapshotConfig;
use xedoc_core_api::History;
use xedoc_core_api::ModelAvailabilityNuxConfig;
use xedoc_core_api::MultiAgentV2Config;
use xedoc_core_api::NewThread;
use xedoc_core_api::Notice;
use xedoc_core_api::OAuthCredentialsStoreMode;
use xedoc_core_api::OPENAI_PROVIDER_ID;
use xedoc_core_api::Op;
use xedoc_core_api::OtelConfig;
use xedoc_core_api::PermissionProfile;
use xedoc_core_api::Permissions;
...
use xedoc_core_api::UserInput;
use xedoc_core_api::WebSearchMode;
use xedoc_core_api::XedocHomeUserInstructionsProvider;
use xedoc_core_api::XedocThread;
use xedoc_core_api::arg0_dispatch_or_else;
use xedoc_core_api::build_models_manager;
use xedoc_core_api::built_in_model_providers;
use xedoc_core_api::find_xedoc_home;
use xedoc_core_api::init_state_db;
use xedoc_core_api::install_image_generation_extension;
use xedoc_core_api::item_event_to_server_notification;
use xedoc_core_api::local_agent_graph_store_from_state_db;
use xedoc_core_api::resolve_installation_id;
use xedoc_core_api::set_default_originator;
use xedoc_core_api::thread_store_from_config;

#[derive(Debug, Parser)]
#[command(
    name = "xedoc-thread-manager-sample",
    about = "Run one Xedoc turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("xedoc_thread_manager_sample".to_string()) {
    ...
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_xedoc_api_key_env*/ false).await;
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        config.xedoc_self_exe.clone(),
        config.xedoc_linux_sandbox_exe.clone(),
    )?;
    ...
    turn_output?;
    shutdown_result.context("shut down Xedoc thread")?;

    Ok(())
}

fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    let xedoc_home = find_xedoc_home().context("find Xedoc home")?;
    ...
    let mut config = Config {
        ...
        sqlite_home: xedoc_home.to_path_buf(),
        log_dir: xedoc_home.join("log").to_path_buf(),
        config_lock_export_dir: None,
        config_lock_allow_xedoc_version_mismatch: false,
...
async fn run_turn(thread: &XedocThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    thread
        .submit(Op::UserInput {
    ...
    loop {
        let event = thread.next_event().await.context("read Xedoc event")?;
