...
use xedoc_core_skills::loader::EnvironmentSkillMetadata;
use xedoc_core_skills::loader::load_environment_skills_from_discovery;
use xedoc_core_skills::loader::load_environment_skills_from_root;
use xedoc_exec_server::CapabilityRootDiscoverRequest;
use xedoc_exec_server::CapabilityRootsDiscoverParams;
...
use xedoc_exec_server::discover_capability_roots;
...
async fn loads_nearest_plugin_namespaces_without_reading_unused_sibling_manifests() {
    ...
    for (plugin_root, name) in [
        ...
        fs::create_dir_all(plugin_root.join(".xedoc-plugin")).expect("manifest dir");
        fs::write(
    ...
    let mut expected_manifest_reads = [&outer_root, &inner_root]
        .into_iter()
        .map(|plugin_root| {
            PathUri::from_host_native_path(plugin_root.join(".xedoc-plugin/plugin.json")).unwrap()
        })
        .collect::<Vec<_>>();
...
async fn reuses_walk_inventory_for_missing_skill_metadata() {
    ...
    let root = tempdir().expect("tempdir");
    let manifest_path = root.path().join(".xedoc-plugin/plugin.json");
...
async fn host_loading_reuses_walk_inventory_for_symlinked_skill_pack() {
    ...
    use xedoc_core_skills::SkillMetadata;
    use xedoc_core_skills::SkillPolicy;
    use xedoc_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
    use xedoc_core_skills::loader::SkillRoot;
    use xedoc_core_skills::loader::load_skills_from_roots;
    ...
    let manifest_path = shared_plugin_root.path().join(".xedoc-plugin/plugin.json");
    fs::create_dir_all(manifest_path.parent().expect("manifest parent")).expect("manifest dir");
...
async fn executor_bundle_parser_matches_the_existing_environment_loader() {
    let root = tempdir().expect("tempdir");
    let plugin_manifest = root.path().join(".xedoc-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".xedoc-plugin/plugin.json", "xedoc-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.xedoc-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "xedoc-name:search");
}
