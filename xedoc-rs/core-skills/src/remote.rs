...
pub async fn list_remote_skills(
    ...
    let auth = ensure_xedoc_backend_auth(auth)?;
    ...
    let request = client
        ...
        .query(&query_params)
        .headers(xedoc_model_provider::auth_provider_from_auth(auth).to_auth_headers());
...
pub async fn export_remote_skill(
    ...
    let output_dir = xedoc_home.join("skills").join(skill_id);
