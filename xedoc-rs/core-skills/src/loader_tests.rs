...
use xedoc_exec_server::CopyOptions;
use xedoc_exec_server::CreateDirectoryOptions;
use xedoc_exec_server::ExecutorFileSystem;
use xedoc_exec_server::ExecutorFileSystemFuture;
use xedoc_exec_server::FileMetadata;
use xedoc_exec_server::FileSystemReadStream;
use xedoc_exec_server::FileSystemSandboxContext;
use xedoc_exec_server::LOCAL_FS;
use xedoc_exec_server::ReadDirectoryEntry;
use xedoc_exec_server::RemoveOptions;
use xedoc_exec_server::WalkOptions;
use xedoc_exec_server::WalkOutcome;
...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".xedoc";
...
async fn make_config(xedoc_home: &TempDir) -> TestConfig {
    make_config_for_cwd(xedoc_home, xedoc_home.path().to_path_buf()).await
}
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    layers
        .into_iter()
        .filter_map(|dir| {
            let dot_xedoc = dir.join(REPO_ROOT_CONFIG_DIR_NAME);
            dot_xedoc.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_xedoc_folder: dot_xedoc.abs(),
                    },
...
async fn make_config_for_cwd(xedoc_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = xedoc_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = xedoc_home.path().join("etc/xedoc/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/xedoc");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("xedoc");
...
async fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    fs::create_dir_all(&dot_xedoc)?;

    let user_file = user_folder.join("config.toml").abs();
    let project_dot_xedoc = dot_xedoc.abs();
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_xedoc.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
fn write_skill(xedoc_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&xedoc_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(xedoc_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &xedoc_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
fn write_plugin_manifest(plugin_root: &Path, contents: &str) {
    let manifest_path = plugin_root.join(".xedoc-plugin/plugin.json");
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&xedoc_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let cfg = make_config(&xedoc_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&xedoc_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&xedoc_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&xedoc_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&xedoc_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Xedoc, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(xedoc_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &xedoc_home.path().join("skills/shared"));
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $XEDOC_HOME/skills/cycle/loop -> $XEDOC_HOME/skills/cycle
    let cycle_dir = xedoc_home.path().join("skills/cycle");
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = xedoc_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = xedoc_home.path().join("skills");
    let outcome = load_skills_from_roots(
        [SkillRoot {
...
async fn loads_valid_skill() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&xedoc_home, "demo", "demo-skill", "does things\ncarefully");
    let cfg = make_config(&xedoc_home).await;
...
async fn skips_hidden_and_invalid() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = xedoc_home.path().join("skills/.hidden");
    ...
    let invalid_dir = xedoc_home.path().join("skills/invalid");
...
async fn preserves_overlong_descriptions() {
    ...
    write_skill(&xedoc_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&xedoc_home).await;
    ...
    write_skill(&xedoc_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_xedoc_dir() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn loads_skills_from_all_xedoc_dirs_under_project_root() {
    ...
    let cfg = make_config_for_cwd(&xedoc_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_xedoc_dir_when_not_git_repo() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&xedoc_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&xedoc_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_xedoc_dirs() {
    let xedoc_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&xedoc_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&xedoc_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&xedoc_home, "system", "system-skill", "from system");
