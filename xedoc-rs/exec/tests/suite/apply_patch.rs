...
/// While we may add an `apply-patch` subcommand to the `xedoc` CLI multitool
/// at some point, we must ensure that the smaller `xedoc-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(xedoc_utils_cargo_bin::cargo_bin("xedoc-exec")?)
        .arg(XEDOC_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
