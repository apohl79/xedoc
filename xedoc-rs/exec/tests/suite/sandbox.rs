...
use xedoc_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use xedoc_core::exec::build_exec_request;
    use xedoc_core::sandboxing::SandboxPermissions;

    #[cfg(target_os = "linux")]
    let xedoc_linux_sandbox_exe = Some(
        core_test_support::find_xedoc_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_xedoc_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .xedoc && echo 'sandbox_mode = \"danger-full-access\"' > .xedoc/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .xedoc");

    let status = child.wait().await.expect("should wait for .xedoc command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .xedoc creation: {status:?}"
    );
    let dot_xedoc_metadata = tokio::fs::symlink_metadata(&dot_xedoc).await;
    if let Ok(metadata) = dot_xedoc_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_xedoc.display()
        );
    } else if let Err(err) = &dot_xedoc_metadata {
        assert_eq!(
            err.kind(),
