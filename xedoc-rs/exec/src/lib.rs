...
pub use event_processor_with_jsonl_output::XedocStatus;
...
use xedoc_app_server_client::EnvironmentManager;
use xedoc_app_server_client::ExecServerRuntimePaths;
use xedoc_app_server_client::InProcessAppServerClient;
use xedoc_app_server_client::InProcessClientStartArgs;
use xedoc_app_server_client::InProcessServerEvent;
...
use xedoc_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use xedoc_core::StateDbHandle;
use xedoc_core::check_execpolicy_for_warnings;
use xedoc_core::config::Config;
use xedoc_core::config::ConfigBuilder;
use xedoc_core::config::ConfigOverrides;
use xedoc_core::config::ConfigTomlLoadResult;
use xedoc_core::config::find_xedoc_home;
use xedoc_core::config::load_config_toml_with_layer_stack;
use xedoc_core::config::resolve_oss_provider;
use xedoc_core::config::resolve_profile_v2_config_path;
use xedoc_core::find_thread_meta_by_name_str;
use xedoc_core::format_exec_policy_error_with_source;
...
use xedoc_login::default_client::set_default_originator;
...
use xedoc_otel::set_parent_from_context;
use xedoc_otel::traceparent_context_from_env;
...
enum StdinPromptBehavior {
    ...
    /// `xedoc exec` behavior for `xedoc exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `xedoc exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("codex_exec".to_string()) {
        tracing::warn!(?err, "Failed to set xedoc exec originator override {err:?}");
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&xedoc_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        xedoc_core::otel_init::build_provider(
            &config,
    ...
    xedoc_core::otel_init::record_process_start(otel.as_ref(), "codex_exec");
    xedoc_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "codex_exec");
    ...
    let state_db = xedoc_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_xedoc_home(config.xedoc_home.clone(), Some(local_runtime_paths))
            .await?
    };
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = xedoc_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Xedoc
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            xedoc_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Xedoc initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        xedoc_app_server_protocol::TurnStatus::Failed
                            | xedoc_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<xedoc_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<xedoc_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.xedoc_home, session_id, Some(state_db.as_ref()))
                .await?
