...
#[command(
    version,
    override_usage = "xedoc exec [OPTIONS] [PROMPT]\n       xedoc exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Xedoc outside a Git repository.
    ...
    /// Do not load `$XEDOC_HOME/config.toml`; auth still uses `XEDOC_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "xedoc resume --last <prompt>" treats the positional as a prompt, not a session id.
