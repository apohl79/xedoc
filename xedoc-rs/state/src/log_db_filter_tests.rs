...
async fn sqlite_sink_drops_low_level_opentelemetry_sdk_logs() {
    let xedoc_home =
        std::env::temp_dir().join(format!("xedoc-state-log-db-filter-{}", Uuid::new_v4()));
    ...
    tracing::trace!(target: "xedoc_state", "retained-trace");
    tracing::trace!(
        target: "xedoc_api::responses_websocket_timing",
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("TRACE", "xedoc_state", Some("retained-trace")),
        ]
    );
