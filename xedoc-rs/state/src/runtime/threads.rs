...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(xedoc_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    async fn thread_metadata_round_trips_history_mode() {
        ...
        let mut metadata = test_thread_metadata(&xedoc_home, thread_id, xedoc_home.clone());
    ...
    async fn delete_thread_cleans_associated_state() -> Result<()> {
        let xedoc_home = unique_temp_dir();
        let runtime = StateRuntime::init(xedoc_home.clone(), "test-provider".to_string()).await?;
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = xedoc_home.join("first");
        let second_cwd = xedoc_home.join("second");
        let other_cwd = xedoc_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&xedoc_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn list_threads_uses_indexes_matching_cwd_filters() {
        let xedoc_home = unique_temp_dir();
        let runtime = StateRuntime::init(xedoc_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(xedoc_git_utils::GitSha::new("rollout-sha")),
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let metadata = test_thread_metadata(&xedoc_home, thread_id, xedoc_home.clone());
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&xedoc_home, thread_id, xedoc_home.clone());
        ...
        let mut fallback = test_thread_metadata(&xedoc_home, thread_id, xedoc_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&xedoc_home, first_id, xedoc_home.clone());
        first.updated_at = updated_at;
        first.recency_at = updated_at;
        let mut second = test_thread_metadata(&xedoc_home, second_id, xedoc_home.clone());
        ...
        let mut older = test_thread_metadata(&xedoc_home, older_id, xedoc_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            xedoc_protocol::protocol::TokenCountEvent {
                info: Some(xedoc_protocol::protocol::TokenUsageInfo {
                    total_token_usage: xedoc_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: xedoc_protocol::protocol::TokenUsage::default(),
