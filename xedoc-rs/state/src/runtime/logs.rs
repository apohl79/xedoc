...
impl StateRuntime {
    ...
    pub async fn insert_logs(&self, entries: &[LogEntry]) -> anyhow::Result<()> {
        ...
        builder.push_values(entries, |mut row, entry| {
            ...
            // `LogEntry.message` is only a write-time fallback for callers that
            // still populate the old field.
...
mod tests {
    ...
    async fn insert_logs_use_dedicated_log_database() {
        ...
        let logs_count = log_row_count(logs_db_path(xedoc_home.as_path()).as_path()).await;
    ...
    async fn init_migrates_message_only_logs_db_to_feedback_log_body_schema() {
        ...
        let logs_path = logs_db_path(xedoc_home.as_path());
    ...
    async fn init_configures_logs_db_with_incremental_auto_vacuum() {
        let xedoc_home = unique_temp_dir();
        let _runtime = StateRuntime::init(xedoc_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        let pool = open_db_pool(logs_db_path(xedoc_home.as_path()).as_path()).await;
