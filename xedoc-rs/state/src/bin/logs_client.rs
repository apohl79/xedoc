...
use xedoc_state::LogQuery;
use xedoc_state::LogRow;
use xedoc_state::StateRuntime;

#[derive(Debug, Parser)]
#[command(name = "xedoc-state-logs")]
#[command(about = "Tail Xedoc logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to XEDOC_HOME. Defaults to $XEDOC_HOME or ~/.xedoc.
    #[arg(long, env = "XEDOC_HOME")]
    xedoc_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --xedoc-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let xedoc_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(xedoc_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let xedoc_home = args.xedoc_home.clone().unwrap_or_else(default_xedoc_home);
    Ok(xedoc_state::logs_db_path(xedoc_home.as_path()))
}

fn default_xedoc_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".xedoc");
    }
    PathBuf::from(".xedoc")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["xedoc-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["xedoc-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["xedoc-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["xedoc-state-logs", "--level", "WARN"])
