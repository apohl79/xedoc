...
//!
//! # async fn example(state_db: std::sync::Arc<xedoc_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("xedoc_otel.log_only", LevelFilter::OFF)
        .with_target("xedoc_otel.trace_safe", LevelFilter::OFF)
        .with_target("rmcp::service", LevelFilter::INFO)
        .with_target("xedoc_api::responses_websocket_timing", LevelFilter::OFF)
}
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Xedoc environments.
...
mod tests {
    ...
    fn temp_xedoc_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("xedoc-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let xedoc_home = temp_xedoc_home();
