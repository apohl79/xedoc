...
//! orchestration and rollout scanning live in `xedoc-core`.
...
pub use model::ThreadTitleSource;
...
pub const SQLITE_HOME_ENV: &str = "XEDOC_SQLITE_HOME";
...
pub const DB_ERROR_METRIC: &str = "xedoc.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "xedoc.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "xedoc.db.backfill.duration_ms";
/// SQLite initialization attempts. Tags: [status, phase, db, error]
pub const DB_INIT_METRIC: &str = "xedoc.sqlite.init.count";
/// SQLite initialization latency. Tags: [status, phase, db, error]
pub const DB_INIT_DURATION_METRIC: &str = "xedoc.sqlite.init.duration_ms";
/// Rollout fallback attempts. Tags: [caller, reason]
pub const DB_FALLBACK_METRIC: &str = "xedoc.sqlite.fallback.count";
