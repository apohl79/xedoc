//! Rendering for the TUI's configuration diagnostics.
...
use xedoc_config::NetworkUnixSocketPermissionToml;
use xedoc_config::RequirementSource;
use xedoc_config::ResidencyRequirement;
use xedoc_config::SandboxModeRequirement;
use xedoc_config::WebSearchModeRequirement;
use xedoc_config::format_config_layer_source;
use xedoc_core_config::config::Config;
use xedoc_core_config::config::Permissions;
use xedoc_protocol::models::PermissionProfile;
use xedoc_protocol::permissions::NetworkSandboxPolicy;
use xedoc_tui_render::city_lights::CityLightsStylize;
use xedoc_tui_transcript::history_cell::PlainHistoryCell;
use xedoc_tui_transcript::session_state::SessionNetworkProxyRuntime;
    ...
    if let Some(proxy) = session_network_proxy {
        ...
        let all_proxy = session_all_proxy_url(
            ...
            config
                ...
                .network
                .as_ref()
                .is_some_and(xedoc_core_config::config::NetworkProxySpec::socks_enabled),
        );
...
mod tests {
    ...
    use xedoc_config::ConstraintError;
    use xedoc_config::FeatureRequirementsToml;
    use xedoc_config::FilesystemConstraints;
    use xedoc_config::HookEventsToml;
    use xedoc_config::HookHandlerConfig;
    use xedoc_config::LoaderOverrides;
    use xedoc_config::ManagedHooksRequirementsToml;
    use xedoc_config::MatcherGroup;
    ...
    use xedoc_config::NetworkUnixSocketPermissionToml;
    use xedoc_config::NetworkUnixSocketPermissionsToml;
    use xedoc_config::RequirementSource;
    use xedoc_config::ResidencyRequirement;
    use xedoc_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\xedoc\\config.toml")
        } else {
            absolute_path("/etc/xedoc/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.xedoc")
        } else {
            absolute_path("/repo/.xedoc")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_xedoc_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Xedoc\\requirements.toml")
        } else {
            absolute_path("/etc/xedoc/requirements.toml")
        };
        ...
        let requirements = ConfigRequirements {
            ...
            feature_requirements: Some(Sourced::new(
                FeatureRequirementsToml {
                    entries: BTreeMap::from([("unified_exec".to_string(), true)]),
                },
                RequirementSource::LegacyManagedConfigTomlFromMdm,
            )),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.xedoc\\config.toml")
        } else {
            absolute_path("/home/alice/.xedoc/config.toml")
        };
        let stack = ConfigLayerStack::new(
        ...
        assert!(rendered.contains(&format!(
            "features: unified_exec=true (source: {requirements_source})"
        )));
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/xedoc.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\xedoc")
        } else {
            absolute_path("/var/lib/xedoc")
        };
