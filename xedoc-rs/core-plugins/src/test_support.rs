...
pub(crate) fn write_curated_plugin_sha_with(xedoc_home: &Path, sha: &str) {
    write_file(&xedoc_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(xedoc_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let xedoc_home = AbsolutePathBuf::try_from(xedoc_home).expect("xedoc home should be absolute");
