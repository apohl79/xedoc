...
use tracing::warn;
use xedoc_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use xedoc_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use zip::ZipArchive;

pub use xedoc_core_plugin_marketplace::curated_paths::curated_plugins_api_marketplace_path;
pub use xedoc_core_plugin_marketplace::curated_paths::curated_plugins_repo_path;
...
const CURATED_PLUGINS_FETCH_REF: &str = "refs/xedoc/curated-sync";
...
// Keep this comfortably above a normal sync attempt so we do not race another Xedoc process.
...
pub fn read_curated_plugins_sha(xedoc_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(xedoc_home).as_path())
}

fn curated_plugins_sha_path(xedoc_home: &Path) -> PathBuf {
    xedoc_home.join(CURATED_PLUGINS_SHA_FILE)
}

pub fn sync_openai_plugins_repo(xedoc_home: &Path) -> Result<String, String> {
    #[cfg(target_os = "macos")]
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    let _file_guard = lock_curated_plugins_startup_sync(xedoc_home)?;

    let git_sync_result = match git_binary {
        Some(git_binary) => sync_openai_plugins_repo_via_git(xedoc_home, git_binary),
    ...
    match git_sync_result {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(xedoc_home, api_base_url) {
                Ok(remote_sha) => {
...
fn lock_curated_plugins_startup_sync(xedoc_home: &Path) -> Result<File, String> {
    let lock_path = xedoc_home.join(CURATED_PLUGINS_SYNC_LOCK_FILE);
...
fn sync_openai_plugins_repo_via_git(
    ...
    let repo_path = curated_plugins_repo_path(xedoc_home);
    let sha_path = xedoc_home.join(CURATED_PLUGINS_SHA_FILE);
