...
fn write_curated_plugin_sha(xedoc_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(xedoc_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(xedoc_home.join(".tmp")) else {
        return false;
    };
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_openai_plugins_repo_via_http(xedoc_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.xedoc-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_xedoc_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.xedoc-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.xedoc-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=Xedoc Test",
            "-c",
            "user.email=xedoc@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.xedoc-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.xedoc-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.xedoc-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.xedoc-plugin/plugin.json"),
            options,
        )
