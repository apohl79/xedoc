...
use xedoc_config::clear_user_plugin;
use xedoc_config::set_user_plugin_enabled;
use xedoc_config::types::PluginConfig;
use xedoc_core_skills::PluginSkillSnapshots;
use xedoc_core_skills::config_rules::skill_config_rules_from_stack;
use xedoc_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use xedoc_hooks::plugin_hook_declarations;
use xedoc_plugin::PluginCapabilitySummary;
use xedoc_plugin::PluginId;
use xedoc_plugin::PluginIdError;
use xedoc_plugin::prompt_safe_plugin_description;
...
impl PluginsManager {
    pub fn new(xedoc_home: PathBuf) -> Self {
        Self::new_with_options(xedoc_home, Some(Product::Xedoc), /*auth_mode*/ None)
    }
    ...
    pub(crate) fn new_with_options_and_legacy_store(
        ...
        // contents of that XEDOC_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single XEDOC_HOME is only used by one product.
        Self {
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.xedoc_home.as_path())
                    .ok_or_else(|| {
        ...
        let xedoc_home = self.xedoc_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(xedoc_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.xedoc_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(xedoc_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let manifest =
            if xedoc_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest,
            self.restriction_product,
            &xedoc_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
            /*plugin_skill_snapshots*/ None,
            Arc::clone(&self.skill_root_scan_slots),
    ...
    fn schedule_non_curated_plugin_cache_refresh(
        ...
        let mut configured_plugin_keys =
            configured_plugins_from_stack(&config.config_layer_stack, self.xedoc_home.as_path())
                .into_keys()
                .collect::<Vec<_>>();
        configured_plugin_keys.sort_unstable();
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_openai_plugins_repo(xedoc_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_xedoc_home(xedoc_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            if matches!(
                ...
                let api_marketplace_path =
                    curated_plugins_api_marketplace_path(self.xedoc_home.as_path());
                api_marketplace_path
                    .is_file()
                    .then_some(api_marketplace_path)
            } else {
                let curated_repo_root = curated_plugins_repo_path(self.xedoc_home.as_path());
