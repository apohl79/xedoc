...
use toml::Value;
use xedoc_config::AppToolApproval;
...
use xedoc_config::McpServerOAuthConfig;
use xedoc_config::McpServerToolConfig;
...
use xedoc_core_skills::PluginSkillSnapshots;
use xedoc_core_skills::SkillsLoadInput;
use xedoc_core_skills::SkillsService;
use xedoc_plugin::PluginId;
use xedoc_protocol::auth::AuthMode;
use xedoc_protocol::protocol::HookEventName;
use xedoc_protocol::protocol::Product;
use xedoc_skills::SkillConfigRules;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file =
        AbsolutePathBuf::try_from(xedoc_home.join(CONFIG_TOML_FILE)).expect("absolute config path");
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(xedoc_home, user_config, requirements),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    ...
    write_plugin(
        &xedoc_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = PluginsManager::new(xedoc_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let xedoc_home = TempDir::new().expect("create Xedoc home");
    let marketplace_root = xedoc_home.path().join("marketplace");
...
"#,
    ...
    let err = PluginsManager::new(xedoc_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let xedoc_home = TempDir::new().expect("create Xedoc home");
    let repo_root = xedoc_home.path().join("repo");
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".xedoc-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "xedoc-test@example.com"]);
    run_git(repo, &["config", "user.name", "Xedoc Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&xedoc_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(xedoc_home, xedoc_home).await;
    PluginsManager::new_with_options(xedoc_home.to_path_buf(), Some(Product::Xedoc), auth_mode)
...
fn write_cached_plugin(xedoc_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &xedoc_home
            .join("plugins/cache")
...
"#;
    let outcome =
        load_plugins_from_config(config_toml, xedoc_home.path(), /*auth_mode*/ None).await;
...
#[tokio::test]
async fn api_key_auth_loads_local_curated_plugins() {
    let xedoc_home = TempDir::new().unwrap();
    write_file(
        &xedoc_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    write_cached_plugin(xedoc_home.path(), "openai-curated", "linear");
    write_cached_plugin(xedoc_home.path(), "openai-api-curated", "linear");
...
async fn install_plugin_materializes_default_command_skills() {
    let xedoc_home = TempDir::new().unwrap();
    let source_root = xedoc_home.path().join("source/sample");

    write_file(
        &source_root.join(".xedoc-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".xedoc-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(xedoc_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".xedoc-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".xedoc-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".xedoc-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".xedoc-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".xedoc-plugin/migrated-command-skills")
            .exists()
    );
}
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![xedoc_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![xedoc_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(xedoc_home.path(), xedoc_home.path()).await;
    let outcome = PluginsManager::new(xedoc_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = xedoc_home
        ...
        .join("test/sample/local");
    write_plugin(
        xedoc_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = xedoc_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn skills_service_reuses_skills_parsed_during_plugin_load() {
    let xedoc_home = TempDir::new().unwrap();
    let xedoc_home_abs = xedoc_home.path().to_path_buf().abs();
    let plugin_root = xedoc_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    let skills_input = SkillsLoadInput::new(
        xedoc_home_abs.clone(),
        plugin_outcome.effective_plugin_skill_roots(),
        config.config_layer_stack.clone(),
        /*bundled_skills_enabled*/ false,
    )
    .with_plugin_skill_snapshots(plugin_skill_snapshots);
    let skills_service = SkillsService::new(xedoc_home_abs, /*bundled_skills_enabled*/ false);
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let xedoc_home = TempDir::new().expect("create Xedoc home");
    let marketplace_root = xedoc_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = config_layer_stack_with_requirements(xedoc_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = PluginsManager::new(xedoc_home.path().to_path_buf());
    ...
    assert!(!xedoc_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    write_file(&xedoc_home.path().join(CONFIG_TOML_FILE), &user_config);
    let config =
        config_layer_stack_with_requirements(xedoc_home.path(), &user_config, &requirements);
    let outcome = manager
        .install_plugin(
            &config,
            PluginInstallRequest {
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".xedoc-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".xedoc-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".xedoc-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".xedoc-plugin/plugin.json").is_file());
}
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".xedoc-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".xedoc/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".xedoc-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_xedoc_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_xedoc_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let xedoc_home = TempDir::new().unwrap();
    let project_root = xedoc_home.path().join("project");
    let plugin_root = xedoc_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".xedoc/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_xedoc_folder: AbsolutePathBuf::try_from(project_root.join(".xedoc")).unwrap(),
            },
    ...
    let plugins = load_plugins_from_layer_stack(
        &stack,
        &PluginStore::new(xedoc_home.path().to_path_buf()),
        /*plugin_skill_snapshots*/ None,
        Some(Product::Xedoc),
        Arc::new(Semaphore::new(MAX_CONCURRENT_ROOT_SCANS)),
    )
