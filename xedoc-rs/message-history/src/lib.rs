...
//!
//! The history is stored at `~/.xedoc/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.xedoc`.
...
impl HistoryConfig {
    pub fn new(xedoc_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            xedoc_home: xedoc_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.xedoc_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.xedoc/history.jsonl` and ensure the parent directory exists.
