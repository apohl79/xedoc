...
`xedoc-otel` is the OpenTelemetry integration crate for Xedoc. It provides:

- Provider wiring for log/trace/metric exporters (`xedoc_otel::OtelProvider`
  and `xedoc_otel::provider`).
- Session-scoped business event emission via `xedoc_otel::SessionTelemetry`.
- Low-level metrics APIs via `xedoc_otel::metrics`.
- Trace-context helpers via `xedoc_otel::trace_context` and crate-root re-exports.
...
```rust
use xedoc_otel::config::OtelExporter;
use xedoc_otel::config::OtelHttpProtocol;
use xedoc_otel::config::OtelSettings;
use xedoc_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "xedoc-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    xedoc_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Xedoc
...
Xedoc-specific session events. Rich session/business events should go through
...
metrics.counter("xedoc.turns", 1, &[("model", "gpt-5.1")])?;
