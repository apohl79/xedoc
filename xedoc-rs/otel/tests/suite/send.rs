...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    metrics.gauge_with_description(
        "xedoc.active",
        "Number of active Xedoc operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter = find_metric(&resource_metrics, "xedoc.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Xedoc turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "xedoc-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "xedoc.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "xedoc.tool_latency")
            ...
            .expect("xedoc.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "xedoc.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Xedoc operations.");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "xedoc.alpha").expect("xedoc.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "xedoc.beta").expect("xedoc.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("xedoc.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("xedoc.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
