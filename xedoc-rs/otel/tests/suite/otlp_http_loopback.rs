...
use xedoc_otel::OtelExporter;
use xedoc_otel::OtelHttpProtocol;
use xedoc_otel::OtelProvider;
use xedoc_otel::OtelSettings;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("xedoc.turns", /*inc*/ 1, &[("source", "test")])?;
    ...
    assert!(
        body.contains("xedoc.turns"),
        "expected metric name not found; body prefix: {}",
        &body.chars().take(2000).collect::<String>()
    );
    assert!(
        body.contains("xedoc.active"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "xedoc.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("xedoc.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "xedoc.test.trace_event",
    ...
    assert!(
        body.contains("xedoc.test.trace_event"),
