use uuid::Uuid;
use xedoc_core_context::world_state::WorldStateSnapshot;
use xedoc_core_context_manager::ContextManager;
use xedoc_core_context_manager::is_user_turn_boundary;
...
/// Return value of [`reconstruct_history_from_rollout`], bundling rebuilt history with
/// resume/fork hydration metadata derived from the same replay.
#[derive(Debug)]
pub struct RolloutReconstruction {
    pub history: Vec<ResponseItem>,
    pub previous_turn_settings: Option<ReconstructedTurnSettings>,
...
/// Settings from the latest reconstructed user turn.
...
struct ActiveReplaySegment<'a> {
    ...
    previous_turn_settings: Option<ReconstructedTurnSettings>,
...
fn finalize_active_segment<'a>(
    ...
    previous_turn_settings: &mut Option<ReconstructedTurnSettings>,
...
/// Reconstructs the retained model history and resume metadata from a rollout.
///
/// `rebuild_legacy_compacted_history` preserves the caller-owned compaction format
/// for old rollout records that did not store replacement history.
pub fn reconstruct_history_from_rollout(
    truncation_policy: TruncationPolicy,
    rollout_items: &[RolloutItem],
    rebuild_legacy_compacted_history: &dyn Fn(&[ResponseItem], &str) -> Vec<ResponseItem>,
        ...
        match item {
            ...
            RolloutItem::Compacted(compacted) => {
                if let Some(replacement_history) = &compacted.replacement_history {
                    ...
                    history.replace(rebuild_legacy_compacted_history(
                        history.raw_items(),
                        &compacted.message,
                    ));
                }
