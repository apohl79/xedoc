...
#[ignore = "requires tmux and a locally built xedoc binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let xedoc = xedoc_binary(&repo_root)?;
    ...
    write_config(xedoc_home.path(), &repo_root)?;
    write_auth(xedoc_home.path())?;

    let session_name = format!("xedoc-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("XEDOC_HOME={}", xedoc_home.path().display()))
    ...
    let xedoc_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!xedoc_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&xedoc_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&xedoc_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("xedoc-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&xedoc_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("xedoc-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn xedoc_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = xedoc_utils_cargo_bin::cargo_bin("xedoc") {
        return Ok(path);
    }

    let fallback = repo_root.join("xedoc-rs/target/debug/xedoc");
    anyhow::ensure!(
        fallback.is_file(),
        "xedoc binary is unavailable; run `cargo build -p xedoc-cli` first"
    );
    Ok(fallback)
}

fn write_config(xedoc_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(xedoc_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(xedoc_home: &Path) -> Result<()> {
    std::fs::write(
