...
use xedoc_app_server_protocol::AuthMode as ApiAuthMode;
...
use xedoc_login::read_openai_api_key_from_env;
...
fn onboarding_request_id() -> xedoc_app_server_protocol::RequestId {
    xedoc_app_server_protocol::RequestId::String(Uuid::new_v4().to_string())
}
...
pub(super) async fn cancel_login_attempt(
    ...
    let _ = request_handle
        .request_typed::<xedoc_app_server_protocol::CancelLoginAccountResponse>(
...
impl AuthModeWidget {
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with ChatGPT to use Xedoc as part of your paid plan".into(),
            ]),
            Line::from(vec![
                "  ".into(),
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Decide how much autonomy you want to grant Xedoc".into(),
            Line::from(vec![
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Xedoc will use usage-based billing with your API key.".into(),
        ];
...
mod tests {
    ...
    async fn widget_forced_chatgpt() -> (AuthModeWidget, TempDir) {
        ...
        let config = ConfigBuilder::default()
            .xedoc_home(xedoc_home_path.clone())
            .build()
            .await
            .unwrap();
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            environment_manager: Arc::new(
                xedoc_app_server_client::EnvironmentManager::default_for_tests(),
            ),
