...
async fn test_config(temp_home: &TempDir) -> Config {
    ...
    crate::set_xedoc_cli_version(workspace_version);

    let mut config = ConfigBuilder::default()
        .xedoc_home(temp_home.path().to_path_buf())
...
async fn status_snapshot_uses_default_reasoning_when_config_empty() {
    ...
    let remote_connection = RemoteConnectionStatus {
        address: "unix:///tmp/xedoc-home/app-server-control/app-server-control.sock".to_string(),
