...
use tokio::time;
use xedoc_app_server_client::AppServerPath;
use xedoc_app_server_protocol::AdditionalFileSystemPermissions;
use xedoc_app_server_protocol::AdditionalNetworkPermissions;
use xedoc_app_server_protocol::AdditionalPermissionProfile;
use xedoc_app_server_protocol::AgentMessageDeltaNotification;
use xedoc_app_server_protocol::AskForApproval;
use xedoc_app_server_protocol::CommandExecutionRequestApprovalParams;
use xedoc_app_server_protocol::FileChangeRequestApprovalParams;
use xedoc_app_server_protocol::FileUpdateChange;
use xedoc_app_server_protocol::ItemStartedNotification;
use xedoc_app_server_protocol::JSONRPCErrorError;
use xedoc_app_server_protocol::McpServerElicitationRequest;
use xedoc_app_server_protocol::McpServerElicitationRequestParams;
use xedoc_app_server_protocol::McpServerStartupState;
use xedoc_app_server_protocol::McpServerStatusUpdatedNotification;
use xedoc_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use xedoc_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use xedoc_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use xedoc_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use xedoc_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use xedoc_app_server_protocol::PatchChangeKind;
use xedoc_app_server_protocol::PermissionsRequestApprovalParams;
...
use xedoc_app_server_protocol::ThreadSettingsUpdatedNotification;
use xedoc_app_server_protocol::ThreadStartedNotification;
use xedoc_app_server_protocol::ThreadStatus;
use xedoc_app_server_protocol::ThreadStatusChangedNotification;
use xedoc_app_server_protocol::ThreadTokenUsage;
use xedoc_app_server_protocol::ThreadTokenUsageUpdatedNotification;
use xedoc_app_server_protocol::TokenUsageBreakdown;
...
use xedoc_app_server_protocol::TurnError as AppServerTurnError;
use xedoc_app_server_protocol::TurnStartedNotification;
use xedoc_app_server_protocol::TurnStatus;
use xedoc_app_server_protocol::UserInput;
use xedoc_app_server_protocol::UserInput as AppServerUserInput;
...
use xedoc_protocol::models::NetworkPermissions;
use xedoc_protocol::models::PermissionProfile;
use xedoc_protocol::protocol::EventMsg;
use xedoc_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
use xedoc_protocol::protocol::SessionSource as RolloutSessionSource;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let xedoc_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: xedoc_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_discards_buffered_foreign_activity() -> Result<()> {
    ...
    app.enqueue_primary_thread_notification(
        ...
        sub_agent_activity_notification(
            ...
            xedoc_app_server_protocol::SubAgentActivityKind::Started,
            Some("anthropic"),
            Some("claude-opus-5"),
        ),
    )
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::XedocOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = xedoc_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = xedoc_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(xedoc_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(xedoc_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: xedoc_app_server_protocol::CollabAgentTool::SendInput,
                status: xedoc_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    xedoc_app_server_protocol::CollabAgentState {
                        status: xedoc_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(xedoc_app_server_protocol::ItemCompletedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: xedoc_app_server_protocol::CollabAgentTool::Wait,
                status: xedoc_app_server_protocol::CollabAgentToolCallStatus::Completed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                prompt: None,
                model: None,
                ...
                agents_states: HashMap::from([(
                    receiver_thread_id.to_string(),
                    xedoc_app_server_protocol::CollabAgentState {
                        status: xedoc_app_server_protocol::CollabAgentStatus::Completed,
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(xedoc_app_server_protocol::ItemCompletedNotification {
            ...
            item: ThreadItem::SubAgentActivity {
                ...
                kind: xedoc_app_server_protocol::SubAgentActivityKind::Started,
...
async fn active_sub_agent_activity_updates_active_agent_display_metadata() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        sub_agent_activity_notification_with_current_activity(
            parent_thread_id,
            agent_thread_id,
            xedoc_app_server_protocol::SubAgentActivityKind::Interacted,
            None,
            None,
...
async fn completed_agent_usage_keeps_tokens_but_drops_live_cost() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        sub_agent_activity_notification(
            parent_thread_id,
            agent_thread_id,
            xedoc_app_server_protocol::SubAgentActivityKind::Completed,
            None,
            None,
        ),
    )
...
fn selected_and_resumed_threads_use_server_capability_for_v1_and_v2_children() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        std::fs::write(
            app.config.xedoc_home.join("config.toml"),
            ...
            .enumerate()
        {
            ...
            let session_meta = SessionMeta {
                ...
                originator: "xedoc-tui-test".to_string(),
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::XedocOp(Op::UserTurn { .. })))
        );
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::XedocOp(op)) => op,
        other => panic!("expected XedocOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            xedoc_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            xedoc_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            xedoc_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            xedoc_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: xedoc_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: xedoc_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: xedoc_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: xedoc_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: xedoc_app_server_protocol::SessionSource::SubAgent(
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                cli_version: "0.0.0".to_string(),
                source: xedoc_app_server_protocol::SessionSource::Unknown,
                can_accept_direct_input: None,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            xedoc_app_server_protocol::ServerRequestResolvedNotification {
                thread_id: parent_thread_id.to_string(),
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        xedoc_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            xedoc_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        xedoc_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let xedoc_home = app_server
        .xedoc_home_path(&app.chat_widget.config_ref().xedoc_home)
        .expect("xedoc home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&xedoc_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&xedoc_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{xedoc_home}/../other/{suffix}"),
        format!("{xedoc_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&xedoc_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/xedoc\\home"),
        &format!("/tmp/xedoc/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/xedoc\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/xedoc\\/a");
    let attachments_dir = app.chat_widget.config_ref().xedoc_home.join("attachments");
...
fn test_turn(turn_id: &str, status: TurnStatus, items: Vec<ThreadItem>) -> Turn {
    Turn {
        ...
        items_view: xedoc_app_server_protocol::TurnItemsView::Full,
        items,
        status,
        error: None,
        started_at: None,
...
fn sub_agent_activity_notification(
    ...
    kind: xedoc_app_server_protocol::SubAgentActivityKind,
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        xedoc_error_info: Some(AppServerXedocErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.xedoc/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
        "session {thread_id} is archived. Run `xedoc unarchive {thread_id}` to unarchive it first."
    );
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(xedoc_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*xedoc_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    for (configured_mode, has_explicit_cwd, has_remote_exec, expected_directory) in [
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&xedoc_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(xedoc_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    .expect("materialized rollout should be created");
    let source_path =
        app_test_support::rollout_path(config.xedoc_home.as_path(), filename_ts, &source_thread_id);
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            xedoc_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_xedoc_backend_auth: app.chat_widget.has_xedoc_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        is_first_run: false,
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    xedoc_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: xedoc_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                xedoc_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
...
async fn replace_chat_widget_preserves_custom_status_line_output() {
    let (mut app, _app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let command = xedoc_config::types::StatusLineCommand::Args(vec![
        "xedoc-statusline-test-command".to_string(),
    ]);
    app.chat_widget
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                xedoc_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            sandbox_policy: xedoc_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                xedoc_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.xedoc/skills/abc/SKILL.md"),
