...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `xedoc` to start a saved session, or `xedoc resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let xedoc_home = app_server.xedoc_home_path(&self.config.xedoc_home);
        match goal_files::objective_text_for_edit(app_server, xedoc_home.as_ref(), &goal.objective)
            .await
        {
