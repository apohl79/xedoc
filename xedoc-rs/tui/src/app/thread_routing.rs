...
fn pending_steer_client_user_message_id(thread_id: ThreadId, pending_steer_id: u64) -> String {
    format!("xedoc-tui:{thread_id}:pending-steer:{pending_steer_id}")
}
...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        xedoc_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        xedoc_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | xedoc_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                thread_id,
                                params.server_name.clone(),
                                request_id.clone(),
                                xedoc_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
        ...
        match &op {
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(xedoc_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = xedoc_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().xedoc_home.clone(),
            &self.chat_widget.config_ref().history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                xedoc_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                xedoc_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
    ...
    pub(super) async fn lookup_message_history_batch(
        &mut self,
        thread_id: ThreadId,
        cursor: xedoc_message_history::HistoryBatchCursor,
        log_id: u64,
    ) -> Result<()> {
        let history_config = xedoc_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().xedoc_home.clone(),
        ...
        tokio::spawn(async move {
            let event = match tokio::task::spawn_blocking(move || {
                xedoc_message_history::lookup_batch(log_id, cursor, &history_config)
            })
            .await
            {
                Ok(Ok(batch)) => {
    ...
    pub(super) async fn enqueue_thread_request(
        ...
        if self.primary_thread_id.is_some() && !self.is_primary_or_known_thread(thread_id) {
            if matches!(&request, ServerRequest::ExtensionInteractionRequest { .. })
                && let Some(interactive_request) = self
                    .interactive_request_for_thread_request(thread_id, &request)
                    .await?
            {
                let store = {
