...
async fn start_recording_app_server(
    ...
    let embedded = crate::start_embedded_app_server(
        xedoc_arg0::Arg0DispatchPaths::default(),
        config.clone(),
        Vec::new(),
        xedoc_config::LoaderOverrides::default(),
        /*strict_config*/ false,
        xedoc_config::CloudConfigBundleLoader::default(),
        /*log_db*/ None,
        state_db,
        Arc::new(xedoc_exec_server::EnvironmentManager::default_for_tests()),
    )
    .await?;
    let xedoc_home = config.xedoc_home.display().to_string();
    ...
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            runtime.block_on(async {
                ...
                xedoc_rollout::append_rollout_item_to_path(
                    &rollout_path(
                        xedoc_home.path(),
                        "2026-01-01T00-00-00",
                        &thread_id.to_string(),
