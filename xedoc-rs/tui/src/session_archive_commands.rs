//! Shared implementation for `xedoc archive`, `xedoc delete`, and `xedoc unarchive`.
//!
...
async fn start_app_server_for_archive_command(
    ...
    let xedoc_home = find_xedoc_home().wrap_err("failed to find Xedoc home")?;
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(xedoc_home.as_path()).await
    } else {
        None
    };
