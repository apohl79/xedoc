...
#[cfg(test)]
use xedoc_app_server_protocol::McpAuthStatus;
...
use xedoc_app_server_protocol::SkillErrorInfo;
...
use xedoc_app_server_protocol::WriteStatus;
...
use xedoc_models_manager::model_presets::HIDE_GPT_5_1_XEDOC_MAX_MIGRATION_PROMPT_CONFIG;
use xedoc_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use xedoc_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        xedoc_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).map(|state| {
                    matches!(
                        &state.status,
                        xedoc_app_server_protocol::CollabAgentStatus::PendingInit
                            | xedoc_app_server_protocol::CollabAgentStatus::Running
                    )
                })
            }
            _ => None,
    ...
    let (thread_id, is_running) = match notification {
        ServerNotification::ThreadStarted(notification) => (
            ...
            matches!(
                ...
                xedoc_app_server_protocol::ThreadStatus::Active { .. }
            ),
        ),
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&xedoc_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&xedoc_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[xedoc_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&xedoc_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<xedoc_app_server_protocol::CommandExecutionApprovalDecision> {
    use xedoc_app_server_protocol::CommandExecutionApprovalDecision;
    use xedoc_app_server_protocol::NetworkPolicyRuleAction;
...
fn resume_hint_for_resumable_thread(
    ...
    xedoc_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.xedoc_error_info,
        Some(AppServerXedocErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `xedoc unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_xedoc_backend_auth: self.chat_widget.has_xedoc_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_xedoc_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            xedoc_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("xedoc.status_line", /*inc*/ 1, &[]);
        }
...
See the Xedoc keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
