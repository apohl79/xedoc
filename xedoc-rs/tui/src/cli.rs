...
use clap::Parser;
use xedoc_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `xedoc resume` subcommand.
    // These are not exposed as user flags on the base `xedoc` command.
    #[clap(skip)]
    ...
    /// top-level `xedoc resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `xedoc fork` subcommand.
    // These are not exposed as user flags on the base `xedoc` command.
    #[clap(skip)]
    ...
    /// top-level `xedoc fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
