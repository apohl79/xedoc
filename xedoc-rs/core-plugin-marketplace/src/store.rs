...
impl PluginStore {
    pub fn new(xedoc_home: PathBuf) -> Self {
        Self::try_new(xedoc_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(xedoc_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(xedoc_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(xedoc_home.join(PLUGINS_DATA_DIR))
        ...
        let xedoc_home = AbsolutePathBuf::from_absolute_path_checked(xedoc_home)
            .map_err(|err| PluginStoreError::io("failed to resolve Xedoc home", err))?;
    ...
    #[doc(hidden)]
    pub fn xedoc_home(&self) -> &AbsolutePathBuf {
        &self.xedoc_home
    }
    ...
    #[doc(hidden)]
    pub fn install_with_fallback_manifest(
        &self,
        source_path: AbsolutePathBuf,
        plugin_id: PluginId,
    ...
    #[doc(hidden)]
    pub fn install_with_version_and_fallback_manifest(
        &self,
        source_path: AbsolutePathBuf,
        plugin_id: PluginId,
        plugin_version: String,
...
pub fn error_context_sub_error_type(context: &str) -> String {
...
pub fn plugin_version_for_source_with_fallback_manifest(
    source_path: &Path,
    manifest_contents: &str,
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".xedoc-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".xedoc-plugin/plugin.json");
