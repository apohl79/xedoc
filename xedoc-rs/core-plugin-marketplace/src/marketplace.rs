...
impl MarketplacePluginManifestFallback {
    ...
    #[doc(hidden)]
    pub fn contents_if_has_metadata(&self) -> Option<&str> {
        self.has_metadata.then_some(self.contents())
    }

    #[doc(hidden)]
    pub fn parse_for_plugin_root(
        &self,
        plugin_root: &Path,
...
impl MarketplacePluginSource {
    #[doc(hidden)]
    pub fn is_install_materialized(&self) -> bool {
...
pub struct MarketplacePluginPolicy {
    ...
    // TODO: Surface or enforce product gating at the Xedoc/plugin consumer boundary instead of
...
fn resolve_marketplace_plugin_entry(
    ...
    let manifest = match &source {
        MarketplacePluginSource::Local { path } => {
            if xedoc_utils_plugins::find_plugin_manifest_path(path.as_path()).is_some() {
