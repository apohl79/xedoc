...
use crate::curated_paths::curated_plugins_api_marketplace_path;
use crate::curated_paths::curated_plugins_repo_path;
...
use xedoc_config::MarketplaceAllowedSourceKind;
use xedoc_config::MarketplaceAllowedSourceToml;
use xedoc_config::RequirementSource;
use xedoc_config::types::MarketplaceConfig;
use xedoc_config::types::MarketplaceSourceType;
use xedoc_config::types::PluginConfig;
use xedoc_plugin::PluginId;
use xedoc_utils_absolute_path::AbsolutePathBuf;
use xedoc_utils_path::paths_match_after_normalization;
...
impl MarketplacePolicy {
    #[doc(hidden)]
    pub fn from_requirements(requirements: &ConfigRequirements) -> Self {
    ...
    pub fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(xedoc_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(xedoc_home),
        )
        .ok_or_else(|| {
...
pub fn project_effective_user_config(
    config_layer_stack: &ConfigLayerStack,
    xedoc_home: &Path,
) -> Option<toml::Value> {
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, xedoc_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(xedoc_home, marketplace) {
...
pub fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, xedoc_home) else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(xedoc_home, path)
    {
...
pub fn validate_marketplace_name_for_add(
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(xedoc_home),
    ) {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if paths_match_after_normalization(root.as_path(), curated_plugins_repo_path(xedoc_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(xedoc_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(xedoc_home, path)
}

fn managed_local_marketplace_name(xedoc_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = xedoc_home
    ...
    let runtime_root = dirs::cache_dir()?
        .join("xedoc-runtimes/xedoc-primary-runtime/plugins")
