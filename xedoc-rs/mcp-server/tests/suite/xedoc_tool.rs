...
use xedoc_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use xedoc_mcp_server::ExecApprovalElicitRequestParams;
use xedoc_mcp_server::ExecApprovalResponse;
use xedoc_mcp_server::PatchApprovalElicitRequestParams;
use xedoc_mcp_server::PatchApprovalResponse;
use xedoc_mcp_server::XedocToolCallParam;
use xedoc_protocol::protocol::FileChange;
use xedoc_protocol::protocol::ReviewDecision;
use xedoc_shell_command::parse_command;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "xedoc" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let xedoc_request_id = mcp_process
        .send_xedoc_tool_call(XedocToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            xedoc_request_id.to_string(),
            params.xedoc_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `xedoc` tool call completes and that the file was created.
    let xedoc_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(xedoc_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(xedoc_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    xedoc_mcp_tool_call_id: String,
    xedoc_event_id: String,
    thread_id: xedoc_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Xedoc to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let xedoc_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        xedoc_elicitation: "exec-approval".to_string(),
        xedoc_mcp_tool_call_id,
        xedoc_event_id,
        xedoc_command: command,
        xedoc_cwd: workdir.to_path_buf(),
        xedoc_call_id: "call1234".to_string(),
        xedoc_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "xedoc" tool request that will trigger the apply_patch command
    let xedoc_request_id = mcp_process
        .send_xedoc_tool_call(XedocToolCallParam {
    ...
    // Verify the original `xedoc` tool call completes
    let xedoc_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(xedoc_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_xedoc_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    xedoc_tool_passes_base_instructions()
        .await
        .expect("xedoc tool should pass base instructions");
}

async fn xedoc_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `xedoc mcp` with a specific config.toml.
    let xedoc_home = TempDir::new()?;
    create_config_toml(xedoc_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(xedoc_home.path()).await?;
    ...
    .await??;
    assert_eq!(xedoc_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(xedoc_response.id, RequestId::Number(xedoc_request_id));
    assert_eq!(
        xedoc_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": xedoc_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("xedoc tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Xedoc to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        xedoc_elicitation: "patch-approval".to_string(),
        xedoc_mcp_tool_call_id,
        xedoc_event_id,
        xedoc_reason: reason,
        xedoc_grant_root: grant_root,
        xedoc_changes: changes,
        xedoc_call_id: "call1234".to_string(),
    })?;
...
/// Create a Xedoc config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(xedoc_home: &Path, server_uri: &str) -> std::io::Result<()> {
