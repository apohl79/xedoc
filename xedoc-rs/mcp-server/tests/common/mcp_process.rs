...
impl McpProcess {
    pub async fn new(xedoc_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(xedoc_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = xedoc_utils_cargo_bin::cargo_bin("xedoc-mcp-server")
            .context("should find binary for xedoc-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = xedoc_login::default_client::originator().value;
    ...
    pub async fn send_xedoc_tool_call(
        &mut self,
        params: XedocToolCallParam,
    ) -> anyhow::Result<i64> {
        let xedoc_tool_call_params = CallToolRequestParams::new("xedoc").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(xedoc_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "xedoc/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "xedoc/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `xedoc-mcp-server` child process.
        //
