...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the xedoc tool call.
    #[serde(rename = "threadId")]
    pub thread_id: ThreadId,
    pub xedoc_elicitation: String,
    pub xedoc_mcp_tool_call_id: String,
    pub xedoc_event_id: String,
    pub xedoc_call_id: String,
    pub xedoc_command: Vec<String>,
    pub xedoc_cwd: PathBuf,
    pub xedoc_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    xedoc_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Xedoc to run `{escaped_command}` in `{cwd}`?",
    ...
    let params = ExecApprovalElicitRequestParams {
        ...
        xedoc_mcp_tool_call_id: tool_call_id.clone(),
        xedoc_event_id: event_id.clone(),
        xedoc_call_id: call_id,
    ...
    {
        let xedoc = xedoc.clone();
        let approval_id = approval_id.clone();
        let event_id = event_id.clone();
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, xedoc).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `xedoc`.
