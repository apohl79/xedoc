//! Asynchronous worker that executes a **Xedoc** tool-call inside a spawned
...
use xedoc_core::config::Config as XedocConfig;
...
/// To adhere to MCP `tools/call` response format, include the Xedoc
...
/// Run a complete Xedoc session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_xedoc_tool_session(
    id: RequestId,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start Xedoc session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the Xedoc submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_xedoc_uuid.lock().await.remove(&id);
        return;
    }

    run_xedoc_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_xedoc_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_xedoc_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<XedocThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(xedoc_event_to_notification(&event)) above has
