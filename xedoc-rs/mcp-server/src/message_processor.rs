...
use xedoc_login::default_client::USER_AGENT_SUFFIX;
use xedoc_login::default_client::get_xedoc_user_agent;
use xedoc_protocol::ThreadId;
use xedoc_protocol::protocol::SessionSource;
use xedoc_protocol::protocol::Submission;

use crate::outgoing_message::OutgoingMessageSender;
use crate::xedoc_tool_config::XedocToolCallParam;
use crate::xedoc_tool_config::XedocToolCallReplyParam;
use crate::xedoc_tool_config::create_tool_for_xedoc_tool_call_param;
use crate::xedoc_tool_config::create_tool_for_xedoc_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_xedoc_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        xedoc_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.xedoc_home.clone()),
        );
        let thread_manager = Arc::new(ThreadManager::new(
            config.as_ref(),
            Arc::clone(&auth_manager),
            xedoc_core::build_models_manager(config.as_ref(), auth_manager),
            SessionSource::Mcp,
            environment_manager,
            Arc::new(extensions.build()),
            user_instructions_provider,
            xedoc_core::thread_store_from_config(config.as_ref(), state_db.clone()),
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_xedoc_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("xedoc-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Xedoc");

        // Preserve Xedoc's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_xedoc_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_xedoc_tool_call_param(),
                create_tool_for_xedoc_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "xedoc" => self.handle_tool_call_xedoc(id, arguments).await,
            "xedoc-reply" => {
                self.handle_tool_call_xedoc_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_xedoc(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<XedocToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load Xedoc configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Xedoc tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for xedoc tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result).await;
                return;
            }
        ...
        let running_requests_id_to_xedoc_uuid = self.running_requests_id_to_xedoc_uuid.clone();

        // Spawn an async task to handle the Xedoc session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Xedoc session and stream events back to the client.
            crate::xedoc_tool_runner::run_xedoc_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_xedoc_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_xedoc_session_reply(
        &self,
        ...
        // parse arguments
        let xedoc_tool_call_reply_param: XedocToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<XedocToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Xedoc tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for Xedoc tool: {e}"
                    ))]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for xedoc-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for xedoc-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match xedoc_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let xedoc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::xedoc_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = xedoc_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_xedoc_uuid = running_requests_id_to_xedoc_uuid.clone();

            async move {
                crate::xedoc_tool_runner::run_xedoc_tool_session_reply(
                    thread_id,
                    xedoc,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_xedoc_uuid.lock().await;
        ...
        // Obtain the Xedoc thread from the server.
        let xedoc_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Xedoc.
        if let Err(e) = xedoc_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: xedoc_protocol::protocol::Op::Interrupt,
                client_user_message_id: None,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Xedoc: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_xedoc_uuid
            .lock()
            .await
            .remove(&request_id);
    }
