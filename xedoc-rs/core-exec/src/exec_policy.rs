...
use xedoc_execpolicy::AmendError;
use xedoc_execpolicy::Decision;
use xedoc_execpolicy::Error as ExecPolicyRuleError;
...
use xedoc_execpolicy::PolicyParser;
use xedoc_execpolicy::RuleMatch;
use xedoc_execpolicy::blocking_append_allow_prefix_rule;
use xedoc_execpolicy::blocking_append_network_rule;
use xedoc_protocol::approvals::ExecPolicyAmendment;
use xedoc_protocol::models::PermissionProfile;
use xedoc_protocol::permissions::FileSystemSandboxKind;
use xedoc_protocol::protocol::AskForApproval;
use xedoc_shell_command::is_dangerous_command::DangerousCommandMatch;
use xedoc_shell_command::is_dangerous_command::dangerous_command_match;
use xedoc_shell_command::is_safe_command::is_known_safe_command;
...
pub fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(xedoc_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
impl ExecPolicyManager {
    ...
    pub async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(xedoc_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &xedoc_execpolicy::Error) -> String {
...
pub async fn load_exec_policy_with_warning(
    config_stack: &ConfigLayerStack,
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            xedoc_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            xedoc_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
pub fn default_policy_path(xedoc_home: &Path) -> PathBuf {
    xedoc_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            xedoc_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and xedoc prompts the user to run outside the sandbox
