...
use crate::tool_sandboxing::ToolError;
use std::collections::HashMap;
#[cfg(unix)]
use std::path::Path;
use xedoc_core_environment::shell::Shell;
#[cfg(unix)]
...
use xedoc_network_proxy::is_managed_mitm_ca_trust_bundle_path;
use xedoc_protocol::models::AdditionalPermissionProfile;
use xedoc_sandboxing::SandboxCommand;
...
pub fn is_managed_proxy_env_var(key: &str, value: &str) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        key == PROXY_GIT_SSH_COMMAND_ENV_KEY
            && value.starts_with(XEDOC_PROXY_GIT_SSH_COMMAND_MARKER)
    }
...
/// PATH entries owned by Xedoc runtime setup.
///
...
#[doc(hidden)]
pub struct RuntimePathPrepends {
    entries: Vec<String>,
}

impl RuntimePathPrepends {
    #[cfg(unix)]
    pub fn prepend(&mut self, env: &mut HashMap<String, String>, path_entry: &Path) {
...
#[cfg(unix)]
#[doc(hidden)]
pub fn apply_package_path_prepend(
...
#[cfg(unix)]
#[doc(hidden)]
pub fn prepend_zsh_fork_bin_to_path(
    env: &mut HashMap<String, String>,
    shell_zsh_path: &Path,
) -> Option<String> {
...
#[cfg(unix)]
#[doc(hidden)]
pub fn apply_zsh_fork_path_prepend(
    env: &mut HashMap<String, String>,
    runtime_path_prepends: &mut RuntimePathPrepends,
    shell_zsh_path: &Path,
) {
...
/// runtime-only vars like `XEDOC_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Xedoc-owned PATH entries already applied to
...
pub fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [XEDOC_THREAD_ID_ENV_VAR, XEDOC_PERMISSION_PROFILE_ENV_VAR] {
    ...
    let (override_captures, override_exports) =
        build_override_exports(&override_env, &[XEDOC_PERMISSION_PROFILE_ENV_VAR]);
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__XEDOC_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__XEDOC_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__XEDOC_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__XEDOC_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_xedoc_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_xedoc_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", XEDOC_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__XEDOC_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
