/*
Module: sandboxing

Core-owned adapter types for exec/runtime plumbing. Policy selection and
command transformation live in the xedoc-sandboxing crate; this module keeps
the exec-only metadata and translates transformed sandbox commands back into
ExecRequest for execution.
*/

use crate::exec::ExecCapturePolicy;
use crate::exec::ExecExpiration;
use crate::exec::StdoutStream;
use crate::exec::execute_exec_request;
#[cfg(target_os = "macos")]
use crate::spawn::CODEX_SANDBOX_ENV_VAR;
use crate::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use std::collections::HashMap;
use xedoc_file_system::FileSystemSandboxContext;
use xedoc_network_proxy::ManagedNetworkSandboxContext;
use xedoc_network_proxy::NetworkProxy;
use xedoc_network_proxy::RemoteNetworkProxyLaunchConfig;
use xedoc_protocol::exec_output::ExecToolCallOutput;
use xedoc_protocol::models::PermissionProfile;
pub use xedoc_protocol::models::SandboxPermissions;
use xedoc_protocol::permissions::FileSystemSandboxPolicy;
use xedoc_protocol::permissions::NetworkSandboxPolicy;
use xedoc_sandboxing::SandboxExecRequest;
use xedoc_sandboxing::SandboxType;
use xedoc_utils_absolute_path::AbsolutePathBuf;
use xedoc_utils_path_uri::PathUri;

#[derive(Debug)]
pub struct ExecOptions {
    pub expiration: ExecExpiration,
    pub capture_policy: ExecCapturePolicy,
}

#[derive(Clone, Debug)]
pub struct ExecServerEnvConfig {
    pub policy: xedoc_exec_server::ExecEnvPolicy,
    pub local_policy_env: HashMap<String, String>,
}

#[derive(Debug)]
pub struct ExecRequest {
    pub command: Vec<String>,
    pub cwd: PathUri,
    pub env: HashMap<String, String>,
    pub exec_server_env_config: Option<ExecServerEnvConfig>,
    pub network: Option<NetworkProxy>,
    pub network_environment_id: Option<String>,
    pub expiration: ExecExpiration,
    pub capture_policy: ExecCapturePolicy,
    pub sandbox: SandboxType,
    pub sandbox_policy_cwd: PathUri,
    pub permission_profile: PermissionProfile,
    pub file_system_sandbox_policy: FileSystemSandboxPolicy,
    pub network_sandbox_policy: NetworkSandboxPolicy,
    pub arg0: Option<String>,
    pub exec_server_sandbox: Option<FileSystemSandboxContext>,
    pub exec_server_enforce_managed_network: bool,
    pub exec_server_managed_network: Option<ManagedNetworkSandboxContext>,
    pub exec_server_network_proxy: Option<RemoteNetworkProxyLaunchConfig>,
}

impl ExecRequest {
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        command: Vec<String>,
        cwd: AbsolutePathBuf,
        env: HashMap<String, String>,
        network: Option<NetworkProxy>,
        network_environment_id: Option<String>,
        expiration: ExecExpiration,
        capture_policy: ExecCapturePolicy,
        sandbox: SandboxType,
        permission_profile: PermissionProfile,
        arg0: Option<String>,
    ) -> Self {
        let cwd = PathUri::from_abs_path(&cwd);
        let (file_system_sandbox_policy, network_sandbox_policy) =
            permission_profile.to_runtime_permissions();
        Self {
            command,
            sandbox_policy_cwd: cwd.clone(),
            cwd,
            env,
            exec_server_env_config: None,
            network,
            network_environment_id,
            expiration,
            capture_policy,
            sandbox,
            permission_profile,
            file_system_sandbox_policy,
            network_sandbox_policy,
            arg0,
            exec_server_sandbox: None,
            exec_server_enforce_managed_network: false,
            exec_server_managed_network: None,
            exec_server_network_proxy: None,
        }
    }

    pub fn from_sandbox_exec_request(request: SandboxExecRequest, options: ExecOptions) -> Self {
        let SandboxExecRequest {
            command,
            cwd,
            sandbox_policy_cwd,
            mut env,
            network,
            network_environment_id,
            sandbox,
            permission_profile,
            file_system_sandbox_policy,
            network_sandbox_policy,
            arg0,
        } = request;
        let ExecOptions {
            expiration,
            capture_policy,
        } = options;
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(CODEX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Self {
            command,
            cwd,
            env,
            exec_server_env_config: None,
            network,
            network_environment_id,
            expiration,
            capture_policy,
            sandbox,
            sandbox_policy_cwd,
            permission_profile,
            file_system_sandbox_policy,
            network_sandbox_policy,
            arg0,
            exec_server_sandbox: None,
            exec_server_enforce_managed_network: false,
            exec_server_managed_network: None,
            exec_server_network_proxy: None,
        }
    }
}

pub async fn execute_env(
    exec_request: ExecRequest,
    stdout_stream: Option<StdoutStream>,
) -> xedoc_protocol::error::Result<ExecToolCallOutput> {
    execute_exec_request(exec_request, stdout_stream, /*after_spawn*/ None).await
}

pub async fn execute_exec_request_with_after_spawn(
    exec_request: ExecRequest,
    stdout_stream: Option<StdoutStream>,
    after_spawn: Option<Box<dyn FnOnce() + Send>>,
) -> xedoc_protocol::error::Result<ExecToolCallOutput> {
    execute_exec_request(exec_request, stdout_stream, after_spawn).await
}
