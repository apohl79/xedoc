...
pub use xedoc_protocol::shell_environment::XEDOC_THREAD_ID_ENV_VAR;
...
pub const XEDOC_PERMISSION_PROFILE_ENV_VAR: &str = "XEDOC_PERMISSION_PROFILE";
...
///
/// `XEDOC_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub fn inject_permission_profile_env(
    env: &mut HashMap<String, String>,
    active_permission_profile: Option<&ActivePermissionProfile>,
) {
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(XEDOC_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(XEDOC_PERMISSION_PROFILE_ENV_VAR);
    }
