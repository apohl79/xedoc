...
use xedoc_protocol::protocol::ExecCommandOutputDeltaEvent;
...
pub use xedoc_sandboxing::is_likely_sandbox_denied;
...
use xedoc_utils_pty::process_group::kill_child_process_group;
...
pub fn build_exec_request(
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            xedoc_linux_sandbox_exe: xedoc_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
        })
        .map(|request| ExecRequest::from_sandbox_exec_request(request, options))
        .map_err(XedocErr::from)
}
...
pub async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| XedocErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let raw_output_result = exec(params, network_sandbox_policy, stdout_stream, after_spawn).await;
    let duration = start.elapsed();
    finalize_exec_result(raw_output_result, sandbox, duration)
}

fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, XedocErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(XedocErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(XedocErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(XedocErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        xedoc_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                xedoc_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
