...
pub(crate) use xedoc_tui_render::style;
pub(crate) use xedoc_tui_render::terminal_hyperlinks;
pub(crate) use xedoc_tui_render::wrapping;
...
use url::Url;
use xedoc_utils_string::normalize_markdown_hash_location_suffix;
...
pub use streaming::StreamingMarkdownRender;
pub use streaming::render_streaming_markdown_lines_with_width_and_cwd;
...
pub fn render_markdown_lines_with_width_and_cwd(
    input: &str,
    width: Option<usize>,
    cwd: Option<&Path>,
) -> Vec<HyperlinkLine> {
...
pub fn render_markdown_lines_with_width_cwd_and_hidden_link_destinations(
    input: &str,
    width: Option<usize>,
    cwd: Option<&Path>,
...
///
/// This accepts the path shapes Xedoc emits today: `file://` URLs, absolute and relative paths,
...
mod tests {
    ...
    fn column_classification_token_heavy_for_local_path_lists() {
        ...
        let rows = vec![
            vec![make_cell(
                "xedoc-rs/core/src/next_prompt_suggestion.rs:1, xedoc-rs/core/src/next_prompt_suggestion_tests.rs:1",
            )],
            vec![make_cell(
                "xedoc-rs/core/src/context/next_prompt_suggestion.rs:1, xedoc-rs/core/src/context/contextual_user_message_tests.rs:1",
            )],
        ];
        let metrics = W::collect_table_column_metrics(&header, &rows, /*column_count*/ 1);
        assert_eq!(metrics[0].kind, TableColumnKind::TokenHeavy);
