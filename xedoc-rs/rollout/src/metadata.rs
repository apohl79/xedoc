...
use xedoc_state::BackfillState;
use xedoc_state::BackfillStats;
use xedoc_state::BackfillStatus;
use xedoc_state::DB_ERROR_METRIC;
use xedoc_state::DB_METRIC_BACKFILL;
use xedoc_state::DB_METRIC_BACKFILL_DURATION_MS;
use xedoc_state::ExtractionOutcome;
use xedoc_state::ThreadMetadataBuilder;
use xedoc_state::apply_rollout_item;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = xedoc_otel::global();
    ...
    let sessions_root = xedoc_home.join(SESSIONS_SUBDIR);
    let archived_root = xedoc_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(xedoc_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(xedoc_home: &Path, path: &Path) -> String {
    path.strip_prefix(xedoc_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
