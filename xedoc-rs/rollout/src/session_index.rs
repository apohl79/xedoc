...
pub struct SessionIndexEntry {
    ...
    pub title_source: Option<xedoc_state::ThreadTitleSource>,
...
pub async fn append_thread_name(
    ...
    append_thread_name_with_source(
        ...
        thread_id,
        name,
        Some(xedoc_state::ThreadTitleSource::Manual),
    )
    .await
}
...
pub async fn append_thread_name_with_source(
    ...
    append_session_index_entry(xedoc_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(xedoc_home);
...
pub async fn find_thread_name_by_id(
    ...
    Ok(find_thread_name_record_by_id(xedoc_home, thread_id)
        .await?
        .map(|record| record.0))
}
...
pub async fn find_thread_name_record_by_id(
    xedoc_home: &Path,
    thread_id: &ThreadId,
) -> std::io::Result<Option<(String, Option<xedoc_state::ThreadTitleSource>)>> {
    let path = session_index_path(xedoc_home);
    if !path.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_names_by_ids(
    ...
    Ok(find_thread_name_records_by_ids(xedoc_home, thread_ids)
        .await?
        .into_iter()
...
pub async fn find_thread_name_records_by_ids(
    xedoc_home: &Path,
    thread_ids: &HashSet<ThreadId>,
) -> std::io::Result<HashMap<ThreadId, (String, Option<xedoc_state::ThreadTitleSource>)>> {
    let path = session_index_path(xedoc_home);
...
fn session_index_path(xedoc_home: &Path) -> PathBuf {
    xedoc_home.join(SESSION_INDEX_FILE)
}
