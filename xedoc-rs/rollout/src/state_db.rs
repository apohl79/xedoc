...
pub use xedoc_state::LogEntry;
use xedoc_state::ThreadMetadataBuilder;
use xedoc_utils_path::normalize_for_path_comparison;

/// Core-facing handle to the SQLite-backed state runtime.
pub type StateDbHandle = Arc<xedoc_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        xedoc_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    xedoc_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == xedoc_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, xedoc_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = xedoc_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match xedoc_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: xedoc_otel::MetricsClient,
    originator: &str,
) -> xedoc_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == xedoc_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<xedoc_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&xedoc_state::StateRuntime>,
    xedoc_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.xedoc_home() != xedoc_home {
        warn!(
            "state db xedoc_home mismatch: expected {}, got {}",
            ctx.xedoc_home().display(),
            xedoc_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => xedoc_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => xedoc_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => xedoc_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<xedoc_state::ThreadRelationFilter>,
    archived: bool,
    search_term: Option<&str>,
) -> Option<xedoc_state::ThreadsPage> {
    let ctx = context?;
    if ctx.xedoc_home() != xedoc_home {
        warn!(
            "state db xedoc_home mismatch: expected {}, got {}",
    ...
    let filters = xedoc_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => xedoc_state::SortDirection::Asc,
            SortDirection::Desc => xedoc_state::SortDirection::Desc,
        },
        search_term,
    };
