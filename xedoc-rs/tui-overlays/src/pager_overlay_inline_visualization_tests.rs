...
fn visualization_thread_dir(xedoc_home: &Path, thread_id: ThreadId) -> PathBuf {
...
fn transcript_overlay_remeasures_visualization_when_artifact_becomes_available() {
    ...
    let thread_dir = visualization_thread_dir(xedoc_home.path(), thread_id);
    let context =
        crate::inline_visualization::InlineVisualizationContext::new(xedoc_home.path(), thread_id)
    ...
    let cell = AgentMarkdownCell::new_with_inline_visualizations(
        "::xedoc-inline-vis{file=\"chart.html\"}".to_string(),
        Path::new("/workspace"),
        Some(context),
    );
