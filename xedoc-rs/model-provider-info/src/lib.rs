//! Registry of model providers supported by Xedoc.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Xedoc works out-of-the-box.
//!   2. User-defined entries inside `~/.xedoc/config.toml` under the `model_providers`
...
use xedoc_api::RetryConfig as ApiRetryConfig;
use xedoc_api::is_azure_responses_provider;
...
pub const CHATGPT_XEDOC_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
pub struct ModelProviderInfo {
    ...
    /// Whether this provider supports namespace tools (e.g. multi_agent_v2).
    ...
    ///
    /// When set, Xedoc tracks session cost in USD using the token usage
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> XedocResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> XedocResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> XedocResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These XEDOC_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_xedoc_oss_base_url = format!(
        "http://localhost:{xedoc_oss_port}/v1",
        xedoc_oss_port = std::env::var("XEDOC_OSS_PORT")
            .ok()
    ...
    let xedoc_oss_base_url = std::env::var("XEDOC_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_xedoc_oss_base_url);
    create_oss_provider_with_base_url(&xedoc_oss_base_url, wire_api)
}
