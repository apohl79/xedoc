# xedoc-app-server-daemon

> `xedoc-app-server-daemon` is experimental and its lifecycle contract may
> change.

`xedoc-app-server-daemon` backs the machine-readable `xedoc app-server`
lifecycle commands that run a managed app-server on the Unix control socket.
...
```sh
xedoc app-server daemon start
xedoc app-server daemon restart
xedoc app-server daemon stop
xedoc app-server daemon version
xedoc app-server daemon bootstrap
```
...
`bootstrap` requires the standalone managed install. It starts app-server as a
pidfile-backed detached process and launches a detached updater loop.

## Installation and update cases

The daemon assumes Xedoc is installed through `install.sh` and always launches
the standalone managed binary under `XEDOC_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `XEDOC_HOME/packages/standalone/current/xedoc` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `XEDOC_HOME/packages/standalone/current/xedoc` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
All mutating lifecycle commands are serialized per `XEDOC_HOME`, so a concurrent
`start`, `restart`, `stop`, or `bootstrap` does not race another in-flight
lifecycle operation.

## State

The daemon stores its local state under `XEDOC_HOME/app-server-daemon/`:
