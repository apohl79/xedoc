...
#[cfg(unix)]
use crate::managed_install::resolved_managed_xedoc_bin;
...
async fn update_once(
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_xedoc_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_xedoc_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_xedoc_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Xedoc binary {}",
            managed_xedoc_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get(INSTALLER_URL)
        ...
        .error_for_status()
        .context("standalone Xedoc updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone Xedoc updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Xedoc updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Xedoc updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Xedoc updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Xedoc updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Xedoc updater exited with status {status}")
    }
}
