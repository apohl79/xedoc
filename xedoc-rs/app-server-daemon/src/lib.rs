...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_xedoc_path: PathBuf,
    pub managed_xedoc_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "xedoc app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by xedoc app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance().await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_xedoc_version(managed_xedoc_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(managed_xedoc_bin)
                        .await?;
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_xedoc_version = self
            .managed_xedoc_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_xedoc_version}",
            self.managed_xedoc_bin.display()
        ));
    }
    ...
    async fn start_managed_backend(&self) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(&self.managed_xedoc_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let managed_xedoc_bin = resolved_managed_xedoc_bin(managed_xedoc_bin).await?;
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(managed_xedoc_bin.as_path()));
        backend.start().await
    }

    fn ensure_managed_xedoc_bin(&self) -> Result<()> {
        if self.managed_xedoc_bin.is_file() {
            return Ok(());
        }

        let managed_xedoc_path = self.managed_xedoc_bin.display();
        Err(anyhow!(
    ...
    #[cfg(unix)]
    async fn managed_xedoc_version_best_effort(&self) -> Option<String> {
        managed_xedoc_version(&self.managed_xedoc_bin).await.ok()
    }
    ...
    fn backend_paths(&self) -> BackendPaths {
        self.backend_paths_with_bin(&self.managed_xedoc_bin)
    }
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_xedoc_bin.display(),
                stderr_log.display()
            )
        );
    }
