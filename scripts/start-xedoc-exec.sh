...
remote_path='~/code/xedoc-sync'
local_exec_server_port="${XEDOC_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${XEDOC_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/xedoc-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/xedoc-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/xedoc-sync"
remote_xedoc_rs="$remote_repo_root/xedoc-rs"

cd "${remote_xedoc_rs}"
cargo build -p xedoc-cli --bin xedoc

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/xedoc exec-server --listen ws://127.0.0.1:0 \
...
echo "Start xedoc via: "
printf '  XEDOC_EXEC_SERVER_URL=ws://127.0.0.1:%s xedoc -C %q\n' \
