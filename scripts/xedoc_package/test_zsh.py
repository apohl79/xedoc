...
from xedoc_package.zsh import resolve_zsh_bin
...
class ResolveZshBinTest(unittest.TestCase):
    def test_uses_manifest_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive = root / "xedoc-zsh.tar.gz"
            source = root / "zsh"
            source.write_bytes(b"standalone zsh")
            with tarfile.open(archive, "w:gz") as tar:
                tar.add(source, arcname="xedoc-zsh/bin/zsh")

            manifest = root / "xedoc-zsh"
            manifest.write_text(
                json.dumps(
                    {
                        "platforms": {
            ...
            with patch(
                "xedoc_package.dotslash.default_cache_root",
