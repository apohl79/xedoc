...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertIn(
            f"Could not fetch GitHub release metadata for Xedoc {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find Xedoc package", result.stderr)
    ...
    def test_exact_release_fetches_metadata_once(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                ...
                f"rust-v{VERSION}/xedoc-package_SHA256SUMS",
            ],
        )
    ...
    def test_json_like_strings_and_nested_fields_do_not_define_assets(self) -> None:
        ...
        self.assertIn("/xedoc-npm-", requests[1])
        self.assertNotIn("xedoc-package_SHA256SUMS", requests[1])

    def test_macos_install_exposes_code_mode_host_beside_xedoc(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            current = root / "xedoc-home" / "packages" / "standalone" / "current"
            xedoc_path = install_bin / "xedoc"
            host_path = install_bin / "xedoc-code-mode-host"
            self.assertEqual(os.readlink(xedoc_path), str(current / "bin" / "xedoc"))
            self.assertEqual(
                os.readlink(host_path),
                str(current / "bin" / "xedoc-code-mode-host"),
            )
        ...
        textwrap.dedent(
            ...
            case "$url" in
              https://api.github.com/*)
                if [ "$XEDOC_TEST_METADATA_FAILURE" = "1" ]; then
                  echo "curl: (22) The requested URL returned error: 403" >&2
                  exit 22
                fi
                printf '%s\n' "$XEDOC_TEST_METADATA_JSON"
                ;;
              */xedoc-package_SHA256SUMS)
                if [ -n "$XEDOC_TEST_CHECKSUM_PATH" ]; then
                  cp "$XEDOC_TEST_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              */xedoc-package-*.tar.gz)
                if [ -n "$XEDOC_TEST_ARCHIVE_PATH" ]; then
                  cp "$XEDOC_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
    ...
    env.update(
        {
            ...
            "XEDOC_RELEASE": release,
            "XEDOC_TEST_ARCHIVE_PATH": str(archive_path or ""),
            "XEDOC_TEST_CHECKSUM_PATH": str(checksum_path or ""),
            "XEDOC_TEST_METADATA_FAILURE": "1" if metadata_failure else "0",
            "XEDOC_TEST_METADATA_JSON": (
...
def create_package_release(root: Path) -> tuple[Path, Path, str]:
    ...
    (package_dir / "xedoc-path").mkdir()
    (package_dir / "xedoc-package.json").write_text("{}\n", encoding="utf-8")
    write_executable(
        package_dir / "bin" / "xedoc",
        f"#!/bin/sh\nprintf 'xedoc-cli {VERSION}\\n'\n",
    )
    write_executable(
        package_dir / "bin" / "xedoc-code-mode-host",
        "#!/bin/sh\nexit 0\n",
    )
    write_executable(package_dir / "xedoc-path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = "xedoc-package-aarch64-apple-darwin.tar.gz"
    ...
    checksum_path = root / "xedoc-package_SHA256SUMS"
...
def legacy_release_metadata_with_decoys() -> str:
    ...
    assets = [
        {
            "metadata": {
                "name": "xedoc-package-x86_64-unknown-linux-musl.tar.gz",
                "digest": fake_digest,
            },
            "digest": f"sha256:{'c' * 64}",
            "name": f"xedoc-npm-{target}-{VERSION}.tgz",
        }
    ...
    return json.dumps(
        {
            "body": (
                f'fake: {{"name":"xedoc-package_SHA256SUMS","digest":"{fake_digest}"}}'
            ),
            "assets": assets,
            "tag_name": f"rust-v{VERSION}",
        },
