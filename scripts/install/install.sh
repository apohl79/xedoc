...
validate_version() {
  ...
  if ! printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta)(\.[0-9]+)?)?$'; then
    echo "Invalid Xedoc release version: $version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]." >&2
    exit 1
  fi
}
...
Environment:
  XEDOC_RELEASE          Version to install; overridden by --release.
...
download_file() {
  ...
  echo "curl or wget is required to install Xedoc." >&2
  exit 1
}
...
resolve_release() {
  ...
  if ! release_json="$(download_text "$metadata_url")"; then
    echo "Could not fetch GitHub release metadata for Xedoc $requested_release. GitHub API may be unavailable or rate limited." >&2
    exit 1
  fi

  if ! release_metadata="$(printf '%s\n' "$release_json" | parse_release_metadata)"; then
    echo "Could not parse GitHub release metadata for Xedoc $requested_release." >&2
    exit 1
  fi
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest Xedoc release version." >&2
      exit 1
    fi
...
package_archive_digest() {
  ...
  if [ -z "$digest" ]; then
    echo "Could not find SHA-256 digest for $asset in xedoc-package_SHA256SUMS." >&2
    exit 1
  fi
...
file_sha256() {
  ...
  echo "sha256sum, shasum, or openssl is required to verify the Xedoc download." >&2
  exit 1
}
...
verify_archive_digest() {
  ...
  if [ "$actual_digest" != "$expected_digest" ]; then
    echo "Downloaded Xedoc archive checksum did not match expected digest." >&2
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Xedoc." >&2
    exit 1
  fi
}
...
add_to_path() {
  ...
  begin_marker="# >>> Xedoc installer >>>"
  end_marker="# <<< Xedoc installer <<<"
...
cleanup_stale_install_artifacts() {
  ...
  if [ -d "$BIN_DIR" ]; then
    find "$BIN_DIR" -mindepth 1 -maxdepth 1 -name '.xedoc.*' -exec rm -f {} +
  fi
}
...
version_from_binary() {
  ...
  if [ ! -x "$xedoc_path" ]; then
    return 1
  fi

  "$xedoc_path" --version 2>/dev/null | sed -n 's/.* \([0-9][0-9A-Za-z.+-]*\)$/\1/p' | head -n 1
}

current_installed_version() {
  version="$(version_from_binary "$CURRENT_LINK/bin/xedoc" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi

  version="$(version_from_binary "$CURRENT_LINK/xedoc" || true)"
  if [ -n "$version" ]; then
    printf '%s\n' "$version"
    return 0
  fi
...
resolve_existing_xedoc() {
  command -v xedoc 2>/dev/null || true
}

classify_existing_xedoc() {
  existing_path="$1"
...
print_launch_instructions() {
  case "$path_action" in
    added)
      step "Current terminal: export PATH=\"$BIN_DIR:\$PATH\" && xedoc"
      step "Future terminals: open a new terminal and run: xedoc"
    ...
    configured)
      ...
      ;;
    *)
      step "Current terminal: xedoc"
      step "Future terminals: open a new terminal and run: xedoc"
      ;;
  esac
}

maybe_launch_xedoc_now() {
  if prompt_yes_no "Start Xedoc now?"; then
    step "Launching Xedoc"
    "$BIN_PATH"
  fi
}

detect_conflicting_install() {
  existing_path="$(resolve_existing_xedoc)"
  manager="$(classify_existing_xedoc "$existing_path" || true)"
  ...
  step "Detected existing $manager-managed Xedoc at $existing_path"
  warn "Multiple managed Xedoc installs can be ambiguous because PATH order decides which one runs."
}
...
handle_conflicting_install() {
  ...
  case "$conflict_manager" in
    brew)
      uninstall_cmd="brew uninstall --cask xedoc"
      ;;
    bun)
  ...
  if prompt_yes_no "Uninstall the existing $conflict_manager-managed Xedoc now?"; then
    step "Running: $uninstall_cmd"
    if ! sh -c "$uninstall_cmd"; then
      warn "Failed to uninstall the existing $conflict_manager-managed Xedoc. Continuing with the standalone install."
    fi
  else
    warn "Leaving the existing $conflict_manager-managed Xedoc installed. PATH order will determine which xedoc runs."
  fi
}
...
install_package_release() {
  ...
  chmod 0755 \
    ...
    "$stage_release/bin/xedoc-code-mode-host" \
    "$stage_release/xedoc-path/rg"
  if [ -f "$stage_release/xedoc-resources/bwrap" ]; then
    chmod 0755 "$stage_release/xedoc-resources/bwrap"
  fi
...
install_legacy_platform_npm_release() {
  ...
  mkdir -p "$stage_release/xedoc-resources" "$extract_dir"
  tar -xzf "$archive_path" -C "$extract_dir"

  cp "$vendor_root/xedoc/xedoc" "$stage_release/xedoc"
  cp "$vendor_root/path/rg" "$stage_release/xedoc-resources/rg"
  chmod 0755 "$stage_release/xedoc" "$stage_release/xedoc-resources/rg"
  if [ -f "$vendor_root/xedoc-resources/bwrap" ]; then
    cp "$vendor_root/xedoc-resources/bwrap" "$stage_release/xedoc-resources/bwrap"
    chmod 0755 "$stage_release/xedoc-resources/bwrap"
  fi
...
release_dir_is_complete() {
  ...
  case "$layout" in
    package)
      [ -f "$release_dir/xedoc-package.json" ] &&
        ...
        [ -x "$release_dir/bin/xedoc-code-mode-host" ] &&
        [ -x "$release_dir/xedoc" ] &&
    ...
    legacy-platform-npm)
      [ -x "$release_dir/xedoc" ] &&
        [ -x "$release_dir/xedoc-resources/rg" ] ||
        return 1
      ;;
    *)
      return 1
  ...
  case "$layout:$expected_target" in
    package:*linux* | legacy-platform-npm:*linux*) [ -x "$release_dir/xedoc-resources/bwrap" ] ;;
    *) true ;;
  esac
}
...
release_xedoc_relative_path() {
  release_dir="$1"

  if [ -x "$release_dir/bin/xedoc" ]; then
    printf 'bin/xedoc\n'
  else
    printf 'xedoc\n'
  fi
...
update_visible_command() {
  ...
  xedoc_relative_path="$(release_xedoc_relative_path "$release_dir")"

  replace_path_with_symlink "$BIN_PATH" "$CURRENT_LINK/$xedoc_relative_path" "$tmp_link"

  if [ "$os" = "darwin" ] && [ -x "$release_dir/bin/xedoc-code-mode-host" ]; then
    replace_path_with_symlink \
      "$CODE_MODE_HOST_BIN_PATH" \
      "$CURRENT_LINK/bin/xedoc-code-mode-host" \
      "$tmp_link"
...
resolve_release
package_asset="xedoc-package-$vendor_target.tar.gz"
checksum_asset="xedoc-package_SHA256SUMS"
if release_asset_exists "$package_asset" &&
  release_asset_exists "$checksum_asset"; then
  install_layout="package"
  asset="$package_asset"
elif release_asset_exists "xedoc-npm-$npm_tag-$resolved_version.tgz"; then
  install_layout="legacy-platform-npm"
  asset="xedoc-npm-$npm_tag-$resolved_version.tgz"
else
  echo "Could not find Xedoc package or platform npm release assets for Xedoc $resolved_version." >&2
  exit 1
fi
...
if [ -n "$current_version" ] && [ "$current_version" != "$resolved_version" ]; then
  step "Updating Xedoc CLI from $current_version to $resolved_version"
elif [ -n "$current_version" ]; then
  step "Updating Xedoc CLI"
else
  step "Installing Xedoc CLI"
fi
