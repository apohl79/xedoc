...
Environment:
  XEDOC_RELEASE_TAG     Same as --tag.
  XEDOC_RELEASE_TARGET  Same as --target.
  XEDOC_RELEASE_REPO    Same as --repo.
  XEDOC_LOCAL_ZIP       Same as --local-zip.
...
validate_tag() {
  printf '%s\n' "$1" |
    ...
    die "Invalid Xedoc release tag: $1"
}
...
latest_release_tag() {
  if ! release_json="$(download_text "$(latest_release_metadata_url)")"; then
    die "Could not fetch the latest Xedoc release metadata. GitHub API may be unavailable or rate limited."
  fi

  tag="$(printf '%s\n' "$release_json" | sed -n 's/.*"tag_name":[[:space:]]*"\([^"]*\)".*/\1/p' | head -n 1)"
  [ -n "$tag" ] || die "Could not resolve the latest Xedoc release tag."
  validate_tag "$tag"
  printf '%s\n' "$tag"
}
...
prepare_local_package() {
  ...
  if [ -n "$RELEASE_TAG" ]; then
    ...
    [ "$RELEASE_TAG" = "$local_tag" ] ||
      die "Local package version does not match tag $RELEASE_TAG."
  else
    RELEASE_TAG="$local_tag"
  fi
  ...
  if [ -n "$RELEASE_TARGET" ]; then
    ...
    [ "$RELEASE_TARGET" = "$local_target" ] ||
      die "Local package target does not match target $RELEASE_TARGET."
  else
    RELEASE_TARGET="$local_target"
  fi
}
...
if [ -n "$LOCAL_ZIP" ]; then
  ...
  step "Found Xedoc release asset"
  printf 'Repository: %s\n' "$RELEASE_REPO"
  printf 'Asset:      %s\n' "$asset"
  printf 'SHA256:     %s\n' "$expected_digest"
  printf 'URL:        %s\n' "$download_url"
