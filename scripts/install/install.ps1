[CmdletBinding()]
param(
    [string]$Release = $env:XEDOC_RELEASE
)
...
$NonInteractive = $env:XEDOC_NON_INTERACTIVE -match "^(?i:1|true|yes)$"
...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-(?:alpha|beta)(?:\.[0-9]+)?)?$") {
        throw "Invalid Xedoc release version: $Version. Expected latest or x.y.z[-alpha[.N]|-beta[.N]]."
    }
}
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Xedoc archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Get-PackageArchiveDigest {
    ...
    throw "Could not find SHA-256 digest for $AssetName in xedoc-package_SHA256SUMS."
}
...
function Resolve-Release {
    ...
    try {
        ...
        throw "Could not fetch GitHub release metadata for Xedoc $requestedRelease. GitHub API may be unavailable or rate limited. $($_.Exception.Message)"
    }

    if ($normalizedVersion -eq "latest") {
        if (-not $releaseMetadata.tag_name) {
            throw "Failed to resolve the latest Xedoc release version."
        }
...
function Get-VersionFromBinary {
    ...
    if (-not (Test-Path -LiteralPath $XedocPath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $XedocPath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -XedocPath (Join-Path $StandaloneCurrentDir "bin\xedoc.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -XedocPath (Join-Path $StandaloneCurrentDir "xedoc.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Test-OldStandaloneBinLayout {
    ...
    $requiredFiles = @("xedoc.exe", "rg.exe")
    ...
    $knownFiles = @(
        "xedoc.exe",
        "rg.exe",
        "xedoc-command-runner.exe",
        "xedoc-windows-sandbox.exe",
        "xedoc-windows-sandbox-setup.exe"
    )
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Xedoc install at $VisibleBinDir"
    Write-WarningStep "To continue, Xedoc needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Xedoc setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'XedocInstaller.Junction').Type) {
        return
    }
...
function Set-JunctionTarget {
    ...
    [XedocInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Test-PackageContentsAreComplete {
    ...
    $expectedFiles = @(
        "xedoc-package.json",
        "bin\xedoc.exe",
        "bin\xedoc-code-mode-host.exe",
        "xedoc-path\rg.exe",
        "xedoc-resources\xedoc-command-runner.exe",
        "xedoc-resources\xedoc-windows-sandbox-setup.exe"
    )
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        ...
        default {
            throw "Unknown Xedoc installer layout: $Layout"
        }
    }
...
function Get-ExistingXedocCommand {
    $existing = Get-Command xedoc -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingXedocManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingXedocCommand
    $manager = Get-ExistingXedocManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed Xedoc at $existingPath"
    Write-WarningStep "Multiple managed Xedoc installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    if (Prompt-YesNo "Uninstall the existing $manager-managed Xedoc now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Xedoc. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Xedoc installed. PATH order will determine which xedoc runs."
    }
}

function Test-VisibleXedocCommand {
    param(
        [string]$VisibleBinDir
    )

    $xedocCommand = Join-Path $VisibleBinDir "xedoc.exe"
    & $xedocCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Xedoc command failed verification: $xedocCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Xedoc requires a 64-bit version of Windows."
    exit 1
}
...
$xedocHome = if ([string]::IsNullOrWhiteSpace($env:XEDOC_HOME)) {
    Join-Path $env:USERPROFILE ".xedoc"
} else {
    $env:XEDOC_HOME
}
$standaloneRoot = Join-Path $xedocHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\Xedoc\bin"
if ([string]::IsNullOrWhiteSpace($env:XEDOC_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:XEDOC_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Xedoc CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Xedoc CLI"
} else {
    Write-Step "Installing Xedoc CLI"
}
...
$packageAsset = "xedoc-package-$target.tar.gz"
$checksumAsset = "xedoc-package_SHA256SUMS"
...
if ($null -eq $packageMetadata -or $null -eq $checksumMetadata) {
    $packageAsset = "xedoc-npm-$npmTag-$resolvedVersion.tgz"
    $packageMetadata = Find-ReleaseAssetMetadata -AssetName $packageAsset -ReleaseMetadata $releaseMetadata
    if ($null -ne $packageMetadata) {
        $installLayout = "LegacyPlatformNpm"
    } else {
        throw "Could not find Xedoc package or platform npm release assets for Xedoc $resolvedVersion."
    }
    $checksumMetadata = $null
}
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("xedoc-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading Xedoc CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Xedoc package archive did not contain the expected package layout."
                }
            } else {
                ...
                $resourcesDir = Join-Path $stagingDir "xedoc-resources"
                New-Item -ItemType Directory -Force -Path $resourcesDir | Out-Null
                $copyMap = @{
                    "xedoc/xedoc.exe" = "xedoc.exe"
                    "xedoc/xedoc-command-runner.exe" = "xedoc-resources\xedoc-command-runner.exe"
                    "xedoc/xedoc-windows-sandbox-setup.exe" = "xedoc-resources\xedoc-windows-sandbox-setup.exe"
                    "path/rg.exe" = "xedoc-resources\rg.exe"
                }
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Xedoc npm archive did not contain the expected legacy platform package layout."
                }
            }
        ...
        try {
            ...
            Test-VisibleXedocCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: xedoc"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: xedoc"
Write-Host "Xedoc CLI $resolvedVersion installed successfully."

$xedocCommand = Join-Path $visibleBinDir "xedoc.exe"
if (Prompt-YesNo "Start Xedoc now?") {
    Write-Step "Launching Xedoc"
    & $xedocCommand
}
