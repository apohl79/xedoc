...
work_root="$(mktemp -d "${temp_root%/}/xedoc-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/xedoc-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${XEDOC_WRAPPER_LOG:?missing XEDOC_WRAPPER_LOG}"
printf '%s\n' "$@" > "$XEDOC_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
XEDOC_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" xedoc-zsh)
