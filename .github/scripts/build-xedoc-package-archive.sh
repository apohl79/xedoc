...
usage() {
  cat <<'EOF'
Usage: build-xedoc-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-manifest <path>] \
  [--xedoc-command-runner-bin <path>] \
  [--xedoc-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --zsh-manifest)
      ...
      shift 2
      ;;
    --xedoc-command-runner-bin)
      resource_args+=(
        --xedoc-command-runner-bin
        "${2:?--xedoc-command-runner-bin requires a value}"
      )
      command_runner_bin_provided="true"
      shift 2
      ;;
    --xedoc-windows-sandbox-setup-bin)
      resource_args+=(
        --xedoc-windows-sandbox-setup-bin
        "${2:?--xedoc-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="xedoc"
    entrypoint="xedoc"
    archive_stem="xedoc-package"
    ;;
  app-server)
    variant="xedoc-app-server"
    entrypoint="xedoc-app-server"
    archive_stem="xedoc-app-server-package"
    ;;
  *)
    echo "No Xedoc package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/xedoc-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/xedoc-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/xedoc-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--xedoc-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--xedoc-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_xedoc_package.py"
  --target "$target"
  --variant "$variant"
