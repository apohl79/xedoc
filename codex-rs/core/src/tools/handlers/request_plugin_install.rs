...
/// Elicitation server name kept for client compatibility with the former Codex Apps flow.
const REQUEST_PLUGIN_INSTALL_ELICITATION_SERVER: &str = "codex_apps";
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        {
            let DiscoverableTool::Plugin(plugin) = &tool;
        ...
        let elicitation = session
            .request_mcp_server_elicitation(
                turn.as_ref(),
                REQUEST_PLUGIN_INSTALL_ELICITATION_SERVER.to_string(),
                request_id,
                request,
            )
            .await;
        ...
        let completed = if user_confirmed {
            verify_request_plugin_install_completed(&session, &tool, auth.as_ref()).await
        } else {
            false
        };
...
async fn verify_request_plugin_install_completed(
    ...
    if is_remote_plugin_install_suggestion(&plugin.id) {
        refresh_remote_installed_plugins_cache_after_install(session, auth, plugin.id.as_str())
            .await;
        return false;
    }
...
async fn refresh_remote_installed_plugins_cache_after_install(
    ...
    let plugins_config = session.get_config().await.plugins_config_input();
