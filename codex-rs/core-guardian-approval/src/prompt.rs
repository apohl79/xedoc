...
use crate::format_guardian_action_pretty;
...
pub fn build_guardian_prompt_items(
    ...
    history_version: u64,
    parent_turn_denied_reads_context: Option<String>,
    retry_reason: Option<String>,
    request: GuardianApprovalRequest,
    mode: GuardianPromptMode,
) -> serde_json::Result<GuardianPromptItems> {
    let transcript_entries = collect_guardian_transcript_entries(history);
    let transcript_cursor = GuardianTranscriptCursor {
        parent_history_version: history_version,
    ...
    if let Some(denied_reads_context) = parent_turn_denied_reads_context {
...
pub const BUNDLED_GUARDIAN_POLICY: &str = include_str!("policy.md");
pub const BUNDLED_GUARDIAN_POLICY_TEMPLATE: &str = include_str!("policy_template.md");
...
pub fn guardian_policy_prompt_with_config_and_template(
