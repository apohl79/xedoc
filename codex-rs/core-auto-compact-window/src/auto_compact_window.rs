//! Auto-compaction window state.

use codex_protocol::protocol::TokenUsage;
use uuid::Uuid;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Identifiers tracked for one auto-compaction window.
pub struct AutoCompactWindowIds {
    /// Identifier of the first window in the session.
    pub first_window_id: Uuid,
    /// Identifier of the preceding window when one exists.
...
impl AutoCompactWindowIds {
    /// Creates the initial compaction-window identifiers.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Snapshot of the active window's prefill accounting.
pub struct AutoCompactWindowSnapshot {
    /// Input tokens included before the current window's body.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Mutable accounting state for one auto-compaction window.
...
impl AutoCompactWindow {
    /// Creates window state from persisted identifiers.
    pub fn new_with_ids(ids: AutoCompactWindowIds) -> Self {
        Self {
            window_number: 0,
            ids,
    ...
    /// Clears the prefill baseline for a new context window.
    pub fn clear_prefill(&mut self) {
        self.prefill_input_tokens = None;
    }

    /// Returns the zero-based auto-compaction window number.
    pub fn window_number(&self) -> u64 {
        self.window_number
    }

    /// Returns the identifiers for the active window.
    pub fn ids(&self) -> AutoCompactWindowIds {
        self.ids
    }

    /// Restores persisted window state.
    pub fn restore(&mut self, window_number: u64, ids: AutoCompactWindowIds) {
        self.window_number = window_number;
        self.ids = ids;
    }

    /// Advances to a fresh window and returns its state.
    pub fn advance(&mut self) -> (u64, AutoCompactWindowIds) {
    ...
    /// Claims the token-budget reminder for the active window.
    pub fn claim_token_budget_reminder(&mut self) -> bool {
        !std::mem::replace(&mut self.token_budget_reminder_delivered, true)
    }

    /// Claims the auto-compaction fallback for the active window.
    pub fn claim_auto_compact_fallback(&mut self) -> bool {
        !std::mem::replace(&mut self.auto_compact_fallback_delivered, true)
    }

    /// Requests a new context window.
    pub fn request_new_context_window(&mut self) {
        self.new_context_window_requested = true;
    }

    /// Consumes the pending new-window request.
    ...
    /// Records a server-observed prefill baseline when none exists.
    pub fn ensure_server_observed_prefill_from_usage(&mut self, usage: &TokenUsage) {
        if matches!(
    ...
    /// Records an estimated prefill baseline until server usage is available.
    pub fn set_estimated_prefill(&mut self, tokens: i64) {
        if matches!(
            self.prefill_input_tokens,
            Some(AutoCompactWindowPrefill::ServerObserved(_))
        ) {
    ...
    /// Returns the public prefill accounting snapshot.
