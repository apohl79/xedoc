...
const REALTIME_TURN_TOKEN_BUDGET: usize = 300;
...
/// Builds the bounded model context for a realtime conversation.
pub async fn build_realtime_startup_context(
    ...
    let workspace_section = build_workspace_section_with_user_root(cwd, user_root).await;
...
/// Truncates text while preserving the requested token budget.
