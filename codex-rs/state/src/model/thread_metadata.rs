...
/// Source of the persisted user-facing thread title.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ThreadTitleSource {
    /// Derived from transcript metadata such as the first user message.
    #[default]
    Derived,
    /// Generated by a side-band model request.
    Generated,
    /// Explicitly supplied by the user.
    Manual,
}

impl ThreadTitleSource {
    pub fn as_str(self) -> &'static str {
        match self {
            ThreadTitleSource::Derived => "derived",
            ThreadTitleSource::Generated => "generated",
            ThreadTitleSource::Manual => "manual",
        }
    }
}

impl TryFrom<&str> for ThreadTitleSource {
    ...
    fn try_from(value: &str) -> std::result::Result<Self, Self::Error> {
        match value {
            ...
            value => Err(anyhow::anyhow!("invalid thread title source: {value}")),
        }
    }
}
...
impl ThreadMetadata {
    ...
    pub fn prefer_existing_explicit_title(&mut self, existing: &Self) {
        ...
        if existing.title_source != ThreadTitleSource::Derived {
            self.title = existing.title.clone();
            self.title_source = existing.title_source;
            return;
        }

        if existing.first_user_message.as_deref().map(str::trim) == Some(existing_title) {
            return;
        }
...
mod tests {
    ...
    fn prefer_existing_explicit_title_preserves_generated_source() {
        ...
        existing.title = "Configurable Session Names".to_string();
        existing.title_source = super::ThreadTitleSource::Generated;

        next.prefer_existing_explicit_title(&existing);
