...
pub const IGNITION_FRAME_TICK: Duration = Duration::from_millis(33);

pub fn effort_animation_enabled(animations_enabled: bool, color_level: StdoutColorLevel) -> bool {
    animations_enabled
        && matches!(
            color_level,
...
impl IgnitionStyle {
    ...
    pub fn random(previous: Option<Self>) -> Self {
...
impl EffortTier {
    pub fn from_effort(effort: Option<&ReasoningEffort>) -> Option<Self> {
        match effort {
    ...
    pub fn prompt(self, charge: f32) -> Span<'static> {
        self.prompt_for(
            charge,
            default_fg(),
            default_bg(),
...
impl EffortIgnition {
    pub fn new(tier: EffortTier, style: IgnitionStyle) -> Self {
        Self {
            tier,
            style,
    ...
    pub fn render(&self, area: Rect, protected: Rect, buf: &mut Buffer) -> bool {
        if area.is_empty() {
            return false;
        }
