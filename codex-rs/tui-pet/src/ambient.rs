...
pub struct AmbientPetDraw {
    ...
    pub rows: u16,
    pub height_px: u16,
    pub sixel_dir: PathBuf,
}

#[derive(Debug)]
pub struct AmbientPet {
    pet: Pet,
...
impl AmbientPet {
    ...
    pub fn set_notification(&mut self, kind: PetNotificationKind, body: Option<String>) {
    ...
    pub fn image_columns(&self) -> u16 {
        self.image_size().columns
    }

    #[doc(hidden)]
    pub fn set_image_support_for_tests(&mut self, support: PetImageSupport) {
    ...
    pub fn draw_request(&self, area: Rect, composer_bottom_y: u16) -> Option<AmbientPetDraw> {
    ...
    pub fn preview_draw_request(&self, area: Rect) -> Option<AmbientPetDraw> {
        let protocol = self.support.protocol()?;
        let size = self.image_size();
...
#[doc(hidden)]
pub fn test_ambient_pet(frame_requester: FrameRequester, animations_enabled: bool) -> AmbientPet {
    AmbientPet {
        pet: Pet {
            id: "test".to_string(),
