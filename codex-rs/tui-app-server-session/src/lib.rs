...
use codex_tui_status::status::plan_type_display_name;
use codex_tui_transcript::session_state::MessageHistoryMetadata;
use codex_tui_transcript::session_state::ThreadSessionState;
use codex_tui_transcript::version::codex_cli_version;
...
impl AppServerSession {
    ...
    pub fn codex_home_path(&self, local_codex_home: &AbsolutePathBuf) -> Option<AppServerPath> {
    ...
    pub async fn memory_reset(&mut self) -> Result<()> {
        let request_id = self.next_request_id();
...
mod tests {
    ...
    use codex_core_config::config::ConfigBuilder;
    use codex_core_config::config::ConfigOverrides;
    ...
    async fn start_embedded_app_server_for_picker(
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "codex-tui-app-server-session-test".to_string(),
    ...
    async fn persisted_resume_applies_model_default_service_tier() -> Result<()> {
        ...
        let mut preset = codex_tui_test_support::TEST_MODEL_PRESETS
            .iter()
